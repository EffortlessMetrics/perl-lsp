...
- **Tokens**: Template variables like `{{version}}`, `{{tests.passed}}`, `{{docs.missing_docs}}`
...
- `{{tests.ignored}}` - Number of ignored tests
- `{{tests.active_tests}}` - Total active tests (passed + failed)
- `{{tests.total_all_tests}}` - All tests (active + ignored)
- `{{tests.pass_rate_active}}` - Pass rate of active tests (%)
- `{{tests.pass_rate_total}}` - Overall pass rate including ignored (%)

#### Documentation Metrics

- `{{docs.missing_docs}}` - Number of missing doc warnings from `cargo doc`
...
```markdown
{{test_passed}}        → Works (legacy)
{{tests.passed}}       → Works (canonical)
{{missing_docs}}       → Works (legacy)
{{docs.missing_docs}}  → Works (canonical)
```
...
1. **Use tokens for all dynamic values**
   ...
   ✅ We have {{tests.passed}} passing tests

   ❌ Version 0.8.8 includes...
   ✅ Version {{version}} includes...

   ❌ 484 missing docs warnings
   ✅ {{docs.missing_docs}} missing docs warnings
   ```
