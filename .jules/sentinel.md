...
## 2025-02-12 - [Debug Adapter Safe Evaluation Bypass]
**Vulnerability:** The "Safe Evaluation Mode" in the Perl Debug Adapter (used for hover tooltips) allowed dangerous operations like `system` and `exec` if they were package-qualified (e.g., `POSIX::system`) but not prefixed with `CORE::`. It also failed to block declarations like `use` and `package`, which can cause side effects.
**Learning:** The logic relied on an allow-list for package-qualified names (`is_package_qualified_not_core`) to support benign method calls, but this created a bypass for standard library modules like `POSIX`. Security validation should default to deny-all for sensitive contexts, especially when string-based regex matching is involved.
**Prevention:** Removed the package-qualification bypass entirely. Now, any identifier matching a dangerous operation (e.g., "system") is blocked regardless of its prefix, unless it is part of a strictly controlled safe-list (like variable names `$system`). Added `use`, `no`, and `package` to the blocklist.
