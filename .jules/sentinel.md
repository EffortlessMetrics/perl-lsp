...
## 2026-10-26 - Safe Evaluation Bypass via Readline Operator
**Vulnerability:** The `perl-dap` safe evaluation mode failed to block the readline operator `<...>` (e.g., `<$fh>`) and file globbing (e.g., `<file>`). This allowed expressions evaluated on hover to trigger I/O operations and mutate file handle state, bypassing the intended "no side effects" guarantee.
**Learning:** Operators that rely on syntax sugar or context (like `<` being both comparison and readline) are difficult to filter with regexes. Ambiguous syntax requires conservative blocking in security contexts, even if it inconveniences some valid use cases (like requiring spaces for comparison).
**Prevention:** Explicitly block ambiguous operators when they appear in a dangerous context (e.g., `<` followed immediately by a variable or word). Enforce coding style (spaces around binary operators) in secure evaluation contexts to distinguish them from unary/circumfix operators. Also, ensure the blocklist includes all mutating builtins, including implicit mutators like `chomp`.
