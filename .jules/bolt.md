...
## 2025-08-07 - [Regex Compilation in Loops]
**Learning:** Compiling regexes inside frequently called functions (like AST traversal) is a massive performance killer (1400x slowdown). Even "simple" regexes add up.
**Action:** Always use `std::sync::OnceLock` (or `lazy_static`) for static regexes in hot paths.
