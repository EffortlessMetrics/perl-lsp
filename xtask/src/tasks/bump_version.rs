...
fn update_readme_version(version: &str) -> Result<()> {
    ...
    let new_content = regex.replace_all(&content, format!(r#"perl-parser = "{}""#, major_minor)).to_string();
    
    // Update VSIX filename references
    let regex = Regex::new(r"perl-language-server-\d+\.\d+\.\d+\.vsix")?;
    let new_content = regex.replace_all(&new_content, format!("perl-language-server-{}.vsix", version)).to_string();
