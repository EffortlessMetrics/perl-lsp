...
fn verify_features() -> Result<()> {
    ...
    for feature in &catalog.feature {
        match feature.maturity.as_str() {
            ...
            other => errors.push(format!("Unknown maturity '{}' for feature {}", other, feature.id)),
        }
    }
    ...
    let snapshot_path = test_dir.join("snapshots/lsp_features_snapshot_test__advertised_vs_caps.snap");
                ...
                if let Some(yaml_start) = content.find("---\n") {
                    ...
                    match serde_yaml::from_str::<serde_yaml::Value>(yaml_content) {
                        Ok(yaml) => {
                            ...
                            let catalog_advertised: std::collections::BTreeSet<String> = catalog.feature.iter()
                            ...
                            if let Some(caps) = yaml.get("caps").and_then(|v| v.as_sequence()) {
                                let caps_set: std::collections::BTreeSet<String> = caps.iter()
                                    .filter_map(|v| v.as_str().map(String::from))
                                    .collect();
                                
                                // Check for mismatches
                                let missing_in_caps: Vec<_> = catalog_advertised.difference(&caps_set).collect();
                                let extra_in_caps: Vec<_> = caps_set.difference(&catalog_advertised).collect();
                                ...
                                warnings.push("Could not find 'caps' array in snapshot".to_string());
                            }
                        }
    ...
    let non_planned = catalog.feature.iter()
        .filter(|f| f.maturity != "planned")
        .count();
    let advertised_ga_prod = catalog.feature.iter()
        .filter(|f| f.advertised && (f.maturity == "ga" || f.maturity == "production"))
        .count();
    ...
    println!("📊 Computed compliance: {}% ({}/{} non-planned features)", 
    ...
    if !errors.is_empty() {
        ...
        return Err(color_eyre::eyre::eyre!("Feature verification failed with {} errors", errors.len()));
    }
