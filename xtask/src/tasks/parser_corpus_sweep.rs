...
const SEMANTIC_BUCKETS: &[(&str, &str)] = &[
    ...
    (
        "expected expression, found 'unless'",
        "unexpected_token_unless",
    ),
    (
        "expected expression, found 'until'",
        "unexpected_token_until",
    ),
    (
        "expected expression, found 'while'",
        "unexpected_token_while",
    ),
    ("expected expression, found 'else'", "unexpected_token_else"),
    (
        "expected expression, found 'elsif'",
        "unexpected_token_elsif",
    ),
    ("expected expression, found 'for'", "unexpected_token_for"),
    (
        "expected expression, found 'foreach'",
        "unexpected_token_foreach",
    ),
    ...
    (
        ...
        "unexpected_token_local",
    ),
    ("expected expression, found 'sub'", "unexpected_token_sub"),
    (
        "expected expression, found 'package'",
        "unexpected_token_package",
    ),
...
pub fn run(config: SweepConfig) -> Result<()> {
    ...
    let default_profile = if config.manifest_path.is_some() {
        "common".to_string()
    } else {
        "system".to_string()
    };
    ...
    let report = SweepReport {
        ...
        first_error_buckets,
        files_by_bucket,
        file_results: if config.verbose {
            file_results
        } else {
            Vec::new()
        },
    ...
    if use_manifest && config.enforce {
        ...
        println!(
            "Strict clean: all {} files parse without errors",
            report.total_files
        );
    }
...
pub fn print_summary(report: &SweepReport) {
    ...
    if !report.slowest_files.is_empty() {
        println!(
            "\n--- Top {} slowest files by parse time ---",
            report.slowest_files.len()
        );
    ...
    if !report.file_results.is_empty() {
        ...
        let mut by_errors: Vec<&FileResult> = report
            .file_results
            .iter()
            .filter(|r| r.error_node_count > 0)
            .collect();
        if !by_errors.is_empty() {
            by_errors.sort_by(|a, b| {
                b.error_node_count
                    .cmp(&a.error_node_count)
                    .then_with(|| a.path.cmp(&b.path))
            });
...
mod tests {
    ...
    fn test_normalize_error_bucket_trailing_at() {
        ...
        assert_eq!(
            normalize_error_bucket("some rare error at 99"),
            "some rare error",
        );
    }
    ...
    fn test_semantic_bucket_mapping() {
        let cases = [
            ...
            (
                "expected expression, found '=>' at 10",
                "unexpected_fat_arrow_expr",
            ),
            ("expected expression, found '->'", "unexpected_arrow_expr"),
            (
                "expected expression, found '/' at 5",
                "unexpected_slash_expr",
            ),
            ("expected expression, found '?'", "unexpected_question_expr"),
            (
                "expected expression, found 'return' at 99",
                "unexpected_return_expr",
            ),
            (
                "expected expression, found SomeOtherToken",
            ...
            (
                "Expected module name or version string",
                "expected_module_name",
            ),
            ("Expected '>' to close angle bracket", "unclosed_angle"),
            (
                "Substitution operator should be s///",
                "substitution_misparse",
            ),
            (
                "Invalid syntax at position 42: Expected variable, found X",
                "expected_variable",
            ),
    ...
    fn test_enforce_ratchet_no_violations() {
        let report = test_report(
            ...
            2,
            BTreeMap::from([
                ("unclosed_brace".to_string(), 10),
                ("unclosed_paren".to_string(), 8),
            ]),
        );
        ...
        assert!(
            violations.is_empty(),
            "Expected no violations when report equals baseline"
        );
    }
    ...
    fn test_enforce_ratchet_regression() {
        let baseline = test_report(
            ...
            25,
            2,
            BTreeMap::from([("unclosed_brace".to_string(), 10)]),
        );
    ...
    fn test_enforce_strict_clean_all_clean() {
        ...
        assert!(
            violations.is_empty(),
            "Expected no violations for all-clean report"
        );
    }
    ...
    fn test_enforce_strict_clean_with_errors() {
        let report = test_report(
            ...
            5,
            0,
            BTreeMap::from([("unclosed_brace".to_string(), 2)]),
        );
        let violations = enforce_strict_clean(&report);
        assert_eq!(
            violations.len(),
    ...
    fn test_enforce_strict_clean_with_unreadable() {
        ...
        assert_eq!(
            violations.len(),
            1,
            "Expected 1 violation for unreadable files"
        );
    ...
    fn test_normalize_error_bucket_first_match_wins() {
        ...
        assert_eq!(
            normalize_error_bucket("expected '}', found ';'"),
            "unclosed_brace_semicolon",
        );
    }
    ...
    fn test_parse_manifest_whitespace_only() {
        ...
        assert!(
            modules.is_empty(),
            "Whitespace-only manifest should yield no modules"
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn test_parse_manifest_whitespace_and_comments_only() {
        ...
        assert!(
            modules.is_empty(),
            "Whitespace+comments-only manifest should yield no modules"
        );
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn test_enforce_strict_clean_all_three_violations() {
        let report = test_report(
            ...
            7,
            2,
            BTreeMap::from([("unclosed_brace".to_string(), 3)]),
        );
        let violations = enforce_strict_clean(&report);
        assert_eq!(
            violations.len(),
    ...
    fn test_enforce_ratchet_improvement_no_violations() {
        ...
        assert!(
            violations.is_empty(),
            "Improvements should not trigger violations"
        );
    }
    ...
    fn test_enforce_ratchet_bucket_disappeared() {
        ...
        assert!(
            violations.is_empty(),
            "Disappeared bucket should not be a violation"
        );
    }
    ...
    fn test_enforce_ratchet_empty_baselines() {
        ...
        assert!(
            violations.is_empty(),
            "Empty baselines should have no violations"
        );
    }
    ...
    fn test_files_by_bucket_populated_for_error_files() {
        ...
        fbb.insert(
            ...
            vec!["Foo.pm".to_string(), "Bar.pm".to_string()],
        );
        let report = SweepReport {
            files_by_bucket: fbb,
            ..test_report(
    ...
    fn test_files_by_bucket_serde_roundtrip() {
        ...
        let report = SweepReport {
            ...
            ..test_report(
                ...
                1,
                0,
                BTreeMap::from([("expected_semicolon".to_string(), 1)]),
            )
        };
    ...
    fn test_files_by_bucket_absent_in_old_schema_deserializes_empty() {
        ...
        assert!(
            ...
            "files_by_bucket should default to empty"
        );
    }
    ...
    fn test_files_by_bucket_not_examined_by_ratchet() {
        ...
        let baseline = SweepReport {
            ...
            ..test_report(
                ...
                1,
                0,
                BTreeMap::from([("unclosed_brace".to_string(), 1)]),
            )
        };
        let report = SweepReport {
            files_by_bucket: fbb_b,
            ..baseline.clone()
        };
        let violations = enforce_ratchet(&report, &baseline);
        assert!(
            violations.is_empty(),
            "files_by_bucket changes should not affect ratchet"
        );
    }
    ...
    fn test_portable_report_path_relativizes_workspace_paths() {
        ...
        assert_eq!(
            portable_report_path(&path),
            "target/cpan-corpus/lib/perl5/Foo.pm"
        );
    }
    ...
    fn test_compute_median_error_density_all_clean() {
        ...
        let measurements = vec![
            measurement("a.pm", 1, 100, 0),
            measurement("b.pm", 1, 200, 0),
        ];
    ...
    fn test_compute_median_error_density_ignores_zero_lines() {
        ...
        assert!(
            (density - 100.0).abs() < 1e-6,
            "expected 100.0, got {density}"
        );
    }
    ...
    fn test_compute_median_error_density_even_count_averages() {
        ...
        let density = compute_median_error_density(&measurements).expect("some");
        assert!(
            (density - 15.0).abs() < 1e-6,
            "expected 15.0, got {density}"
        );
    }
    ...
    fn test_sweep_report_schema_1_3_0_roundtrip() {
        ...
        let json = serde_json::to_string(&report).expect("serialize");
        assert!(
            json.contains("\"phase_timings\""),
            "phase_timings key missing"
        );
        assert!(
            json.contains("\"discovery_ms\":12"),
            "discovery_ms value missing"
        );
        assert!(
        ...
        assert!(
            json.contains("\"slowest_files\""),
            "slowest_files key missing"
        );

        let back: SweepReport = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(back.phase_timings, report.phase_timings);
        assert_eq!(
            back.median_error_density_per_1k_loc,
            report.median_error_density_per_1k_loc
        );
