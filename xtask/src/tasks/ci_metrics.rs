...
struct BaselineSummary {
    ...
    overall_signal_per_dollar: Option<f64>,
}
...
fn build_baseline_report(
    ...
    let mut runs_by_sha: HashMap<String, Vec<CompletedRun>> = HashMap::new();
    ...
    for run in runs {
            ...
            .or_else(|| run.get("head_sha").and_then(Value::as_str))
        {
            runs_by_sha
                .entry(sha.to_string())
                .or_default()
                .push(CompletedRun { workflow_key: key, failed: conclusion != "success" });
        }
    }
    ...
    for completed_runs in runs_by_sha.values() {
        let mut failed_lanes = completed_runs.iter().filter(|run| run.failed);
        if let Some(first_failure) = failed_lanes.next()
            && failed_lanes.next().is_none()
        {
            *unique_catches.entry(first_failure.workflow_key.clone()).or_default() += 1;
        }
    }
    ...
    for (key, counters) in workflow_counters {
        ...
        let unique_catch_count = *unique_catches.get(&key).unwrap_or(&0);
        workflow_reports.insert(
            ...
            BaselineWorkflow {
                ...
                unique_catch_count,
                unique_catch_rate_percent: percent(unique_catch_count, counters.failure_count),
                signal_per_dollar: signal_per_dollar(unique_catch_count, counters.billable_minutes),
            },
        );
    }
    ...
    Some(BaselineReport {
        ...
        summary: BaselineSummary {
            ...
            overall_signal_per_dollar: signal_per_dollar(total_unique_catches, total_billable),
        },
    })
}
...
fn signal_per_dollar(unique_catches: u64, billable_minutes: u64) -> Option<f64> {
    if unique_catches == 0 || billable_minutes == 0 {
        return None;
    }
    Some(round_two_decimals(unique_catches as f64 / (billable_minutes as f64 * COST_PER_MINUTE)))
}
...
fn build_baseline_markdown(report: &BaselineReport) -> Result<String> {
    ...
    let overall_signal = report
        .summary
        .overall_signal_per_dollar
        .map(|value| format!("{value:.2} catches/$"))
        .unwrap_or_else(|| "n/a".to_string());
    out.push_str(&format!("| Overall Signal per $ | {} |\n\n", overall_signal));
    ...
    out.push_str(
        "- Unique Catch: A commit where exactly one lane fails (candidate single-lane catch)\n",
    );
    out.push_str("- Signal/$: Unique catches divided by estimated run cost\n");
...
mod tests {
    ...
    fn baseline_report_keeps_zero_minute_runs_and_excludes_skips_from_success_rate() -> Result<()> {
        ...
        assert_eq!(workflow.unique_catch_count, 0);
        assert_eq!(workflow.unique_catch_rate_percent, 0.0);
        assert_eq!(workflow.signal_per_dollar, None);
        ...
        assert_eq!(report.summary.overall_signal_per_dollar, None);

        let markdown = build_baseline_markdown(&report)?;
        assert!(markdown.contains("| CI | 3 | 50.0% | 90s | 90s | 2m | 0 | 0.0% | n/a |"));

        Ok(())
    }

    #[test]
    fn baseline_report_only_counts_single_failure_commits_as_unique_catches() -> Result<()> {
        let generated_at =
            DateTime::parse_from_rfc3339("2026-03-25T12:00:00Z")?.with_timezone(&Utc);
        let cutoff = DateTime::parse_from_rfc3339("2026-03-24T12:00:00Z")?.with_timezone(&Utc);
        let runs = vec![
        ...
        let report = build_baseline_report("master", 1, generated_at, cutoff, &runs)
            .ok_or_else(|| eyre!("expected baseline report"))?;
        let ci = report.workflows.get("CI").ok_or_else(|| eyre!("missing CI"))?;
        let lint = report.workflows.get("Lint").ok_or_else(|| eyre!("missing Lint"))?;
        assert_eq!(ci.failure_count, 2);
        assert_eq!(ci.unique_catch_count, 1);
        assert_eq!(ci.unique_catch_rate_percent, 50.0);
        assert_eq!(lint.failure_count, 1);
        assert_eq!(lint.unique_catch_count, 0);
