...
pub fn fetch_list(config: &CpanCorpusConfig) -> Result<()> {
    println!(
        "Fetching top {} distributions from MetaCPAN...",
        config.top_n
    );
    ...
    let distributions: Vec<&str> = response
        .hits
        .hits
        .iter()
        .map(|h| h._source.name.as_str())
        .collect();
    ...
    writeln!(
        file,
        "# CPAN top {} distributions by reverse dependency count",
        config.top_n
    )?;
    writeln!(
        file,
        "# Auto-generated by: cargo xtask cpan-corpus fetch-list"
    )?;
...
pub fn install(config: &CpanCorpusConfig) -> Result<()> {
    ...
    let local_lib = config
        .install_dir
        .canonicalize()
        .unwrap_or_else(|_| config.install_dir.clone());
    let normalized_distributions: Vec<String> = distributions
        .iter()
        .map(|dist| normalize_distribution_for_cpanm(dist))
        .collect();
    ...
    for (batch_idx, chunk) in normalized_distributions.chunks(batch_size).enumerate() {
        ...
        println!(
            "Batch {batch_num}/{total_batches}: installing {} distributions...",
            chunk.len()
        );
    ...
    let status = loop {
        match child.try_wait().context("Failed to poll cpanm process")? {
            ...
            None if Instant::now() >= deadline => {
                ...
                break child
                    .wait()
                    .context("Failed to wait for timed-out cpanm process")?;
            }
...
fn install_distributions_individually(
    ...
    for dist in chunk {
        ...
        if output.timed_out {
            ...
            println!(
                "  timed out after {}s: {}",
                CPANM_SINGLE_DIST_TIMEOUT.as_secs(),
                dist
            );
            continue;
        }
...
fn bootstrap_cpanm_script(config: &CpanCorpusConfig) -> Result<PathBuf> {
    ...
    println!(
        "System cpanm not found; bootstrapping standalone cpanm to {}",
        script_path.display()
    );
...
pub fn ratchet(config: &CpanCorpusConfig) -> Result<()> {
    ...
    if new_modules.is_empty() {
        println!(
            "All {} clean modules already in manifest.",
            clean_modules.len()
        );
        return Ok(());
    }
...
mod tests {
    ...
    fn test_path_to_module_name() {
        ...
        assert_eq!(
            path_to_module_name("/opt/cpan/lib/perl5/auto/DBI/DBI.so", &lib),
            None,
        );
    }
    ...
    fn test_default_config() {
        ...
        assert_eq!(
            config.dist_list,
            root.join(".ci/cpan-top-1000-distributions.txt")
        );
    ...
    fn test_bootstrap_cpanm_path() {
        ...
        assert_eq!(
            bootstrap_cpanm_path(&install_dir),
            install_dir.join("bin").join("cpanm")
        );
    }
    ...
    fn test_normalize_distribution_for_cpanm() {
        ...
        assert_eq!(
            normalize_distribution_for_cpanm("ExtUtils-MakeMaker"),
            "ExtUtils::MakeMaker"
        );
        assert_eq!(
