...
fn generate_trend_recommendations(
    ...
    match direction {
        TrendDirection::Increasing => {
            recommendations
                .push("Consider implementing systematic ignored test resolution plan".to_string());
    ...
    if data.len() > 10 {
        ...
        if recent_variance > 10.0 {
            recommendations.push(
                "High variance in ignored test count indicates inconsistent progress".to_string(),
            );
        }
    }
    ...
    for (test_metadata, expected_min_score, description) in quality_test_cases {
        ...
        if test_metadata.target_timeline.as_secs() > 0 {
            ...
            if test_metadata.target_timeline > max_reasonable_timeline {
                println!(
                    "Warning: Timeline may be too long for complexity level: {:?} -> {:?}",
...
fn test_comprehensive_ci_guardrail_integration() -> Result<()> {
    ...
    for (scenario, should_pass_ci, description) in ci_scenarios {
        ...
        for (crate_name, count) in &scenario.per_crate_changes {
            ...
            {
                if *count > limit {
                    per_crate_violations.push(format!(
                        "Crate '{}' has {} ignored tests, limit is {}",
                        crate_name, count, limit
                    ));
                }
            }
        }
        ...
        for test in &scenario.new_tests {
                    ...
                    < guardian.governance.quality_gates.ci_validation.min_quality_score
            {
                quality_failures.push(format!(
                    "Test '{}' failed quality validation (score: {:.1})",
                    test.test_name, validation.quality_score
                ));
            }
        }
        ...
        let ci_blocking_conditions = vec![
            (
                guardian.governance.quality_gates.ci_validation.block_on_count_increase
                    && regression_result.is_regression,
                "Count increase regression",
            ),
            (!per_crate_violations.is_empty(), "Per-crate limit violations"),
            (!quality_failures.is_empty(), "Quality validation failures"),
        ];

        let has_blocking_conditions =
            ci_blocking_conditions.iter().any(|(condition, _)| *condition);
        ...
        if should_pass_ci {
            assert!(
                ci_should_pass
                    || (!regression_result.is_regression
                        && per_crate_violations.is_empty()
                        && quality_failures.is_empty()),
                "{}: CI validation failed unexpectedly",
                description
            );
        } else {
            assert!(
                !ci_should_pass || has_blocking_conditions,
                "{}: CI should have failed but passed",
                description
            );
        }
        ...
        if !ci_should_pass {
            ...
            if regression_result.is_regression {
                failure_reasons.push(format!(
                    "Baseline regression: {} tests (baseline: {}, limit: +{})",
