...
fn test_max_backtrack_steps_constant() {
    ...
    assert_eq!(perl_lexer::MAX_BACKTRACK_STEPS, 100_000, "MAX_BACKTRACK_STEPS should be 100,000");
}
...
fn test_normal_patterns_under_backtrack_limit() {
    ...
    for input in test_cases {
        ...
        let has_unknown_rest =
            tokens.iter().any(|t| matches!(t.token_type, TokenType::UnknownRest));
        assert!(!has_unknown_rest, "Normal pattern should not trigger backtrack limit: {}", input);
...
fn test_boundary_backtrack_limit() {
    ...
    assert!(has_regex, "Pattern under MAX_BACKTRACK_STEPS should parse successfully");
}
...
fn test_backtrack_limit_performance() {
    ...
    let has_unknown_rest = tokens.iter().any(|t| matches!(t.token_type, TokenType::UnknownRest));
    assert!(has_unknown_rest, "Large pattern should trigger backtracking limit");
}
