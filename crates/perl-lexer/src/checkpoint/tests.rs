...
fn test_find_before_binary_search() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(cache.find_before(30).ok_or("find_before(30) should hit")?.position, 30);
    assert_eq!(cache.find_before(25).ok_or("find_before(25) should hit")?.position, 20);
    assert_eq!(cache.find_before(100).ok_or("find_before(100) should hit")?.position, 50);
