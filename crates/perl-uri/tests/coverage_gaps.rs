...
mod mojibake_repair_branches {
    ...
    /// The raw decoding of `%C3%83%C2%A9` produces `ÃÂ©` (the Ã/Â markers),
    /// so `looks_like_mojibake` returns true, the repair loop collects the bytes,
    /// and the candidate `café` has fewer markers.
    ///
    ...
    ///
    /// `%C3%83` decodes to "Ã" (mojibake marker, U+00C3 = 195 ≤ 255).
    /// `%F0%9F%9A%80` decodes to "🚀" (U+1F680, code point 128640 > 255).
    /// When the repair loop hits the emoji char, `u8::try_from(0x1F680)` returns Err.
    #[test]
    fn mojibake_with_high_codepoint_not_repaired() {
        // %C3%83 = Ã (mojibake marker, U+00C3 = 195)
        // %F0%9F%9A%80 = 🚀 (U+1F680, code point 128640)
    ...
    /// the valid UTF-8 encoding of "ÃÃ" (two U+00C3 chars = two Ã mojibake markers).
    ///
    /// `repair_mojibake_text("...ÃÃ...")` sees mojibake markers (Ã), collects the byte
    /// values of all chars: for Ã→0xC3, Ã→0xC3, so the relevant bytes are [0xC3, 0xC3].
    ...
    ///
    /// `%C3%83%C3%83` percent-decodes to bytes [0xC3, 0x83, 0xC3, 0x83] which is
    /// the valid UTF-8 encoding of "ÃÃ".  The repair loop collects Ã→0xC3, Ã→0xC3.
    ...
    fn mojibake_with_invalid_utf8_bytes_not_repaired() -> Result<(), String> {
        // %C3%83 = Ã (U+00C3, mojibake marker) — encoded twice.
        ...
        // The path contains the original "ÃÃ" (not repaired to some other form).
        if !s.contains("ÃÃ") {
    ...
    /// text is returned.
    ///
    /// `%C3%83%C2%83` decodes to "Ã\u{0083}" (Ã marker + U+0083 C1 control).
    /// The repair loop collects Ã→0xC3, U+0083→0x83.
    /// `String::from_utf8([..., 0xC3, 0x83, ...])` succeeds → "Ã" (still a marker!).
    /// `mojibake_marker_count("Ã") = 1` is NOT less than `mojibake_marker_count("Ã\u{0083}") = 1`.
    #[test]
    fn mojibake_repair_candidate_not_better() -> Result<(), String> {
        // %C3%83 = Ã (U+00C3, mojibake marker)
        // %C2%83 = U+0083 (C1 control)
        // Decoded: "Ã\u{0083}" — one Ã marker.
        // Repair: bytes [0xC3, 0x83] → "Ã" — still one Ã marker.
