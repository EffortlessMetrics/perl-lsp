...
fn is_file_uri_returns_true_for_file_triple_slash() {
    assert!(
        is_file_uri("file:///tmp/test.pl"),
        "file:/// must be file URI"
    );
}
...
fn is_file_uri_returns_true_for_file_double_slash_with_host() {
    ...
    assert!(
        is_file_uri("file://localhost/path/to/file.pl"),
        "file:// with host must be file URI"
    );
}
...
fn is_file_uri_returns_false_for_https() {
    assert!(
        !is_file_uri("https://example.com/file.pl"),
        "https must not be file URI"
    );
}
...
fn is_file_uri_returns_false_for_untitled() {
    assert!(
        !is_file_uri("untitled:Untitled-1"),
        "untitled must not be file URI"
    );
}
...
fn is_special_scheme_true_for_untitled() {
    assert!(
        is_special_scheme("untitled:Untitled-1"),
        "untitled: is special"
    );
}
...
fn is_special_scheme_true_for_vscode_vfs() {
    assert!(
        is_special_scheme("vscode-vfs:/mount/path"),
        "vscode-vfs: is special"
    );
}
...
fn is_special_scheme_false_for_file() {
    assert!(
        !is_special_scheme("file:///tmp/test.pl"),
        "file: is not special"
    );
}
...
fn is_special_scheme_false_for_https() {
    ...
    assert!(
        is_special_scheme("https://example.com"),
        "https: scheme != file → special"
    );
}
...
fn is_special_scheme_invalid_uri_with_git_prefix() {
    ...
    assert!(
        is_special_scheme("git:something"),
        "git: must always be special"
    );
}
...
fn uri_key_already_lowercase_drive_unchanged() {
    ...
    assert!(
        key.starts_with("file:///d:"),
        "Lowercase drive must stay lowercase: {key}"
    );
}
