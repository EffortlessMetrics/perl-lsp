                ...
                if i < len && bytes[i] == b'?' {
                    ...
                    if i < len && bytes[i] == b'<' {
                        ...
                        if let Some((name, next_pos)) = parse_named_capture_name_from(bytes, i, b'>') {
                            capture_index += 1;
                            i = next_pos;
                            let sub = collect_group_sub_pattern(bytes, len, &mut i);
...
fn collect_group_sub_pattern(bytes: &[u8], len: usize, i: &mut usize) -> String {
    let pattern_start = *i;
    ...
    if *i > 0 && pattern_start < *i - 1 {
        return String::from_utf8_lossy(&bytes[pattern_start..*i - 1]).into_owned();
    }
    String::new()
}

fn describe_modifier(modifier: char) -> Option<&'static str> { match modifier {
...
fn parse_named_capture_name(bytes: &[u8], pos: usize, open_delim: u8, close_delim: u8) -> Option<(String, usize)> {
    if pos >= bytes.len() || bytes[pos] != open_delim { return None; }
    let mut i = pos + 1;
    let name_start = i;
    while i < bytes.len() && bytes[i] != close_delim { i += 1; }
    if i == name_start || i >= bytes.len() { return None; }
...
fn parse_named_capture_name_from(bytes: &[u8], start: usize, close_delim: u8) -> Option<(String, usize)> {
    ...
    let mut i = start;
    while i < bytes.len() && bytes[i] != close_delim { i += 1; }
    if i == start || i >= bytes.len() { return None; }
