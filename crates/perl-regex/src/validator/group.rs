    ...
    pub(crate) fn push(
        ...
        match group {
            GroupType::Lookbehind => {
                let depth = self.stack.iter().filter(|g| matches!(g, GroupType::Lookbehind)).count();
                if depth >= config.max_nesting {
                    return Err(RegexError::syntax("Regex lookbehind nesting too deep", start_pos + offset));
                }
            }
