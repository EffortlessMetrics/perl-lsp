        ...
        match ch {
            ...
            b'(' => {
                if i + 1 < len && bytes[i + 1] == b'?' {
                    i += 2;
                    if i < len && matches!(bytes[i], b':' | b'=' | b'!' | b'<' | b'>' | b'|' | b'P' | b'#') {
                        i += 1;
                    }
                } else {
                    i += 1;
            ...
            b'+' | b'*' | b'?' | b'{' => {
                if last_type == 2 {
                    if ch == b'{' {
                        let mut peek_i = i + 1;
                        if is_brace_quantifier(bytes, &mut peek_i) {
                            return true;
                        }
                        last_type = 0;
                        i += 1;
                ...
                if let Some(last) = group_stack.last_mut() { *last = true; }
                last_type = 1;
            }
            _ => last_type = 0,
        }
...
fn is_brace_quantifier(bytes: &[u8], i: &mut usize) -> bool {
    ...
    while *i < bytes.len() {
        ...
        *i += 1;
        ...
        else if ch == b',' && !has_comma { has_comma = true; }
        else if ch == b'}' && has_digit { return true; }
        else { break; }
    }
    false
}
