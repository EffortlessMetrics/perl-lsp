...
pub(crate) fn check_complexity(
    ...
    while let Some(event) = scanner.next_event() {
        match event {
            RegexEvent::UnicodeProperty { offset } => {
                unicode_properties.observe(start_pos + offset)?
            }
            RegexEvent::GroupStart { kind, offset } => {
                groups.push(kind, offset, start_pos, config)?
            }
            RegexEvent::Alternation { offset } => {
                groups.observe_alternation(offset, start_pos, config)?
            }
