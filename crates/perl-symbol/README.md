...
Run the crate benchmark suite:

```bash
cargo bench -p perl-symbol
```

The suite includes stable benchmark surfaces for cursor extraction and range
lookup, UTF-16 token-under-cursor, symbol index insertion/query paths, and
AST surface declaration extraction workloads.
