...
/// Regression: adversarial input with hundreds of `no warnings` categories must
/// not grow the `disabled_warning_categories` Vec past the hard cap of 256.
/// Without the cap, each `any()` dedup check is O(n) and the state-clone cost
/// becomes O(n²) for adversarial AST nodes.
#[test]
fn no_warnings_category_tracking_is_bounded() {
    ...
    let map = PragmaTracker::build(&ast);
    let state =
        &map.last().expect("expected non-empty pragma map after building warning statements").1;
...
/// Regression: after `use warnings` resets a fully-capped disabled list,
/// subsequent `no warnings 'fresh'` should be tracked again from zero.
#[test]
fn use_warnings_resets_fully_capped_disabled_list() {
    let mut statements = Vec::new();
    statements.push(use_node("warnings", &[], 0, 15));
