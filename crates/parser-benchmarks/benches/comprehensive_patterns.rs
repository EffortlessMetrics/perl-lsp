...
"#),
    ...
    ("regex_substitution", r#"
$text =~ s/foo/bar/g;
$text =~ s{old}{new}gi;
$text =~ s|pattern|replacement|x;
"#),
    
    ("complex_dereference", r#"
...
fn bench_parsers(c: &mut Criterion) {
    ...
    for (name, code) in PATTERNS {
        ...
        group.bench_with_input(
            BenchmarkId::new("tree-sitter-c", name),
            code,
            |b, code| {
                b.iter(|| {
...
fn generate_code(lines: usize) -> String {
    ...
    for i in 0..lines {
        match i % 10 {
            ...
            3 => code.push_str(&format!("if ($var{} > {}) {{ $result += $var{}; }}\n", i, i/2, i)),
            ...
            8 => code.push_str(&format!("$result{} = $var{} =~ /pattern{}/ ? 1 : 0;\n", i, i, i%10)),
