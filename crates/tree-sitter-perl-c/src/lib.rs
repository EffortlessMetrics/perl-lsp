...
/// Low-level tree-sitter language entrypoint for consumers that expect a
/// [`tree_sitter_language::LanguageFn`] symbol.
///
/// This mirrors the shape generated by `tree-sitter-cli` grammar crates and
/// helps downstream integrations that load language symbols dynamically.
...
mod tests {
    ...
    #[test]
    fn test_language_fn_compatibility() -> Result<(), Box<dyn std::error::Error>> {
        let mut parser = Parser::new();
        let language: Language = LANGUAGE.into();
        parser.set_language(&language)?;
        let tree = parser.parse("my $x = 1;", None).ok_or("parse returned None")?;
