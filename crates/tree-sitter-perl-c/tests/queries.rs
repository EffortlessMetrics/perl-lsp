...
/// Test that `load_highlights_query()` returns a valid `Query`.
#[test]
fn test_load_highlights_query_returns_valid_query() {
    let result = queries::load_highlights_query();
    assert!(
        result.is_ok(),
        "expected load_highlights_query() to return Ok, got Err: {:?}",
        result.err()
    );
    let query = result.unwrap();
    assert!(
