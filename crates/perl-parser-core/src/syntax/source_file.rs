...
mod tests {
    ...
    fn classifies_filesystem_paths() {
        ...
        assert!(is_perl_source_path(Path::new(
            "/var/www/cgi-bin/upload.CGI"
        )));
    ...
    fn classifies_uri_like_inputs() {
        ...
        assert!(is_perl_source_uri(
            "file:///workspace/app.psgi?version=1#section"
        ));
        assert!(is_perl_source_uri("file:///var/www/cgi-bin/form.cgi"));
        assert!(is_perl_source_uri(
            "file:///var/www/cgi-bin/search.cgi?q=perl#results"
        ));
    ...
    fn binary_content_null_byte_is_detected() {
        ...
        assert!(
            ...
            "null bytes must trigger binary guard"
        );
    }
    ...
    fn binary_content_single_null_byte_triggers_guard() {
        ...
        assert!(
            ...
            "single null byte must trigger binary guard"
        );
    }
    ...
    fn binary_content_clean_perl_is_not_binary() {
        ...
        assert!(
            ...
            "clean Perl source must not be classified as binary"
        );
    }
    ...
    fn binary_content_empty_string_is_not_binary() {
        assert!(
            !is_binary_content(""),
            "empty string must not be classified as binary"
        );
    }
    ...
    fn binary_content_unicode_text_is_not_binary() {
        ...
        assert!(
            !is_binary_content(utf8),
            "UTF-8 text without null bytes must not be binary"
        );
    }
    ...
    fn binary_content_null_byte_at_probe_boundary() {
        ...
        assert!(
            is_binary_content(&text),
            "null byte at probe boundary must trigger binary guard"
        );
    }
    ...
    fn binary_content_elf_header_is_detected() {
        ...
        assert!(
            is_binary_content(elf_like),
            "ELF-like header with null bytes must be binary"
        );
    }
    ...
    fn binary_content_zip_pk_header_is_detected() {
        ...
        assert!(
            is_binary_content(zip_like),
            "ZIP-like header with null bytes must be binary"
        );
    }
}
