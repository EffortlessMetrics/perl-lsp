        ...
        Ok(ast) => {
            ...
            if let NodeKind::Program { statements } = &ast.kind {
                ...
                match &statements[0].kind {
                    ...
                    _ => panic!("Expected Error node for first statement, got: {:?}", statements[0].kind),
                }
                ...
                match &statements[1].kind {
                    ...
                    _ => panic!("Expected ExpressionStatement for second statement, got: {:?}", statements[1].kind),
                }
            } else {
            ...
            if let NodeKind::Program { statements } = &ast.kind {
                ...
                if let NodeKind::Subroutine { body, .. } = &statements[0].kind {
                    if let NodeKind::Block { statements } = &body.kind {
                        assert_eq!(statements.len(), 2, "Block should have 2 statements (1 error, 1 valid)");
                        
                        match &statements[0].kind {
                            NodeKind::Error { message, .. } => println!("Found block error: {}", message),
                            _ => panic!("Expected Error node in block"),
                        }
                        
                        match &statements[1].kind {
                            NodeKind::ExpressionStatement { .. } => println!("Found valid statement in block"),
