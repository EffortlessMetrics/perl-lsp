...
// ===== Builtin names before fat arrow should be autoquoted =====
//
// In Perl, `=>` (fat arrow) autoquotes the bareword to its left as a string.
// This applies to ALL barewords, including names that happen to match Perl
// builtin functions. The parser's builtin dispatch must not consume these
// names when they appear before `=>`.
//
// Common CPAN pattern: Mojo/Moo/Moose `has` with builtin-named attributes:
//   has log => sub { Mojo::Log->new };
//   has abs => (is => 'ro');

// --- Mojo-style `has <builtin> => sub { ... }` ---
...
// --- Mojo-style with complex sub bodies ---
...
// --- Multiple `has` declarations (Mojolicious.pm pattern) ---
...
// --- Moose/Moo before/after/around with builtin names ---
...
// --- Builtins in hash context (should still work) ---
...
// --- Builtins as function calls still work ---
