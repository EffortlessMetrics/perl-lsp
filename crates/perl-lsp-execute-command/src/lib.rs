...
//!
//! ```no_run
//! use perl_lsp_execute_command::{ExecuteCommandProvider, get_supported_commands};
...
//!
//! ```no_run
//! use perl_lsp_execute_command::{ExecuteCommandProvider, command_exists};
//! use serde_json::Value;
//!
...
use perl_lsp_protocol::JsonRpcError;
use perl_parser::perl_critic::{BuiltInAnalyzer, CriticAnalyzer, CriticConfig};
...
///
/// ```no_run
/// use perl_lsp_execute_command::PerlCommand;
/// use serde_json;
///
...
///
/// ```
/// use perl_lsp_execute_command::CommandResult;
///
...
///
/// ```no_run
/// use perl_lsp_execute_command::ExecuteCommandProvider;
/// use serde_json::Value;
///
/// let provider = ExecuteCommandProvider::new();
///
...
impl ExecuteCommandProvider {
    ...
    fn run_builtin_critic(&self, file_path: &Path) -> Result<Value, String> {
        ...
        let (ast, parse_error) = match parser.parse() {
            ...
            Err(error) => {
                ...
                (
                    perl_parser::ast::Node::new(
                        perl_parser::ast::NodeKind::Error {
                            message,
                            expected: vec![],
                            found: None,
                            partial: None,
                        },
                        perl_parser::ast::SourceLocation { start: 0, end: code_text.len() },
                    ),
                    Some(error),
                )
            }
    ...
    fn create_syntax_error_violation(
        ...
        _content: &str,
        file_path: &Path,
    ) -> perl_parser::perl_critic::Violation {
        ...
        perl_parser::perl_critic::Violation {
            ...
            severity: perl_parser::perl_critic::Severity::Brutal, // Critical severity for syntax errors
            range: perl_parser::position::Range {
                start: perl_parser::position::Position { byte: 0, line: line as u32, column: column as u32 },
                end: perl_parser::position::Position { byte: 1, line: line as u32, column: (column + 1) as u32 },
            },
...
///
/// ```no_run
/// use perl_lsp_execute_command::command_exists;
///
/// if command_exists("perlcritic") {
...
///
/// ```
/// use perl_lsp_execute_command::get_supported_commands;
///
/// let commands = get_supported_commands();
...
pub fn get_supported_commands() -> Vec<String> {
    perl_lsp_protocol::capabilities::get_supported_commands()
}
...
///
/// ```no_run
/// use perl_lsp_execute_command::CommandExecutor;
/// use serde_json::Value;
///
