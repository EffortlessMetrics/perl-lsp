...
pub enum Token {
    ...
    Do, Given, When, Default, Continue,
    Last, Next, Redo, Goto, Return,
    My, Our, Local, State,
    Sub, Package, Use, Require, No,
    Try, Catch, Finally, Defer,
    Class, Method, Field, Role, With,
    Print, Say, Die, Warn,
    Eval, Defined, Undef, Ref, Bless,
    Tie, Tied, Untie,
    Delete, Exists,
    Keys, Values, Each,
    Push, Pop, Shift, Unshift,
    Splice, Sort, Grep, Map,
    Split, Join, Reverse,
    And, Or, Xor, Not,
...
impl fmt::Display for Token {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use Token::*;
        match self {
            Number(s) | HexNumber(s) | BinNumber(s) | OctNumber(s) |
            DoubleString(s) | SingleString(s) | Backtick(s) |
            Identifier(s) | ScalarVar(s) | ArrayVar(s) | 
...
impl<'source> PerlLexer<'source> {
    ...
    pub fn next(&mut self) -> Option<(Token, std::ops::Range<usize>)> {
        ...
        let token = match &token {
            ...
            Token::M | Token::S | Token::Tr | Token::Y | Token::Q | 
            Token::Qq | Token::Qw | Token::Qx | Token::Qr => {
                self.parse_quote_like(token.clone(), span.end)
            }
            _ => token,
        };
    ...
    fn update_mode(&mut self, token: &Token) {
        ...
        self.mode = match token {
            ...
            Comma | Semicolon | Arrow | FatComma |
            Bang | BitNot | UnaryPlus | UnaryMinus |
            Assign | PlusAssign | MinusAssign | StarAssign | SlashAssign |
            ModAssign | PowAssign | DotAssign | RepeatAssign |
            BitAndAssign | BitOrAssign | BitXorAssign |
            LeftShiftAssign | RightShiftAssign |
            AndAndAssign | OrOrAssign | DefinedOrAssign |
            Lt | Gt | Le | Ge | NumEq | NumNe | Cmp |
            StrLt | StrGt | StrLe | StrGe | StrEq | StrNe | StrCmp |
            AndAnd | OrOr | DefinedOr | And | Or | Xor | Not |
            SmartMatch | BindMatch | NotMatch |
            BitAnd | BitOr | BitXor | LeftShift | RightShift |
            Question | Colon | Newline |
            If | Elsif | Unless | While | Until | For | Foreach |
            Return | Print | Say | Die | Warn => LexerMode::ExpectTerm,
            
            // After these, we expect an operator
            RightParen | RightBracket | RightBrace |
            Identifier(_) | Number(_) | HexNumber(_) | BinNumber(_) | OctNumber(_) |
            DoubleString(_) | SingleString(_) | Backtick(_) |
