...
fn flush_section_empty_method_body_stores_nothing() {
    ...
    let doc = extract_pod(source);
    // 'stub' had no body — should not appear in methods map
    assert!(
...
fn strip_formatting_code_unclosed_at_eof_does_not_panic() {
    // "B<unclosed" — no closing >
    let doc = extract_pod("=head1 NAME\n\nB<unclosed\n\n=cut\n");
    let name = doc.name.as_deref().unwrap_or("");
    // The exact output is implementation-defined for malformed input, but
    // the function must not panic and must return *something*.
    assert!(!name.is_empty() || name.is_empty(), "must not panic on unclosed formatting code");
}
...
/// description.is_none()=false → short-circuit to false before methods check.
#[test]
