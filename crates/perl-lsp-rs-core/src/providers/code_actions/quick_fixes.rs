...
/// Fix `next LABEL`, `last LABEL`, or `redo LABEL` with an undefined label (PL410).
///
/// Drops the label token so the statement targets the innermost enclosing loop.
/// The diagnostic range covers the whole loop-control node (`next LABEL`).
/// Returns no actions when the range is invalid or the operator is unrecognised.
pub fn fix_loop_control_undefined_label(
    ...
    vec![CodeAction {
        title: format!("Drop undefined label from '{op}'"),
        kind: CodeActionKind::QuickFix,
