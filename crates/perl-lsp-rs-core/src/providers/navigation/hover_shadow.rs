...
fn classify_hover_result(
    ...
    match entity_occ {
        Some((ref entity, ref occurrence)) => {
            ...
            let context = matching_visible.first().and_then(|v| v.context.as_ref());
            let explanation = build_exact_explanation(entity, occurrence, context, symbol);
...
fn build_exact_explanation(
    ...
    if let Some(ctx) = context {
        if let Some(ref module) = ctx.source_module {
            parts.push(format!("Imported from `{module}`"));
        }
    }
...
fn build_visible_symbol_explanation(vis: &VisibleSymbol, symbol: &str) -> HoverExplanation {
    ...
    if let Some(ref ctx) = vis.context {
        if let Some(ref module) = ctx.source_module {
            parts.push(format!("Imported from `{module}`"));
        }
    }
