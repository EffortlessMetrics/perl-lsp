...
impl CompletionProvider {
    ...
    /// Create a completion provider with explicit external module search roots.
    ///
    /// `include_paths` are project/workspace roots (including PERL5LIB-derived roots),
    /// while `system_inc_paths` are optional system `@INC` roots used only when
