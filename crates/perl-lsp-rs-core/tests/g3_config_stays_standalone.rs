...
fn g3_config_stays_published_not_absorbed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_lib = "crates/perl-lsp-config/src/lib.rs";
    assert!(
        Path::new(config_lib).exists(),
    ...
    let rs_core_config_module = "crates/perl-lsp-rs-core/src/config.rs";
    let rs_core_config_mod_dir = "crates/perl-lsp-rs-core/src/config/";
    assert!(
        !Path::new(rs_core_config_module).exists() && !Path::new(rs_core_config_mod_dir).exists(),
    ...
    let config_toml = "crates/perl-lsp-config/Cargo.toml";
...
fn g3_config_follows_wave_h_deferred() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(Path::new(adr_path).exists(), "ADR 0041 should exist and document config deferral");
