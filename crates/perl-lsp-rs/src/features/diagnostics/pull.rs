...
fn code_description_for_code_str(code: &str) -> Option<CodeDescription> {
    let href = DiagnosticCode::parse_code(code).and_then(|parsed| parsed.documentation_url())?;
    Uri::from_str(href).ok().map(|uri| CodeDescription { href: uri })
}
