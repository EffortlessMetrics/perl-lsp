...
impl LspServer {
    ...
    fn semantic_tokens_class_specific_expansion_receipts(
        ...
        vec![json!({
            ...
            "no_live_behavior_change": true,
            "no_live_token_output_change": true,
            "claim_boundary": "class-specific compiler method-declaration receipt only; token:function remains the only compiler-backed live slice, and method declarations stay shadowed until explicit class-specific approval lands",
            "shadow_receipt": shadow.receipt
        })]
    }
