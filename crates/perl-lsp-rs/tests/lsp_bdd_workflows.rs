...
"#;
    ...
    // The server may still be indexing; accept empty but verify structure if non-empty
...
"#;
    ...
    // Cross-file indexing may be incomplete; the important thing is that the request
    // does not error and returns a valid array shape.
    let incoming_arr = incoming.as_array().ok_or("expected array from incomingCalls")?;
    // If indexing completed, the caller from app.pl should appear
    let caller_names: Vec<&str> = incoming_arr
        .iter()
        .filter_map(|c| c.pointer("/from/name").and_then(|v| v.as_str()))
        .collect();
    if !caller_names.is_empty() {
        // At least one caller should have a non-empty name
        assert!(
...
"#;
    ...
    // Find the first call site: "my $x = helper()"
    let (line, character) = find_position(code, "my $x = helper");
    let call_site_char = character + 8; // offset to "helper" within "my $x = helper"
    ...
    let main_item = main_items.first().ok_or("expected at least one CallHierarchyItem for main")?;
