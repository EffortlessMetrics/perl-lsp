...
impl<'a> ParserV2<'a> {
    ...
    fn parse_statement(&mut self) -> Result<Node, ParseError> {
        ...
        if self.check_keyword("my") || self.check_keyword("our") || 
           self.check_keyword("local") || self.check_keyword("state") {
    ...
    fn parse_relational(&mut self) -> Result<Node, ParseError> {
        ...
        while self.match_any(&[
            ...
            TokenType::Spaceship,
            TokenType::StringLt, TokenType::StringGt,
            TokenType::StringLe, TokenType::StringGe,
            TokenType::StringEq, TokenType::StringNe,
            TokenType::StringCmp,
        ]) {
            let op = self.previous().token_type.clone();
            self.skip_whitespace();
    ...
    fn parse_unary(&mut self) -> Result<Node, ParseError> {
        ...
        if self.match_any(&[
            ...
            TokenType::Not, TokenType::BitwiseNot,
            TokenType::Backslash,
        ]) {
            let start = self.previous().start;
            let op = self.previous().token_type.clone();
    ...
    fn parse_postfix(&mut self) -> Result<Node, ParseError> {
        ...
        loop {
            ...
            } else if self.check(&TokenType::LeftBrace) && matches!(expr.kind, NodeKind::Variable { .. }) {
                // Direct hash access
                self.advance();
                let key = self.parse_expression()?;
                self.consume(&TokenType::RightBrace)?;
