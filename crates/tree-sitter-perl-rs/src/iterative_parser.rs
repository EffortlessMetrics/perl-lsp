...
use std::sync::Arc;
use crate::pure_rust_parser::{AstNode, Rule, PureRustPerlParser};
...
impl PureRustPerlParser {
    ...
    pub fn build_node_iterative(&mut self, initial_pair: Pair<Rule>) -> Result<Option<AstNode>, Box<dyn std::error::Error>> {
    ...
    fn process_state<'a>(&mut self, state: BuildState<'a>) -> Result<ProcessResult<'a>, Box<dyn std::error::Error>> {
        match state {
            BuildState::Process(pair) => {
                ...
                match rule {
                    Rule::scalar_variable => {
                        return Ok(ProcessResult::Complete(Some(AstNode::ScalarVariable(Arc::from(pair_str)))));
                    }
                    Rule::array_variable => {
                        return Ok(ProcessResult::Complete(Some(AstNode::ArrayVariable(Arc::from(pair_str)))));
                    }
                    Rule::hash_variable => {
            ...
            BuildState::BuildFromChildren { rule, children, original_str } => {
                // Build node from processed children
                Ok(ProcessResult::Complete(self.build_node_from_children(rule, children, original_str)?))
            }
        }
    }
    ...
    fn build_node_from_children(
        ...
        match rule {
            ...
            Rule::block => {
                Ok(Some(AstNode::Block(children)))
            }
