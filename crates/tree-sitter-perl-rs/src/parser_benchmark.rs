...
use pest::Parser;
use crate::pure_rust_parser::{PerlParser, PureRustPerlParser, Rule, AstNode};
...
impl BenchmarkSuite {
    ...
    pub fn summary(&self) {
        ...
        for (name, results) in &self.results {
            ...
            println!("{:30} | Avg: {:>8.2?} | Success: {:>5.1}%", 
    ...
    pub fn compare(&self, baseline: &str, comparison: &str) {
        if let (Some(baseline_results), Some(comparison_results)) = 
            ...
            let baseline_avg: Duration = baseline_results.iter()
                .map(|r| r.duration)
                .sum::<Duration>() / baseline_results.len() as u32;
                
            let comparison_avg: Duration = comparison_results.iter()
                .map(|r| r.duration)
                .sum::<Duration>() / comparison_results.len() as u32;
...
impl ParserBenchmark {
    ...
    pub fn bench_parser(&mut self, impl_type: ParserImpl, input: &str) -> Result<AstNode, String> {
        ...
        match impl_type {
            ...
            ParserImpl::Iterative => {
                self.parser.build_node_iterative(pair)
                    .map_err(|e| format!("Build error: {:?}", e))?
                    .ok_or_else(|| "No AST node produced".to_string())
            }
        }
    ...
    fn test_deep_nesting_benchmark() {
        ...
        for depth in [100, 500, 1000] {
            ...
            suite.bench(&format!("{} - Iterative", test_name), 1, || {
                let mut bench = ParserBenchmark::new();
                bench.bench_parser(ParserImpl::Iterative, &expr)
                    .map(|_| ())
                    .map_err(|e| e.to_string())
            });
            ...
            suite.bench(&format!("{} - Stacker", test_name), 1, || {
                let mut bench = ParserBenchmark::new();
                bench.bench_parser(ParserImpl::RecursiveWithStacker, &expr)
                    .map(|_| ())
                    .map_err(|e| e.to_string())
            });
        }
