...
static ENCODING_PRAGMA: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?m)^\s*use\s+encoding\s+['"]([\w\-]+)['"]"#).unwrap()
});

static UTF8_PRAGMA: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)^\s*(use|no)\s+utf8\s*;").unwrap()
});

static LOCALE_PRAGMA: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)^\s*use\s+locale\s*;").unwrap()
});
...
impl EncodingAwareLexer {
    ...
    fn handle_encoding_pragma(&mut self, encoding_name: &str, line: usize) {
        let encoding = match encoding_name.to_lowercase().as_str() {
            ...
            _ => {
                self.warnings.push(EncodingWarning {
                    line,
                    message: format!("Unsupported encoding '{}', defaulting to UTF-8", encoding_name),
        ...
        self.warnings.push(EncodingWarning {
            line,
            message: format!("Encoding changed to {} - heredoc delimiters may be affected", 
                encoding_name),
    ...
    pub fn generate_diagnostics(&self) -> Vec<EncodingDiagnostic> {
        ...
        if self.context.encoding_stack.len() > 1 {
            diagnostics.push(EncodingDiagnostic {
                message: format!("Multiple encoding changes detected ({} total)", 
