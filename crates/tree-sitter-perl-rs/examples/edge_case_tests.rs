...
"#),
        ...
        ("Multiple heredocs on one line", r#"
print <<EOF, <<'END';
First heredoc
EOF
Second heredoc
...
"#),
        ...
        // Regex edge cases
        ("Complex regex with modifiers", r#"
$text =~ s{
    (\w+)       # word
...
"#),
        ...
        ("Regex with unusual delimiters", r#"
$text =~ m!pattern!;
$text =~ s|old|new|g;
$text =~ tr/a-z/A-Z/;
"#),
...
"#),
        ...
        ("Signatures and prototypes", r#"
sub add ($x, $y) { $x + $y }
sub old_style ($$) { $_[0] + $_[1] }
sub optional ($x, $y = 0) { $x + $y }
"#),

        ("Given/when (deprecated but valid)", r#"
use feature 'switch';
given ($value) {
    when (1) { say "one" }
...
"#),
        ...
        // Unicode edge cases
        ("Unicode in various contexts", r#"
my $καλημέρα = "good morning";
sub Σ { sum(@_) }
my %χάρτης = (κλειδί => 'τιμή');
"#),

        ("Mixed Unicode scripts", r#"
my $مرحبا = "hello";
my $你好 = "hello";
my $🐪 = "camel";
"#),
...
"#),
        ...
        ("Missing semicolons", r#"
my $x = 1
my $y = 2
print $x + $y
"#),

        ("Unclosed constructs", r#"
if ($x) {
    print "unclosed"
