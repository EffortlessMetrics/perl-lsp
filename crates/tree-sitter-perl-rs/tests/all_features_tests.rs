...
mod tests {
    ...
    fn test_operator_precedence() {
        ...
        for (code, op) in operators {
            let ast = parser.parse(code).unwrap();
            let sexp = parser.to_sexp(&ast);
            assert!(sexp.contains(op) || sexp.contains("binary_expression"), 
                "Failed to parse operator {} in code: {}", op, code);
        }
    }
    ...
    fn test_typeglob_support() {
        ...
        let slots = vec!["SCALAR", "ARRAY", "HASH", "CODE", "IO", "GLOB", "FORMAT", "NAME", "PACKAGE"];
        for slot in slots {
            let code = format!("*foo{{{}}}", slot);
            let ast = parser.parse(&code).unwrap();
            let sexp = parser.to_sexp(&ast);
            assert!(sexp.contains("typeglob_slot_access"), 
                "Failed to parse typeglob slot access for {}", slot);
        }
    ...
    fn test_quote_like_operators() {
        ...
        for code in qr_tests {
            let ast = parser.parse(code).unwrap();
            let sexp = parser.to_sexp(&ast);
            assert!(sexp.contains("qr_regex") || sexp.contains("regex"), 
...
"#;
    ...
    fn test_special_blocks() {
        ...
        for (code, expected) in blocks {
            let ast = parser.parse(code).unwrap();
            let sexp = parser.to_sexp(&ast);
            assert!(sexp.contains(expected), 
                "Failed to parse {} - expected to find {}", code, expected);
        }
    }
