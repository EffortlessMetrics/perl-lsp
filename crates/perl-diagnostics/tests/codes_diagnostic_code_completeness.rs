...
fn scope_variable_shadowing_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL104",
        "VariableShadowing should have stable code PL104"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL104"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
}
...
fn scope_variable_redeclaration_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL105",
        "VariableRedeclaration should have stable code PL105"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL105"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn scope_duplicate_parameter_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL106",
        "DuplicateParameter should have stable code PL106"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL106"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn scope_parameter_shadows_global_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL107",
        "ParameterShadowsGlobal should have stable code PL107"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL107"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn scope_unused_parameter_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL108",
        "UnusedParameter should have stable code PL108"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL108"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn scope_unquoted_bareword_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL109",
        "UnquotedBareword should have stable code PL109"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL109"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn scope_uninitialized_variable_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL110",
        "UninitializedVariable should have stable code PL110"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL110"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn misspelled_pragma_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL111",
        "MisspelledPragma should have stable code PL111"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL111"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn assignment_in_condition_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL403",
        "AssignmentInCondition should have stable code PL403"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL403"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn numeric_comparison_with_undef_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        DiagnosticCode::parse_code("PL404"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn duplicate_hash_key_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL408",
        "DuplicateHashKey should have code PL408"
    );
    assert_eq!(
        code.severity(),
    ...
    assert!(
        ...
        "DuplicateHashKey should have a context hint"
    );
    assert_eq!(
...
fn deprecated_defined_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL500",
        "DeprecatedDefined should have stable code PL500"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL500"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn deprecated_array_base_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL501",
        "DeprecatedArrayBase should have stable code PL501"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL501"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn phase_scoped_strict_pragma_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL502",
        "PhaseScopedStrictPragma should have code PL502"
    );
    assert_eq!(
        code.severity(),
        perl_diagnostics::codes::DiagnosticSeverity::Warning,
    ...
    assert!(
        code.context_hint().is_some(),
        "PhaseScopedStrictPragma should have a context hint"
    );
    assert_eq!(
...
fn phase_scoped_warnings_pragma_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL503",
        "PhaseScopedWarningsPragma should have code PL503"
    );
    assert_eq!(
        code.severity(),
        perl_diagnostics::codes::DiagnosticSeverity::Warning,
    ...
    assert!(
        code.context_hint().is_some(),
        "PhaseScopedWarningsPragma should have a context hint"
    );
    assert_eq!(
...
fn security_string_eval_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL600",
        "SecurityStringEval should have stable code PL600"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL600"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn security_backtick_exec_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL601",
        "SecurityBacktickExec should have stable code PL601"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL601"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn security_signal_handler_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL602",
        "SecuritySignalHandler should have code PL602"
    );
    assert_eq!(
        code.severity(),
        perl_diagnostics::codes::DiagnosticSeverity::Warning,
    ...
    assert!(
        code.context_hint().is_some(),
        "SecuritySignalHandler should have a context hint"
    );
    assert_eq!(
...
fn unused_import_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL700",
        "UnusedImport should have stable code PL700"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL700"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn module_not_found_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL701",
        "ModuleNotFound should have code PL701"
    );
    assert_eq!(
        code.severity(),
        perl_diagnostics::codes::DiagnosticSeverity::Warning,
    ...
    assert!(
        code.context_hint().is_some(),
        "ModuleNotFound should have a context hint"
    );
    assert_eq!(
...
fn heredoc_in_format_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL800",
        "HeredocInFormat should have stable code PL800"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL800"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_in_begin_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL801",
        "HeredocInBegin should have stable code PL801"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL801"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_dynamic_delimiter_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL802",
        "HeredocDynamicDelimiter should have stable code PL802"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL802"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_in_source_filter_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL803",
        "HeredocInSourceFilter should have stable code PL803"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL803"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_in_regex_code_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL804",
        "HeredocInRegexCode should have stable code PL804"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL804"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_in_eval_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL805",
        "HeredocInEval should have stable code PL805"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL805"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
...
fn heredoc_tied_handle_code_exists() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        code.as_str(),
        "PL806",
        "HeredocTiedHandle should have stable code PL806"
    );
    assert_eq!(
        DiagnosticCode::parse_code("PL806"),
        Some(code),
        "parse_code round-trip failed"
    );
    Ok(())
