...
- **Perltidy integration**: compatibility re-exports of `perl-lsp-perltidy` for code formatting with caching, range formatting, and a `BuiltInFormatter` fallback
...
Tier 2 infrastructure crate in the `tree-sitter-perl-rs` workspace. Used by formatting, diagnostics, and code analysis layers of the Perl LSP server. The crate now composes the dedicated `perl-lsp-perltidy` microcrate instead of owning the formatter implementation directly.
