...
fn advertised_features_clone_produces_equal_values() -> Result<(), Box<dyn std::error::Error>> {
    let af = AdvertisedFeatures { completion: true, hover: true, ..Default::default() };
    let cloned = af.clone();
...
fn build_flags_partial_eq_and_eq() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let c = BuildFlags { completion: true, ..Default::default() };
    assert_ne!(a, c);
    Ok(())
}
...
fn all_is_superset_of_production() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for id in &prod_ids {
        assert!(all_ids.contains(id), "all() should contain production id: {id}");
    }
    Ok(())
}
...
fn all_is_superset_of_ga_lock() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for id in &ga_ids {
        assert!(all_ids.contains(id), "all() should contain ga_lock id: {id}");
    }
    Ok(())
}
...
fn feature_ids_have_no_duplicates_for_all_profiles() -> Result<(), Box<dyn std::error::Error>> {
    for (label, flags) in [
        ...
        deduped.dedup();
        assert_eq!(ids, deduped, "feature ids for {label} must have no duplicates");
    }
    Ok(())
}
...
fn single_flag_type_definition_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { type_definition: true, ..Default::default() };
...
fn single_flag_implementation_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { implementation: true, ..Default::default() };
...
fn single_flag_references_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { references: true, ..Default::default() };
...
fn single_flag_document_symbol_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { document_symbol: true, ..Default::default() };
...
fn single_flag_workspace_symbol_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { workspace_symbol: true, ..Default::default() };
...
fn single_flag_execute_command_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { execute_command: true, ..Default::default() };
...
fn single_flag_call_hierarchy_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { call_hierarchy: true, ..Default::default() };
...
fn single_flag_type_hierarchy_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { type_hierarchy: true, ..Default::default() };
...
fn single_flag_linked_editing_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { linked_editing: true, ..Default::default() };
...
fn single_flag_inline_values_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { inline_values: true, ..Default::default() };
...
-> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { notebook_cell_execution: true, ..Default::default() };
...
fn single_flag_moniker_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { moniker: true, ..Default::default() };
...
fn single_flag_formatting_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { formatting: true, ..Default::default() };
...
fn single_flag_folding_range_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { folding_range: true, ..Default::default() };
...
fn single_flag_document_highlight_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { document_highlight: true, ..Default::default() };
...
fn single_flag_declaration_produces_correct_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { declaration: true, ..Default::default() };
...
fn workspace_symbol_resolve_flag_has_no_feature_id() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { workspace_symbol_resolve: true, ..Default::default() };
...
fn to_advertised_maps_each_direct_field() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let direct_mappings: Vec<(BuildFlags, Box<dyn Fn(&AdvertisedFeatures) -> bool>)> = vec![
        (BuildFlags { completion: true, ..Default::default() }, Box::new(|af| af.completion)),
        (BuildFlags { hover: true, ..Default::default() }, Box::new(|af| af.hover)),
        (BuildFlags { definition: true, ..Default::default() }, Box::new(|af| af.definition)),
        (BuildFlags { references: true, ..Default::default() }, Box::new(|af| af.references)),
        (
            BuildFlags { document_symbol: true, ..Default::default() },
            Box::new(|af| af.document_symbol),
        ),
        (
            BuildFlags { workspace_symbol: true, ..Default::default() },
            Box::new(|af| af.workspace_symbol),
        ),
        (BuildFlags { code_lens: true, ..Default::default() }, Box::new(|af| af.code_lens)),
        (BuildFlags { formatting: true, ..Default::default() }, Box::new(|af| af.formatting)),
        (
            BuildFlags { range_formatting: true, ..Default::default() },
            Box::new(|af| af.range_formatting),
        ),
        (BuildFlags { rename: true, ..Default::default() }, Box::new(|af| af.rename)),
        (BuildFlags { folding_range: true, ..Default::default() }, Box::new(|af| af.folding_range)),
        (
            BuildFlags { linked_editing: true, ..Default::default() },
            Box::new(|af| af.linked_editing),
        ),
        (BuildFlags { inlay_hints: true, ..Default::default() }, Box::new(|af| af.inlay_hints)),
        (
            BuildFlags { semantic_tokens: true, ..Default::default() },
            Box::new(|af| af.semantic_tokens),
        ),
        (
            BuildFlags { call_hierarchy: true, ..Default::default() },
            Box::new(|af| af.call_hierarchy),
        ),
        (
            BuildFlags { type_hierarchy: true, ..Default::default() },
            Box::new(|af| af.type_hierarchy),
        ),
        (
            BuildFlags { document_color: true, ..Default::default() },
            Box::new(|af| af.document_color),
        ),
        (
            BuildFlags { notebook_document_sync: true, ..Default::default() },
            Box::new(|af| af.notebook_document_sync),
        ),
        (
            BuildFlags { notebook_cell_execution: true, ..Default::default() },
            Box::new(|af| af.notebook_cell_execution),
        ),
        (
            BuildFlags { signature_help: true, ..Default::default() },
            Box::new(|af| af.signature_help),
        ),
        (
            BuildFlags { document_highlight: true, ..Default::default() },
            Box::new(|af| af.document_highlight),
        ),
        (BuildFlags { declaration: true, ..Default::default() }, Box::new(|af| af.declaration)),
    ];

    for (i, (flags, check)) in direct_mappings.iter().enumerate() {
        let af = flags.to_advertised_features();
        assert!(check(&af), "direct mapping {i} should be true in advertised features");
    }
    Ok(())
}
...
fn two_flags_produce_two_sorted_ids() -> Result<(), Box<dyn std::error::Error>> {
    let flags = BuildFlags { hover: true, completion: true, ..Default::default() };
    let ids = flags.to_feature_ids();
    assert_eq!(ids.len(), 2);
    assert!(ids.contains(&LSP_COMPLETION));
    assert!(ids.contains(&LSP_HOVER));
...
fn feature_id_strings_use_lsp_dot_prefix() -> Result<(), Box<dyn std::error::Error>> {
    let ids = BuildFlags::all().to_feature_ids();
    for id in &ids {
        assert!(id.starts_with("lsp."), "feature id {id} should start with 'lsp.'");
    }
    Ok(())
}
