...
mod tests {
    ...
    fn test_symlink_escape_outside_workspace() -> TestResult {
        ...
        #[cfg(unix)]
        std::os::unix::fs::symlink(external_dir.path(), &link_path)?;
    ...
    fn test_symlink_within_workspace_is_allowed() -> TestResult {
        ...
        let link_path = workspace.join("lib_link");
        #[cfg(unix)]
    ...
    fn test_chained_symlinks_escaping_workspace() -> TestResult {
        ...
        let hop1 = workspace.join("hop1");
        #[cfg(unix)]
        std::os::unix::fs::symlink(external_dir.path(), &hop1)?;
