...
`perl-lsp-ast-utils` is a **Tier 1 LSP leaf crate** that provides shared AST range-lookup, insertion-position, and indentation helpers used by other LSP feature crates.
...
**Version**: 0.10.0 (workspace-inherited)

## Commands

```bash
cargo build -p perl-lsp-ast-utils          # Build this crate
cargo test -p perl-lsp-ast-utils           # Run tests
cargo clippy -p perl-lsp-ast-utils         # Lint
cargo doc -p perl-lsp-ast-utils --open     # View documentation
```
...
- `perl-parser-core` — AST types (`Node`, `NodeKind`, `SourceLocation`)
