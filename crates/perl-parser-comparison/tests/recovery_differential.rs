...
//! misparse the input*: the string boundary shifts, the missing semicolon is
...
/// - v2 (Pest): PartialRecovery — recovers `suffix` but `@@@` consumes sub sub
...
fn zzz_recovery_summary_header() {
    ...
    println!(
        "  v2 = perl-parser-pest    (Pest/PEG legacy)  — often \"recovers\" by silent mismismatch"
    );
