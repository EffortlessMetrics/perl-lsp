...
impl SemanticTokensProvider {
    ...
    fn visit_node(&mut self, node: &Node, tokens: &mut Vec<SemanticToken>, is_declaration_context: bool) {
        match &node.kind {
            NodeKind::Package { name, block } => {
                // Package name is a namespace
                self.add_token_from_string(name, SemanticTokenType::Namespace, vec![SemanticTokenModifier::Declaration], tokens, node);
            ...
            NodeKind::Subroutine { name, params, body, .. } => {
                ...
                if let Some(name_str) = name {
                    ...
                    self.add_token_from_string(name_str, SemanticTokenType::Function, modifiers, tokens, node);
                }
            ...
            NodeKind::VariableDeclaration { variable, .. } => {
                ...
                // Mark as declaration
                self.add_token(variable, SemanticTokenType::Variable, vec![SemanticTokenModifier::Declaration], tokens);
            }
            ...
            NodeKind::MethodCall { object, method, args } => {
                ...
                // Method name
                self.add_token_from_string(method, SemanticTokenType::Method, vec![SemanticTokenModifier::Reference], tokens, node);
            ...
            NodeKind::FunctionCall { name, args } => {
                ...
                self.add_token_from_string(name, SemanticTokenType::Function, modifiers, tokens, node);
            ...
            NodeKind::Use { module, .. } => {
                ...
                self.add_token_from_string(module, SemanticTokenType::Namespace, vec![SemanticTokenModifier::Reference], tokens, node);
            }
    ...
    fn add_token_from_string(&self, name: &str, token_type: SemanticTokenType, modifiers: Vec<SemanticTokenModifier>, tokens: &mut Vec<SemanticToken>, parent_node: &Node) {
    ...
    fn is_builtin_function(&self, name: &str) -> bool {
        ...
        matches!(name, "print" | "say" | "open" | "close" | "read" | "write" | 
                 "push" | "pop" | "shift" | "unshift" | "grep" | "map" | 
                 "sort" | "reverse" | "join" | "split" | "substr" | "length" |
                 "chomp" | "chop" | "lc" | "uc" | "index" | "rindex" |
                 "die" | "warn" | "eval" | "require" | "use" | "package")
    }

    /// Add a semantic token
    fn add_token(&self, node: &Node, token_type: SemanticTokenType, modifiers: Vec<SemanticTokenModifier>, tokens: &mut Vec<SemanticToken>) {
    ...
    fn visit_children(&mut self, node: &Node, tokens: &mut Vec<SemanticToken>, is_declaration_context: bool) {
        match &node.kind {
            NodeKind::Block { statements } => {
                for stmt in statements {
                    self.visit_node(stmt, tokens, false);
