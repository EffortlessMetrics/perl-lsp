...
impl Scope {
    ...
    fn use_variable(&mut self, name: &str) -> bool {
        ...
        // Try to mark as used in parent scope
    ...
    fn mark_used_in_parent(&mut self, name: &str) {
        if !self.variables.contains_key(name) {
            if let Some(ref mut parent) = self.parent {
                parent.mark_used_in_parent(name);
            }
        } else if let Some((line, _)) = self.variables.get(name).cloned() {
            self.variables.insert(name.to_string(), (line, true));
        }
    }
    ...
    fn analyze_node(&self, node: &Node, scope: &mut Scope, issues: &mut Vec<ScopeIssue>, code: &str, strict_mode: bool) {
        match &node.kind {
            ...
            NodeKind::Block { statements } => {
                ...
                // Don't report unused variables for block-local vars yet
                // They'll be reported at the end of the analysis
            }
