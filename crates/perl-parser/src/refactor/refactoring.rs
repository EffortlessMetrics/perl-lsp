...
pub struct RefactoringEngine {
    ...
    #[cfg(feature = "modernize")]
    modernize: super::modernize::PerlModernizer,
...
impl RefactoringEngine {
    ...
    pub fn index_file(&mut self, path: &Path, content: &str) -> ParseResult<()> {
        #[cfg(feature = "workspace_refactor")]
        {
            let uri_str = perl_workspace::workspace_index::fs_path_to_uri(path).map_err(|e| {
                ParseError::SyntaxError {
    ...
    fn filter_rename_result_by_scope(
        &self,
        mut result: super::workspace_refactor::RefactorResult,
        scope: &RefactoringScope,
    ) -> ParseResult<super::workspace_refactor::RefactorResult> {
        match scope {
            RefactoringScope::Workspace
            | RefactoringScope::Directory(_)
    ...
    fn filter_file_edits_to_range(
        file_edits: Vec<super::workspace_refactor::FileEdit>,
        target_file: &Path,
        start_off: usize,
        end_off: usize,
    ) -> Vec<super::workspace_refactor::FileEdit> {
        file_edits
            .into_iter()
            ...
            #[cfg(feature = "workspace_refactor")]
            {
                if all_occurrences {
                    ...
                    match self.workspace_refactor.inline_variable_all(symbol_name, def_file, (0, 0))
                    {
                        ...
                        Err(super::workspace_refactor::RefactorError::SymbolNotFound {
                            ..
                        }) => {
                            warnings.push(format!(
    ...
    fn apply_file_edits(
        &self,
        file_edits: &[super::workspace_refactor::FileEdit],
    ) -> ParseResult<usize> {
        let mut files_modified = 0;
...
"#;
    ...
    mod validation_tests {
        ...
        fn test_cleanup_respects_retention_count() {
            ...
            // Should have removed 2 oldest directories, keeping the 2 newest.
