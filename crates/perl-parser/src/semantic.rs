...
use crate::symbol::{Symbol, SymbolExtractor, SymbolKind, SymbolTable, ScopeId};
            ...
            NodeKind::Variable { sigil, name } => {
                ...
                if let Some(symbol) = symbols.first() {
                    let hover = HoverInfo {
                        ...
                        details: vec![
                            format!("Defined at line {}", self.line_number(symbol.location.start)),
                        ],
                    };
            ...
            NodeKind::FunctionCall { name, args } => {
                // Check if this is a built-in function
                {
                    let token_type = if is_builtin_function(name) {
                        ...
                        let symbols = self.symbol_table.find_symbol(name, scope_id, SymbolKind::Subroutine);
                        if symbols.is_empty() {
                            SemanticTokenType::Function
                        } else {
                            SemanticTokenType::Function
