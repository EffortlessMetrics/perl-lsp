...
fn test_duplicate_parameter_code_actions() {
    ...
    let duplicate_actions: Vec<_> = actions.iter()
...
sub process($data) {
    ...
    let shadow_actions: Vec<_> = actions.iter()
    ...
    assert!(shadow_actions.iter().any(|a| a.title.contains("$p_data") || a.title.contains("$data_param")));
}
...
fn test_unused_parameter_code_actions() {
    ...
    let unused_actions: Vec<_> = actions.iter()
...
print FOO;"#;
    ...
    let bareword_actions: Vec<_> = actions.iter()
...
print LOGFILE "Starting process";"#;
    ...
    let filehandle_actions: Vec<_> = actions.iter()
