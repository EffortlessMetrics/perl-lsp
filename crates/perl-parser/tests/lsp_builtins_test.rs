...
fn get_signature_help(server: &mut LspServer, uri: &str, line: u32, character: u32) -> Option<Value> {
    let request = JsonRpcRequest {
        _jsonrpc: "2.0".to_string(),
        id: Some(json!(1)),
...
fn test_file_operation_signatures() {
    ...
    assert!(sig["signatures"][0]["label"].as_str().unwrap().contains("FILEHANDLE, POSITION, WHENCE"));
...
fn test_math_signatures() {
    ...
    for (i, (code, func_name, expected_params, cursor_pos)) in math_functions.iter().enumerate() {
        ...
        assert!(label.contains(expected_params), "Label doesn't contain params for {}: {}", func_name, label);
    }
}
...
fn test_all_114_builtins_are_recognized() {
    ...
    let all_builtins = vec![
        // Original 40
        "print", "printf", "open", "close", "read", "write", "die", "warn",
        "substr", "length", "index", "rindex", "sprintf", "join", "split",
        "push", "pop", "shift", "unshift", "splice", "grep", "map", "sort",
        "reverse", "keys", "values", "each", "exists", "delete", "defined",
        "undef", "ref", "bless", "chomp", "chop", "chr", "ord", "lc", "uc",
        "lcfirst", "ucfirst",
        
        // File operations (17)
        "seek", "tell", "stat", "lstat", "chmod", "chown", "unlink", "rename",
        "mkdir", "rmdir", "opendir", "readdir", "closedir", "link", "symlink",
        "readlink", "truncate",
        
        // String/Data (7)
        "pack", "unpack", "quotemeta", "hex", "oct", "vec", "crypt",
        
        // Array/List (2)
        "scalar", "wantarray",
        
        // Math (12)
        "abs", "int", "sqrt", "exp", "log", "sin", "cos", "tan", "atan2",
        "rand", "srand",
        
        // System/Process (14)
        "system", "exec", "fork", "wait", "waitpid", "kill", "sleep", "alarm",
        "exit", "getpgrp", "setpgrp", "getppid", "getpriority", "setpriority",
        
        // Time (4)
        "time", "localtime", "gmtime", "times",
        
        // User/Group (5)
        "getpwuid", "getpwnam", "getgrgid", "getgrnam", "getlogin",
        ...
        // Control flow (9)
        "eval", "require", "do", "caller", "return", "goto", "last", "next", "redo",
        
        // Misc (10)
        "tie", "untie", "tied", "dbmopen", "dbmclose", "select", "syscall",
        "dump", "prototype", "lock",
    ];
    ...
    for func in all_builtins {
        ...
        let sig = result.unwrap();
        let label = sig["signatures"][0]["label"].as_str().unwrap();
        assert!(label.contains(func), "Signature for {} doesn't contain function name: {}", func, label);
    }
}