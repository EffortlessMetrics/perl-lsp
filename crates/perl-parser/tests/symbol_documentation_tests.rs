...
"#;
    ...
    assert_eq!(do_something_symbols[0].documentation.as_deref(), Some("This method does something\nwith multiple parameters"));
    
    let simple_symbols = table.symbols.get("simple_method").expect("symbol simple_method");
    assert_eq!(simple_symbols[0].documentation.as_deref(), Some("Another method with no parameters"));
}
...
fn unicode_in_comments() {
    ...
    let expected = "Документация на русском языке\nDocumentation with émojis 🚀\nand Unicode symbols ∑∏∆";
    assert_eq!(symbols[0].documentation.as_deref(), Some(expected));
}
...
fn performance_with_large_comment_blocks() {
    ...
    let symbols = table.symbols.get("large_comment_function").expect("symbol large_comment_function");
...
fn performance_benchmark_comment_extraction() {
    ...
    assert!(per_iteration < 100_000, "Comment extraction too slow: {} ns per iteration", per_iteration);
    
    // Print performance info for manual verification  
    if per_iteration > 50_000 {
        println!("Warning: Comment extraction slower than expected: {} ns per iteration", per_iteration);
    }
}
