...
fn test_special_variable_completion() {
    ...
    let items = response["result"].as_array().unwrap();
    assert!(items.len() >= 2, "Should have special variables like $^O and $^V");
...
fn test_completion_ranking() {
    ...
    let first_items: Vec<String> = items.iter()
        .take(5)
        .map(|item| item["label"].as_str().unwrap().to_string())
        .collect();
...
$p"#
    ...
    let response1 = send_request(&mut server, json!({
        "jsonrpc": "2.0",
        "id": 18,
        "method": "textDocument/completion",
        "params": {
...
$pre"#
    ...
    let response2 = send_request(&mut server, json!({
        "jsonrpc": "2.0",
        "id": 19,
        "method": "textDocument/completion",
        "params": {
...
$prefi"#
    ...
    let response3 = send_request(&mut server, json!({
        "jsonrpc": "2.0",
        "id": 20,
        "method": "textDocument/completion",
        "params": {
    ...
    let labels3: Vec<String> = items3.iter()
        .map(|item| item["label"].as_str().unwrap().to_string())
        .collect();
