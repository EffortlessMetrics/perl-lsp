...
impl TestContext {
    fn new() -> Self {
        ...
        ctx.send_request("initialize", Some(json!({
...
fn test_hover_golden() {
    ...
    if let Some(h) = hover {
        ...
        if let Some(text) = content {
            ...
            assert!(text.contains("calculate_sum") || text.contains("sub"), 
...
fn test_folding_ranges_golden() {
    ...
    if let Some(r) = ranges {
        ...
        let sub_folds = ranges_arr.iter().filter(|r| {
...
"#;
    ...
    ctx.send_notification("textDocument/didOpen", Some(json!({
        "textDocument": {
    ...
    for (version, (line, char, text)) in edits.iter().enumerate() {
        ctx.send_notification("textDocument/didChange", Some(json!({
            "textDocument": {
                "uri": "file:///tmp/fuzz_test.pl",
                "version": version + 2
            },
        ...
        let _ = ctx.send_request("textDocument/hover", Some(json!({
