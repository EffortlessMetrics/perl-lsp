...
fn test_var_decl_in_same_block() -> TestResult {
    ...
    let links = provider
        .find_declaration(11, 0)
        .ok_or("Should find declaration")?;
...
"#;
    ...
    let inner_decl_pos = content
        .rfind("my $x = 2")
        .ok_or("Could not find inner decl")?;
    assert!(
...
"#;
    ...
    let links = provider
        .find_declaration(foo_usage, 0)
        .ok_or("Should find FOO declaration")?;
    assert_eq!(links.len(), 1);
    ...
    let _links = provider
        .find_declaration(bar_usage, 0)
        .ok_or("Should find BAR declaration")?;

    // Test QUX (qw form)
    let qux_usage = content.rfind("QUX").ok_or("Could not find QUX usage")?;
    let _links = provider
        .find_declaration(qux_usage, 0)
        .ok_or("Should find QUX declaration")?;
    Ok(())
}
...
fn test_unicode_and_crlf() -> TestResult {
    ...
    let links = provider
        .find_declaration(pi_usage, 0)
        .ok_or("Should find π declaration")?;
    assert_eq!(links.len(), 1);
    ...
    let _links = provider
        .find_declaration(snake_usage, 0)
        .ok_or("Should find snake declaration")?;
...
"#;
    ...
    let priv_call = content
        .rfind("_priv()")
        .ok_or("Could not find _priv call")?;
    let _links = provider
        .find_declaration(priv_call, 0)
        .ok_or("Should find _priv declaration")?;

    // Test métód_π (unicode method name)
    let unicode_call = content
        .rfind("métód_π()")
        .ok_or("Could not find unicode call")?;
    let _links = provider
