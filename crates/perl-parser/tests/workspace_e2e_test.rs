...
fn test_goto_definition_across_files() {
    ...
    use std::io::Cursor;
    let output = Arc::new(Mutex::new(Box::new(Cursor::new(Vec::new())) as Box<dyn std::io::Write + Send>));
...
fn test_find_references_across_files() {
    ...
    fs::write(&script1_file, "use lib 'lib';\nuse Utils;\n\nmy $result = Utils::process_data(21);\nprint $result;\n").unwrap();
    ...
    if let Some(refs) = result {
        ...
        assert!(refs_array.len() >= 3, "Should find at least 3 references, found: {}", refs_array.len());
        
        // Check that references are in different files
        let uris: Vec<String> = refs_array.iter()
...
fn test_workspace_symbol_completion() {
    ...
    if let Some(completions) = result {
        ...
        assert!(labels.iter().any(|l| l.contains("calculate_factorial")), 
                "Should suggest calculate_factorial, got: {:?}", labels);
        assert!(labels.iter().any(|l| l.contains("calculate_fibonacci")), 
                "Should suggest calculate_fibonacci, got: {:?}", labels);
    } else {
