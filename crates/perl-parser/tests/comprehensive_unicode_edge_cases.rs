...
fn test_complex_unicode_scripts() {
    ...
    let test_cases = vec![
        ...
        ("Mixed LTR/RTL", r#"my $english_العربية_variable = 42; print "$english_العربية_variable\n";"#),
    ...
    for (name, code) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                assert!(parse_time < MAX_UNICODE_PARSE_TIME, "Parse time exceeded limit for {}", name);
            ...
            Err(e) => {
                ...
                assert!(parse_time < MAX_UNICODE_PARSE_TIME, "Error detection took too long for {}", name);
            }
        }
    }
}
...
fn test_bidirectional_and_combining_characters() {
    ...
    let test_cases = vec![
        ...
        ("Unicode regex class", r#"my $text = "العربية"; if ($text =~ /\p{Arabic}/) { print "Arabic\n"; }"#),
    ...
    for (name, code) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                let sexp = ast.to_sexp();
                assert!(sexp.contains("string") || sexp.contains("regex") || sexp.contains("match"), 
                       "String/regex not found in AST for {}", name);
            }
            Err(e) => {
                println!("  ✗ Failed to parse: {}", e);
    ...
    for (name, code) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                // Verify the text is preserved
                let sexp = ast.to_sexp();
                assert!(sexp.contains("string") || sexp.contains("variable"), 
                       "String/variable not found in AST for {}", name);
            }
            Err(e) => {
                println!("  ✗ Failed to parse: {}", e);
...
fn test_unicode_in_perl_constructs() {
    ...
    let test_cases = vec![
        ...
        ("Unicode module", r#"package 中文模块; sub test { return "测试"; } print 中文模块::test();"#),
        
        // Unicode in subroutine names
        ("Unicode sub", r#"sub العربية_دالة { return "العربية"; } print العربية_دالة();"#),
        ("Unicode method", r#"package 中文类; sub 中文方法 { return "方法"; } my $obj = 中文类->new(); print $obj->中文方法();"#),
        
        // Unicode in hash keys
        ("Unicode hash keys", r#"my %hash = ('العربية' => 'Arabic', '中文' => 'Chinese'); print $hash{'العربية'};"#),
        ("Unicode mixed keys", r#"my %hash = ('keyالعربية' => 'Mixed', '中文key' => 'Mixed'); print $hash{'keyالعربية'};"#),
        
        // Unicode in array indices (with proper dereferencing)
        ("Unicode array access", r#"my @array = ('a', 'b', 'c'); my $العربية = 1; print $array[$العربية];"#),
        ...
        ("Unicode regex match", r#"my $text = "العربية"; if ($text =~ /العربية/) { print "Match\n"; }"#),
        ("Unicode regex class", r#"my $text = "café"; if ($text =~ /[éèêë]/) { print "Accented\n"; }"#),
        ("Unicode regex quantifier", r#"my $text = "العربية العربية"; if ($text =~ /العربية+/) { print "Repeated\n"; }"#),
        
        // Unicode in heredocs
        ("Unicode heredoc", r#"my $text = <<'END';
العربية
中文
עברית
END
print "$text\n";"#),
        ...
        ("Unicode pack", r#"my $text = "العربية"; my $packed = pack("A*", $text); print "$packed\n";"#),
        ("Unicode unpack", r#"my $text = "العربية"; my @chars = unpack("C*", $text); print "@chars\n";"#),
    ];
...
fn test_unicode_special_variables() {
    ...
    let test_cases = vec![
        ...
        ("Unicode format", r#"format STDOUT =
العربية: @<<<<<
$العربية
.
my $العربية = 42;
    ...
    for (name, code) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                let sexp = ast.to_sexp();
                assert!(sexp.contains("variable") || sexp.contains("special_var") || sexp.contains("format"), 
                       "Variable not found in AST for {}", name);
            }
            Err(e) => {
                println!("  ✗ Failed to parse: {}", e);
...
fn test_unicode_file_operations() {
    ...
    let test_cases = vec![
        ...
        ("Unicode filehandle", r#"open my $العربية_ملف, '<', 'test.txt' or die $!; while (<$العربية_ملف>) { print; } close $العربية_ملف;"#),
        
        // Unicode file paths
        ("Unicode path", r#"open my $fh, '<', 'العربية.txt' or die $!; while (<$fh>) { print; } close $fh;"#),
        ("Mixed path", r#"open my $fh, '<', 'test_العربية.txt' or die $!; while (<$fh>) { print; } close $fh;"#),
        
        // Unicode in directory operations
        ("Unicode opendir", r#"opendir my $العربية_دليل, '.' or die $!; while (readdir $العربية_دليل) { print "$_\n"; } closedir $العربية_دليل;"#),
    ...
    for (name, code) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                let sexp = ast.to_sexp();
                assert!(sexp.contains("file") || sexp.contains("open") || sexp.contains("require") || 
                       sexp.contains("system") || sexp.contains("exec"), 
                       "File operation not found in AST for {}", name);
            }
            Err(e) => {
                println!("  ✗ Failed to parse: {}", e);
...
fn test_unicode_advanced_features() {
    ...
    let test_cases = vec![
        ...
        ("Unicode bless", r#"package العربية_فئة; sub new { bless {} } my $obj = bless {}, 'العربية_فئة';"#),
        ("Unicode isa", r#"use base 'العربية_فئة'; sub test { return "test"; }"#),
        
        // Unicode in prototypes
        ("Unicode prototype", r#"sub العربية_دالة ($) { return $_[0] * 2; } print العربية_دالة(21);"#),
        
        // Unicode in attributes
        ("Unicode attribute", r#"sub العربية_دالة :lvalue { return $العربية_متغير; }"#),
        
        // Unicode in try/catch
        ("Unicode try/catch", r#"use feature 'try'; try { die "العربية error"; } catch ($e) { print "$e\n"; }"#),
        
        // Unicode in signatures
        ("Unicode signature", r#"use feature 'signatures'; sub العربية_دالة ($العربية_معامل) { return $العربية_معامل * 2; } print العربية_دالة(21);"#),
        
        // Unicode in given/when
        ("Unicode given/when", r#"use feature 'switch'; my $العربية_متغير = 42; given ($العربية_متغير) { when (42) { print "Answer\n"; } }"#),
        
        // Unicode in smartmatch
        ("Unicode smartmatch", r#"my $العربية_متغير = 42; if ($العربية_متغير ~~ 42) { print "Match\n"; }"#),
        
        // Unicode in state variables
        ("Unicode state", r#"use feature 'state'; sub العربية_دالة { state $العربية_计数 = 0; return ++$العربية_计数; } print العربية_دالة();"#),
        
        // Unicode in lexical subs
        ("Unicode lexical sub", r#"use feature 'lexical_subs'; my sub العربية_دالة { return 42; } print العربية_دالة();"#),
    ];
