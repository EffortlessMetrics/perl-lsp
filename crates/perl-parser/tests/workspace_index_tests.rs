...
"#;
    ...
    let unused_funcs: Vec<_> = unused.iter()
        .filter(|s| s.name == "unused_func")
        .collect();
    ...
    assert!(member_names.contains(&&"unused_func".to_string()), "Should find 'unused_func' in Foo package");
}
