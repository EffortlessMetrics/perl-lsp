...
fn various_delimiters_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (code, delim_name) in delimiters {
        ...
        match parser.parse() {
            ...
            Err(e) => {
                panic!("Failed to parse m operator with {} delimiter {}: {}", delim_name, code, e);
            }
        }
    }
    Ok(())
...
fn substitution_various_delimiters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (code, delim_name) in delimiters {
        ...
        match parser.parse() {
            ...
            Err(e) => {
                panic!("Failed to parse s operator with {} delimiter {}: {}", delim_name, code, e);
            }
        }
    }
    Ok(())
...
fn qr_operator_various_delimiters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (code, delim_name) in delimiters {
        ...
        match parser.parse() {
            ...
            Err(e) => {
                panic!("Failed to parse qr operator with {} delimiter {}: {}", delim_name, code, e);
            }
        }
    }
    Ok(())
