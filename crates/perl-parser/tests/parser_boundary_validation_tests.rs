...
fn test_recursion_depth_boundary() {
    ...
    match result {
        Ok(_ast) => {
            println!("  ✓ At limit ({}): parsed successfully in {:?}", MAX_RECURSION_DEPTH, parse_time);
        }
        Err(e) => {
...
fn test_heredoc_depth_boundary() {
    ...
    match result {
        Ok(_ast) => {
            println!("  ✓ At limit ({}): parsed successfully in {:?}", MAX_HEREDOC_DEPTH, parse_time);
        }
        Err(e) => {
...
fn test_token_count_boundaries() {
    ...
    for (target_tokens, description) in token_tests {
        ...
        assert!(
            ...
            description, time_per_1k_tokens
        );
    }
}
...
fn test_concurrent_boundary_conditions() {
            ...
            thread::spawn(move || {
                for operation in 0..operations_per_thread {
                    ...
                    let (code, scenario_name) = match scenario {
                        0 => (generate_nested_code(MAX_RECURSION_DEPTH + operation % 10), "recursion"),
    ...
    let mut scenario_stats: std::collections::HashMap<&str, (usize, Duration, usize)> = std::collections::HashMap::new();
    ...
    for (thread_id, operation, scenario_name, success, parse_time) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_secs(10),
            ...
            thread_id, operation, scenario_name, parse_time
        );
    }
    ...
    for (scenario_type, (count, total_time, success_count)) in scenario_stats {
        ...
        println!("    {}: {} ops, avg: {:?}, success: {:.1}%",
                 scenario_type, count, avg_time, success_rate * 100.0);
        ...
        assert!(
            ...
            scenario_type, success_rate * 100.0
        );
    }
}
    ...
    for (case_name, code) in edge_cases {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    !e.to_string().is_empty(),
                    "Error message should not be empty for edge case: {}", case_name
                );
            }
        }
...
fn generate_token_dense_code(target_tokens: usize) -> String {
    ...
    while token_count < target_tokens {
        let expression = format!(
            "my $expr{} = {} + {} * {} / {} - {} % {} && {} || {};\n",
            counter, counter, counter+1, counter+2, counter+3, counter+4, counter+5, counter+6, counter+7
        );
