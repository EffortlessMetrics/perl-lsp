...
"#;
    ...
    let has_sub_range = ranges.iter().any(|r| {
        r["startLine"].as_u64() == Some(1) && 
        r["endLine"].as_u64().is_some()
    });
...
"#;
    ...
    let has_import_range = ranges.iter().any(|r| {
        r.get("kind").and_then(|k| k.as_str()) == Some("imports")
    });
