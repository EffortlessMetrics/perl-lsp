    ...
    for (name, string_content) in test_cases {
        ...
        match result {
            Ok(ast) => {
                ...
                let sexp = ast.to_sexp();
                assert!(sexp.contains("string") || sexp.contains("literal"), 
                       "String not found in AST for {}", name);
            }
            Err(e) => {
                println!("  ✗ Failed to parse: {}", e);
    ...
    for (i, code) in test_cases.iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(parse_time < MAX_PARSE_TIME, "Error detection took too long for case {}", i);
            }
        }
    }
}
