...
fn analyze_strict(code: &str) -> Vec<perl_semantic_analyzer::analysis::scope_analyzer::ScopeIssue> {
    ...
    let pragma_map = vec![(0..code.len(), PragmaState {
        strict_refs: true,
        strict_subs: true,
        strict_vars: true,
        warnings: true
...
fn test_use_before_definition_same_scope() {
    ...
    let undeclared = issues.iter().find(|i| i.kind == IssueKind::UndeclaredVariable && i.variable_name == "$x");
...
fn test_use_before_initialization() {
    ...
    let uninitialized = issues.iter().find(|i| i.kind == IssueKind::UninitializedVariable && i.variable_name == "$x");
...
fn test_undeclared_assignment() {
    ...
    let undeclared = issues.iter().find(|i| i.kind == IssueKind::UndeclaredVariable && i.variable_name == "$undeclared");
...
fn test_list_assignment_initialization() {
    ...
    let uninit_x = issues.iter().find(|i| i.kind == IssueKind::UninitializedVariable && i.variable_name == "$x");
    let uninit_y = issues.iter().find(|i| i.kind == IssueKind::UninitializedVariable && i.variable_name == "$y");
