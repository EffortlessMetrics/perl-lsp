...
"#;
    ...
    assert!(token_type_counts.len() >= 5, "Should have at least 5 different token types, got {}: {:?}", token_type_counts.len(), token_type_counts);
    
    // Verify we have namespace tokens (packages)
    let namespace_type = SemanticTokenType::all().iter().position(|&t| t == SemanticTokenType::Namespace).unwrap() as u32;
    assert!(token_type_counts.contains_key(&namespace_type), "Should contain namespace tokens");
    
    // Verify we have function tokens
    let function_type = SemanticTokenType::all().iter().position(|&t| t == SemanticTokenType::Function).unwrap() as u32;
    assert!(token_type_counts.contains_key(&function_type), "Should contain function tokens");
    
    // Verify we have variable tokens
    let variable_type = SemanticTokenType::all().iter().position(|&t| t == SemanticTokenType::Variable).unwrap() as u32;
...
"#;
    ...
    for (_, _, _, _, modifiers) in &tokens {
        if *modifiers & (1 << SemanticTokenModifier::all().iter().position(|&m| m == SemanticTokenModifier::Declaration).unwrap()) != 0 {
            has_declaration_modifier = true;
        }
        if *modifiers & (1 << SemanticTokenModifier::all().iter().position(|&m| m == SemanticTokenModifier::Reference).unwrap()) != 0 {
            has_reference_modifier = true;
        }
        if *modifiers & (1 << SemanticTokenModifier::all().iter().position(|&m| m == SemanticTokenModifier::Modification).unwrap()) != 0 {
...
"#;
    ...
    let function_type = SemanticTokenType::all().iter().position(|&t| t == SemanticTokenType::Function).unwrap() as u32;
    let default_library_bit = 1 << SemanticTokenModifier::all().iter().position(|&m| m == SemanticTokenModifier::DefaultLibrary).unwrap();
...
fn semantic_tokens_performance_stress_test() {
    ...
    assert!(duration.as_millis() < 5000, "semantic tokens should complete within 5 seconds, took {:?}", duration);
