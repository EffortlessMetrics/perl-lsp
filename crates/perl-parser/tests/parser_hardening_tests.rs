...
fn modifier_canonicalization_is_stable() {
    ...
    for group in regex_tests {
        ...
        for input in &group {
            ...
            let ast = p.parse().expect(&format!("Failed to parse: {}", input));
            asts.push((input, ast));
        }
        ...
        for (input, ast) in &asts[1..] {
            ...
            if first_mods != mods {
                println!("Note: Modifiers are not canonicalized - {} vs {}", 
    ...
    for group in subst_tests {
        ...
        for (input, ast) in &asts[1..] {
            ...
            if first_mods != mods {
                println!("Note: Substitution modifiers are not canonicalized - {} vs {}", 
                    first_mods.as_deref().unwrap_or("none"),
                    mods.as_deref().unwrap_or("none"));
            }
        }
    ...
    for group in qr_tests {
        ...
        for (input, ast) in &asts[1..] {
            ...
            if first_mods != mods {
                println!("Note: qr// modifiers are not canonicalized - {} vs {}", 
                    first_mods.as_deref().unwrap_or("none"),
                    mods.as_deref().unwrap_or("none"));
            }
        }
...
fn extract_modifiers_from_node(node: &perl_parser::ast::Node) -> Option<String> {
    ...
    match &node.kind {
        ...
        NodeKind::Regex { modifiers, .. } |
        NodeKind::Match { modifiers, .. } => {
            Some(modifiers.clone())
        }
        NodeKind::Substitution { modifiers, .. } => {
            Some(modifiers.clone())
        }
        NodeKind::Binary { left, right, .. } => {
            extract_modifiers_from_node(left)
                .or_else(|| extract_modifiers_from_node(right))
        }
