...
fn test_lsp_workspace_symbol_no_internal_fields() {
    ...
    let lsp_symbols: Vec<LspWorkspaceSymbol> = internal_symbols
        .iter()
        .map(|sym| sym.into())
        .collect();
    ...
    if let Value::Array(symbols) = json_value {
        for symbol in symbols {
            ...
            assert!(!obj.contains_key("has_body"), "Internal field 'has_body' leaked to LSP output");
...
fn test_symbol_kind_uses_lsp_constants() {
    ...
    for kind in &[
        ...
        assert!(valid_lsp_kinds.contains(&lsp_kind), 
            "{:?} maps to invalid LSP kind {}", kind, lsp_kind);
    }
}
