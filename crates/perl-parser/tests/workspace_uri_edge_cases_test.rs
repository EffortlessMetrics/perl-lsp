...
"#;
        ...
        if let Some(result) = result {
            ...
            assert!(def_uri.contains("Math%20Utils.pm"), 
                    "Definition should be in 'Math Utils.pm' with encoded space");
            assert!(def_uri.contains("Lib%20Modules"),
...
"#;
        ...
        if let Some(result) = result {
            ...
            let lines: Vec<u32> = references.iter()
...
"#;
        ...
        if let Some(result) = result {
            ...
            assert!(labels.iter().any(|l| l.contains("日本語")), 
                    "Should have Japanese function");
            assert!(labels.iter().any(|l| l.contains("café")), 
                    "Should have accented function");
            assert!(labels.iter().any(|l| l.contains("𝕦𝕟𝕚𝕔𝕠𝕕𝕖")), 
