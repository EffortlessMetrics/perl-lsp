...
fn benchmark_initialization() {
    ...
    assert!(duration < MAX_INIT_TIME, 
            "Initialization took {:?}, expected < {:?}", duration, MAX_INIT_TIME);
}
...
"#;
    ...
    assert!(duration < MAX_PARSE_TIME_PER_KB * (size_kb.ceil() as u32),
...
fn benchmark_parsing_large_file() {
    ...
    println!("Large file parse time: {:?} ({:.0} µs/KB for {:.1} KB)",
...
"#;
    ...
    assert!(duration < MAX_DIAGNOSTIC_TIME,
            "Diagnostics took {:?}, expected < {:?}", duration, MAX_DIAGNOSTIC_TIME);
}
...
fn benchmark_document_symbols() {
    ...
    assert!(duration < MAX_SYMBOL_TIME,
            "Symbol extraction took {:?}, expected < {:?}", duration, MAX_SYMBOL_TIME);
}
...
"#;
    ...
    assert!(duration < MAX_DEFINITION_TIME,
            "Go to definition took {:?}, expected < {:?}", duration, MAX_DEFINITION_TIME);
}
...
"#;
    ...
    assert!(duration < MAX_REFERENCES_TIME,
            "Find references took {:?}, expected < {:?}", duration, MAX_REFERENCES_TIME);
}
...
"#;
    ...
    assert!(duration < MAX_HOVER_TIME,
            "Hover took {:?}, expected < {:?}", duration, MAX_HOVER_TIME);
}
...
fn benchmark_concurrent_requests() {
    ...
    assert!(avg_per_request < Duration::from_millis(50),
