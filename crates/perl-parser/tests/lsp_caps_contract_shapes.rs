...
fn test_capability_shapes_lsp_318_contract() {
    ...
    assert!(caps_json["textDocumentSync"].is_object(), 
    ...
    if build.completion {
        assert!(caps_json["completionProvider"].is_object(),
    ...
    if build.hover {
        assert!(caps_json["hoverProvider"].is_boolean() || 
                caps_json["hoverProvider"].is_object());
    }
    ...
    assert!(caps_json["documentHighlightProvider"].is_boolean() ||
    ...
    if build.document_symbol {
        assert!(caps_json["documentSymbolProvider"].is_boolean() ||
    ...
    if build.code_actions {
        assert!(caps_json["codeActionProvider"].is_object(),
            "codeActionProvider must be an object per LSP 3.18");
        assert!(caps_json["codeActionProvider"]["codeActionKinds"].is_array(),
    ...
    if build.code_lens {
        assert!(caps_json["codeLensProvider"].is_object(),
    ...
    if build.document_color {
        assert!(caps_json["colorProvider"].is_boolean() ||
                caps_json["colorProvider"].is_object());
    }
    
    // Test formatting shape (boolean or object)
    if build.formatting {
        assert!(caps_json["documentFormattingProvider"].is_boolean() ||
                caps_json["documentFormattingProvider"].is_object());
    }
    
    // Test range formatting shape (boolean or object)
    if build.range_formatting {
        assert!(caps_json["documentRangeFormattingProvider"].is_boolean() ||
    ...
    if build.rename {
        assert!(caps_json["renameProvider"].is_object(),
    ...
    if build.moniker {
        assert!(caps_json["monikerProvider"].is_boolean() ||
                caps_json["monikerProvider"].is_object());
    }
    
    // Test inline value shape (boolean or object)
    if build.inline_values {
        assert!(caps_json["inlineValueProvider"].is_boolean() ||
    ...
    if build.workspace_symbol {
        assert!(caps_json["workspaceSymbolProvider"].is_boolean() ||
    ...
    if build.inline_completion {
        ...
        assert!(caps_json["experimental"].is_object(),
            "experimental must exist for inline completion");
        assert!(caps_json["experimental"]["inlineCompletionProvider"].is_object(),
