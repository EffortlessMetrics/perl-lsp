...
fn validate_position_boundary_conditions() {
    ...
    for (pos, description) in boundary_tests {
        ...
        assert!(byte_offset_utf16 <= rope.len_bytes(), 
               "UTF-16 byte offset {} exceeds bounds for {}", byte_offset_utf16, description);
        assert!(byte_offset_utf8 <= rope.len_bytes(),
               "UTF-8 byte offset {} exceeds bounds for {}", byte_offset_utf8, description);
        ...
        assert!(back_utf16.line < rope.len_lines() as u32, 
               "UTF-16 converted line {} exceeds document lines for {}", back_utf16.line, description);
        assert!(back_utf8.line < rope.len_lines() as u32,
               "UTF-8 converted line {} exceeds document lines for {}", back_utf8.line, description);
    }
}
...
fn validate_position_conversion_performance() {
    ...
    println!("Position conversion performance: {:?} for {} conversions", duration, test_positions.len() * 4);
    
    // Performance should be reasonable for large documents
    assert!(duration.as_millis() < 10, 
           "Position conversion took {} ms, expected < 10ms", duration.as_millis());
}
...
fn validate_problematic_unicode_scenarios() {
    ...
    for (content, test_pos, description) in problematic_cases {
        ...
        assert!(line_match, "Line mismatch for {}: expected {}, got {}", 
                description, test_pos.line, converted_back.line);
        
        // Character positions may vary for complex Unicode, but should be reasonable
        assert!(char_diff <= 10, "Character position too far off for {}: diff = {}", 
                description, char_diff);
