...
"#;
        ...
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                for iteration in 0..iterations_per_thread {
                    ...
                    match result {
                        Ok(_ast) => {
                            results_clone.lock().unwrap().push((thread_id, iteration, true, parse_time));
                        }
                        Err(e) => {
                            *error_count_clone.lock().unwrap() += 1;
                            results_clone.lock().unwrap().push((thread_id, iteration, false, parse_time));
    ...
    println!("  ✓ Thread safety: {} operations, success rate: {:.1}%, errors: {}",
             results.len(), success_rate * 100.0, error_count);
...
fn test_concurrent_parsing_requests() {
    ...
    for (thread_id, request, code_index, success, parse_time, code_size) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_millis(100),
            ...
            thread_id, request, code_index, parse_time
        );
    }
    ...
    println!("  ✓ Concurrent requests: {} total, avg time: {:?}, success: {:.1}%, throughput: {:.0} bytes/sec",
             results.len(), avg_time_per_request, success_rate * 100.0, throughput);
...
"#;
    ...
    for (thread_id, modification, success, parse_time, code_size) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_millis(50),
            ...
            thread_id, modification, parse_time
        );
    }
    ...
    println!("  ✓ Incremental parsing: {} operations, avg time: {:?}, success: {:.1}%",
...
fn test_concurrent_workspace_indexing() {
    ...
    let workspace_files: Vec<String> = (0..(thread_count * files_per_thread))
        .map(|i| generate_workspace_file(i))
        .collect();
            ...
            thread::spawn(move || {
                for file_index in 0..files_per_thread {
                    ...
                    if let Ok(ast) = result {
                        ...
                        {
                            let mut index = workspace_index_clone.write().unwrap();
                            for symbol in &symbols {
                                index.insert(symbol, global_file_index);
                            }
                        }
    ...
    let results = results.lock().unwrap();
    let workspace_index = workspace_index.read().unwrap();
    ...
    for (thread_id, file_index, success, parse_time, symbol_count) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_millis(100),
            ...
            thread_id, file_index, parse_time
        );
    }
    ...
    println!("  ✓ Workspace indexing: {} files, avg time: {:?}, success: {:.1}%, total symbols: {}",
             results.len(), avg_time, success_rate * 100.0, workspace_index.len());
...
fn test_high_contention_scenarios() {
    ...
    for (thread_id, operation, success, parse_time) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_millis(200),
            ...
            thread_id, operation, parse_time
        );
    }
    ...
    println!("  ✓ High contention: {} operations, avg time: {:?}, success: {:.1}%",
             results.len(), avg_time, success_rate * 100.0);
    ...
    for (thread_id, operation, operation_name, success, parse_time) in results.iter() {
        ...
        assert!(
            *parse_time < Duration::from_millis(500),
            ...
            thread_id, operation, operation_name, parse_time
        );
    }
        ...
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                for iteration in 0..stress_iterations {
                    ...
                    stress_results_clone.lock().unwrap().push((
                        thread_id,
                        iteration,
                        result.is_ok(),
                        parse_time,
        ...
        .map(|thread_id| {
            ...
            thread::spawn(move || {
                for iteration in 0..stress_iterations {
                    ...
                    let counter_value = {
                        let counter = shared_counter_clone.lock().unwrap();
                        *counter
                    };
...
"#, counter_value);
                    ...
                    recovery_results_clone.lock().unwrap().push((
                        thread_id,
                        iteration,
                        result.is_ok(),
                        parse_time,
    ...
    for handle in stress_handles {
        handle.join().expect("Stress thread should complete successfully");
    }
    
    for handle in recovery_handles {
        handle.join().expect("Recovery thread should complete successfully");
    }
    
    let stress_results = stress_results.lock().unwrap();
    let recovery_results = recovery_results.lock().unwrap();
    ...
    let recovery_success_count = recovery_results.iter().filter(|(_, _, success, _, _)| *success).count();
    let recovery_success_rate = recovery_success_count as f64 / recovery_results.len() as f64;
    
    println!("  ✓ Stress and recovery:");
    println!("    Stress: {} operations, success: {:.1}%", stress_results.len(), stress_success_rate * 100.0);
    println!("    Recovery: {} operations, success: {:.1}%", recovery_results.len(), recovery_success_rate * 100.0);
...
fn modify_code(base_code: &str, thread_id: usize, modification: usize) -> String {
    ...
    modified.push_str(&format!("my $thread_{}_mod_{} = {};\n", thread_id, modification, thread_id * modification));
    modified.push_str(&format!("print \"Thread {} modification {}: $thread_{}_mod_{}\\n\";\n", 
                                thread_id, modification, thread_id, modification));
