use perl_parser::{Parser, PragmaTracker, NodeKind};
...
fn walk_ast(node: &perl_parser::Node, pragma_map: &[(std::ops::Range<usize>, perl_parser::PragmaState)], depth: usize) {
    let indent = "  ".repeat(depth);
    let pragma_state = PragmaTracker::state_for_offset(pragma_map, node.location.start);
    
    match &node.kind {
        NodeKind::Identifier { name } => {
            println!("{}Identifier '{}' at offset {}: strict_subs={}", 
                indent, name, node.location.start, pragma_state.strict_subs);
        }
        NodeKind::FunctionCall { name, args } => {
            println!("{}FunctionCall '{}' at offset {}: strict_subs={}", 
                indent, name, node.location.start, pragma_state.strict_subs);
            for arg in args {
