...
fn demo_simple_value_change() {
    ...
    let reuse_percent = (parser.reused_nodes as f64 / 
...
fn demo_performance() {
    ...
    println!("Test program: {} lines, {} bytes", 
             source.lines().count(), source.len());
    ...
    println!("Speedup: {:.1}x", full_parse_time.as_secs_f64() / inc_parse_time.as_secs_f64());
    ...
    let reuse_percent = (inc_parser.reused_nodes as f64 / 
        (inc_parser.reused_nodes + inc_parser.reparsed_nodes) as f64) * 100.0;
    println!("  Tree reuse: {:.1}%", reuse_percent);
}