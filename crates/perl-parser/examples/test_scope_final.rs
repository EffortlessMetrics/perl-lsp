use perl_parser::{
    Parser,
    scope_analyzer::{IssueKind, ScopeAnalyzer},
};
...
fn main() {
    ...
    for (name, code, expected_count, expected_kind) in tests {
        ...
        let relevant_issues: Vec<_> = issues.iter().filter(|i| i.kind == expected_kind).collect();
