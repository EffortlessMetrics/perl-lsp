...
fn main() {
    ...
    for (category_name, tests) in categories {
        ...
        println!("{:<25} {:>4}/{:<4} ({:>5.1}%)", 
                 category_name, passed, passed + failed, percentage);
    }
    ...
    for failure in &all_failures {
        ...
        failure_patterns.entry(pattern.to_string()).or_insert_with(Vec::new).push(failure);
    }
    ...
    for (category, (_, failed, _)) in &category_stats {
        if *failed > 0 {
            ...
            let category_failures: Vec<_> = all_failures.iter()
    ...
    for failure in &all_failures {
        ...
        } else if failure.description.contains("v-string") || failure.description.contains("version") {
            "Version strings"
        } else if failure.description.contains("unicode") || failure.description.contains("encoding") {
            "Unicode/Encoding"
