...
"#;
        ...
        for (i, (description, edit)) in test_edits.iter().enumerate() {
            ...
            println!(
                "  Editing at position {} (replacing {} bytes with {} bytes)",
                edit.start,
                edit.end - edit.start,
                edit.new_text.len()
            );
        ...
        println!(
            ...
            final_stats.tokens_reused + final_stats.tokens_relexed
        );
        ...
        println!(
            "1. **Context Preservation**: Checkpoints save lexer mode (ExpectTerm vs ExpectOperator)"
        );
        println!(
            "2. **Stateful Lexing**: Handles constructs like heredocs, formats, and nested quotes"
        );
