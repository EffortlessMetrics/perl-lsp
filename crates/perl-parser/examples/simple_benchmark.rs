...
sub process_data {
    ...
    println!("{:<15} {:<10} {:<15} {:<15}", "Test Case", "Size", "Avg Time (µs)", "Throughput (MB/s)");
    println!("{:-<55}", "");
    ...
    for (name, code) in &test_cases {
        ...
        let throughput_mbps = (code_bytes as f64 * ITERATIONS as f64) / elapsed.as_secs_f64() / 1_000_000.0;
        
        println!("{:<15} {:<10} {:<15.2} {:<15.2}", 
            name, 
            format!("{} B", code_bytes),
            avg_time_us,
            throughput_mbps
    ...
    let avg_throughput = (total_bytes as f64 * ITERATIONS as f64 * test_cases.len() as f64) 
        / (total_time * test_cases.len() as f64 / 1_000_000.0) / 1_000_000.0;
    ...
    println!("  • Parse rate: ~{:.0} µs/KB", avg_time * 1000.0 / (total_bytes as f64 / test_cases.len() as f64));
