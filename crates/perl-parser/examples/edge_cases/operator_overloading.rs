...
pub fn get_tests() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            r#"use overload '+' => sub { $_[0]{val} + $_[1] }"#,
            "overload addition",
        ),
        (
            r#"use overload '-' => sub { $_[0]{val} - $_[1] }"#,
        ...
        (
            r#"use overload 'cmp' => sub { $_[0]{str} cmp $_[1] }"#,
            "overload string compare",
        ),
        (
            r#"use overload '<' => sub { $_[0]{val} < $_[1] }"#,
            "overload less than",
        ...
        (
            r#"use overload '>=' => sub { $_[0]{val} >= $_[1] }"#,
            "overload greater equal",
        ),
        (
            r#"use overload '==' => sub { $_[0]{val} == $_[1] }"#,
            "overload numeric equality",
        ),
        (
            r#"use overload '!=' => sub { $_[0]{val} != $_[1] }"#,
            "overload numeric inequality",
        ),
        (
            r#"use overload 'eq' => sub { $_[0]{str} eq $_[1] }"#,
            "overload string equality",
        ),
        (
            r#"use overload 'ne' => sub { $_[0]{str} ne $_[1] }"#,
            "overload string inequality",
        ),
        // Bitwise overloading
        (
            r#"use overload '&' => sub { $_[0]{val} & $_[1] }"#,
        ...
        (
            r#"use overload '>>' => sub { $_[0]{val} >> $_[1] }"#,
            "overload right shift",
        ),
        // String overloading
        (
            r#"use overload '""' => sub { $_[0]->to_string }"#,
            "overload stringification",
        ),
        (
            r#"use overload '0+' => sub { $_[0]->to_number }"#,
            "overload numification",
        ),
        (
            r#"use overload 'bool' => sub { $_[0]->is_true }"#,
            "overload boolification",
        ),
        (
            r#"use overload '!' => sub { !$_[0]->is_true }"#,
            "overload negation",
        ),
        (
            r#"use overload 'qr' => sub { $_[0]->to_regex }"#,
            "overload regex conversion",
        ),
        // Special overloading
        (
            r#"use overload '=' => sub { $_[0]->clone }"#,
            "overload assignment",
        ),
        (
            r#"use overload 'x' => sub { $_[0]->repeat($_[1]) }"#,
            "overload repetition",
        ),
        (
            r#"use overload '.' => sub { $_[0]->concat($_[1]) }"#,
            "overload concatenation",
        ),
        (
            r#"use overload 'abs' => sub { abs($_[0]{val}) }"#,
            "overload abs",
        ),
        (
            r#"use overload 'neg' => sub { bless { val => -$_[0]{val} } }"#,
            "overload negation",
        ),
        (
            r#"use overload '++' => sub { $_[0]{val}++ }"#,
        ...
        (
            r#"use overload 'atan2' => sub { atan2($_[0]{y}, $_[0]{x}) }"#,
            "overload atan2",
        ),
        (
            r#"use overload 'cos' => sub { cos($_[0]{angle}) }"#,
            "overload cos",
        ),
        (
            r#"use overload 'sin' => sub { sin($_[0]{angle}) }"#,
            "overload sin",
        ),
        (
            r#"use overload 'exp' => sub { exp($_[0]{val}) }"#,
            "overload exp",
        ),
        (
            r#"use overload 'log' => sub { log($_[0]{val}) }"#,
            "overload log",
        ),
        (
            r#"use overload 'sqrt' => sub { sqrt($_[0]{val}) }"#,
            "overload sqrt",
        ),
        (
            r#"use overload 'int' => sub { int($_[0]{val}) }"#,
            "overload int",
        ),
        // Dereferencing overloading
        (
            r#"use overload '@{}' => sub { $_[0]{array} }"#,
            "overload array deref",
        ),
        (
            r#"use overload '%{}' => sub { $_[0]{hash} }"#,
            "overload hash deref",
        ),
        (
            r#"use overload '&{}' => sub { $_[0]{code} }"#,
            "overload code deref",
        ),
        (
            r#"use overload '*{}' => sub { $_[0]{glob} }"#,
            "overload glob deref",
        ),
        (
            r#"use overload '${}' => sub { \$_[0]{scalar} }"#,
            "overload scalar deref",
        ),
        // Multiple overloads
        (
            r#"use overload
    ...
    '""' => sub { $_[0]{str} }"#,
        ...
        // Fallback
        (
            r#"use overload '+' => sub { ... }, fallback => 1"#,
            "overload with fallback",
        ),
        (
            r#"use overload '+' => sub { ... }, fallback => 0"#,
            "overload no fallback",
        ),
        (
            r#"use overload '+' => sub { ... }, fallback => 'undef'"#,
            "overload undef fallback",
        ),
        // Method names instead of subs
        (r#"use overload '+' => 'add'"#, "overload with method name"),
        (
            r#"use overload '-' => 'subtract', '*' => 'multiply'"#,
