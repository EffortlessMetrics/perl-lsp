...
use perl_lsp_launcher::{
    LaunchAction, LaunchConfig, TransportMode, enable_stderr_logging, format_health_output,
    format_info_output, help_text, parse_args, port_in_use_message, shell_completion,
};
use std::env;
use std::process;
use std::sync::Arc;
...
fn run_server(launch_config: LaunchConfig) {
    if launch_config.enable_logging
        && let Err(error) = enable_stderr_logging(
            "perl-lsp",
            "perl_lsp=info",
