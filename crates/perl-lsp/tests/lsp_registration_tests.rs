...
fn test_register_capability_request_contract() -> TestResult {
    ...
    assert!(
        reg["registerOptions"].is_object(),
        "registration must have registerOptions"
    );

    // Validate file watcher options
    let watchers = reg["registerOptions"]["watchers"]
        .as_array()
    ...
    for watcher in watchers {
        ...
        assert!(
            kind > 0 && kind <= 7,
            "kind must be a valid WatchKind bitmask (1..=7)"
        );
    }
...
fn test_unregister_capability_request_contract() -> TestResult {
    ...
    assert!(
        unreg["method"].is_string(),
        "unregistration must have a method"
    );
...
fn test_register_multiple_capabilities_at_once() -> TestResult {
    ...
    let id0 = registrations[0]["id"]
        .as_str()
        .ok_or("id must be a string")?;
    let id1 = registrations[1]["id"]
        .as_str()
        .ok_or("id must be a string")?;
...
fn test_file_watcher_glob_pattern_variants() -> TestResult {
    ...
    for watcher in watchers {
        ...
        assert!(
            (1..=7).contains(&kind),
            "kind {kind} is not a valid WatchKind bitmask"
        );
    }
