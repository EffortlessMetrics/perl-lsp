...
"#;
        ...
        if let Some(result) = result {
            ...
            let items = result.get("items").and_then(|i| i.as_array()).or_else(|| result.as_array());
...
mod deadline_enforcement_tests {
    ...
    fn test_workspace_symbols_early_exit_returns_partial() {
        ...
        assert!(
            result.is_ok(),
            "Workspace symbols should return Ok, not timeout"
        );
...
mod windows_uri_path_tests {
    ...
    fn test_drive_letter_case_normalization() {
        ...
        assert!(
            result_upper.is_ok(),
            "Upper case drive letter should work"
        );
...
"#;
    ...
    fn test_backslash_path_traversal_security() {
        ...
        assert!(
            result.is_ok(),
            "Should handle suspicious paths without crashing"
        );
    }
    ...
    fn test_unc_path_handling() {
        ...
        assert!(
            result.is_ok(),
            "Should handle UNC-style paths without crashing"
        );
    }
}
