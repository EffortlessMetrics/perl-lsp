...
fn on_type_braces_indent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = srv
        .handle_request(req)
        .ok_or("onTypeFormatting request failed")?;
    let edits = res
        .result
        .ok_or("onTypeFormatting response missing result")?;
...
fn on_type_newline_after_open_brace_indents() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = srv
        .handle_request(req)
        .ok_or("onTypeFormatting newline request failed")?;
    let edits = res
        .result
        .ok_or("onTypeFormatting newline response missing result")?;
    ...
    assert!(
        ...
        "should return indent edits after open brace newline"
    );
    Ok(())
}
...
fn on_type_tab_size_4_produces_four_space_indent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = srv
        .handle_request(req)
        .ok_or("onTypeFormatting tabSize:4 request failed")?;
    let edits = res
        .result
        .ok_or("onTypeFormatting tabSize:4 response missing result")?;
