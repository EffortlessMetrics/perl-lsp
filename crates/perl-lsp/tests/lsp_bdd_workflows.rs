...
#[test]
fn bdd_goto_definition_resolves_builtin_functions() -> Result<(), Box<dyn std::error::Error>> {
    let scenario = BddScenario::new("Go-to-definition for built-in functions");
    scenario.given("a script calling a built-in function like print");
...
"#;
    ...
    scenario.when("requesting definition on the built-in function");
...
#[test]
fn bdd_hover_on_builtin_functions_shows_docs() -> Result<(), Box<dyn std::error::Error>> {
    let scenario = BddScenario::new("Hover on built-in functions shows documentation");
    scenario.given("a script calling a built-in function like 'print'");
...
"#;
    ...
    scenario.when("requesting hover on the built-in function 'print'");
    let (line, col) = find_position(script, "print");
    ...
    scenario.then("the hover response should contain the documentation for 'print'");
    assert!(!response.is_null(), "Hover response should not be null");
    let hover_text_str = hover_text(&response);
    // Built-in docs usually contain the function signature or description
    assert!(
        hover_text_str.contains("print"),
        "Hover should contain docs for print, got: {}",
        hover_text_str
    );
...
#[test]
fn bdd_diagnostics_flag_missing_strict() -> Result<(), Box<dyn std::error::Error>> {
    let scenario = BddScenario::new("Diagnostics flag missing strict pragma");
    scenario.given("a script missing 'use strict'");

    let script = r#"use warnings;
...
"#;
    ...
    scenario.then("the diagnostics should include a warning about missing strict");
    let diagnostics = diagnostic_items(&response);

    // We expect a diagnostic for missing strict
    let strict_diagnostic = diagnostics.iter().find(|d| {
        let msg = d["message"].as_str().unwrap_or_default();
        msg.contains("strict") && msg.contains("adding 'use strict;'")
    });

    assert!(
        strict_diagnostic.is_some(),
        "Expected diagnostic for missing strict, got: {:?}",
        diagnostics
    );
...
#[test]
fn bdd_diagnostics_flag_unused_lexical_variables() -> Result<(), Box<dyn std::error::Error>> {
    let scenario = BddScenario::new("Diagnostics flag unused lexical variables");
    scenario.given("a script with an unused lexical variable");
...
"#;
    ...
    scenario.then("the diagnostics should include a warning about the unused variable");
    let diagnostics = diagnostic_items(&response);

    let unused_diagnostic = diagnostics.iter().find(|d| {
        let msg = d["message"].as_str().unwrap_or_default();
        msg.contains("unused") && msg.contains("$unused_var")
    });

    assert!(
        unused_diagnostic.is_some(),
        "Expected diagnostic for unused variable, got: {:?}",
        diagnostics
    );
...
#[test]
fn bdd_document_symbols_hierarchical_outline() -> Result<(), Box<dyn std::error::Error>> {
    let scenario = BddScenario::new("Document symbols hierarchical outline");
    scenario.given("a script with variables, subroutines, and a package");

    let script = r#"package MyModule;
use strict;
use warnings;
...
"#;
    ...
    scenario.then("the document symbols should include the hierarchical structure of the file");
    let empty_vec = vec![];
    let symbols = response.as_array().unwrap_or(&empty_vec);

    // Check if we have the package
    let package_symbol =
        symbols.iter().find(|s| s["name"].as_str().unwrap_or_default() == "MyModule");
    assert!(package_symbol.is_some(), "Expected MyModule package in document symbols");

    let package_obj = package_symbol.unwrap();
    // Sometimes subroutines are nested inside packages in hierarchical outlines
    let children = package_obj["children"].as_array();

    let has_do_work = if let Some(kids) = children {
        kids.iter().any(|k| k["name"].as_str().unwrap_or_default() == "do_work")
    } else {
        symbols.iter().any(|s| s["name"].as_str().unwrap_or_default() == "do_work")
    };

    assert!(has_do_work, "Expected do_work subroutine in document symbols hierarchy");
