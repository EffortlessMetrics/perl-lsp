...
fn test_pragma_code_actions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_strict_action = actions
        .iter()
        .any(|a| a["title"].as_str() == Some("Add use strict;"));
    let has_warnings_action = actions
        .iter()
        .any(|a| a["title"].as_str() == Some("Add use warnings;"));
...
fn test_formatting_provider_capability() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_formatting = result["capabilities"]["documentFormattingProvider"]
        .as_bool()
        .unwrap_or(false);
