...
"#,
    ...
    if !response.is_null() {
        ...
        if let Some(changes) = response.get("changes") {
            ...
            assert!(
                changes.is_object(),
                "changes should be an object mapping URIs to edits"
            );
