//! Scenario 40 - Dancer2 diagnostics quality receipt.
//!
//! This receipt exercises diagnostics over the committed Dancer2 skeleton
//! workspace and records a second real-project false-positive / false-negative
//! boundary without changing provider behavior.
//!
//! Receipt signals:
//! - diagnostic notification and payload shape for selected real-workspace files
//! - no false PL701 for Dancer2 modules present in the fixture workspace
//! - conservative handling for dynamic typeglob DSL/plugin export boundaries
//! - true-missing-module PL701 behavior from injected probe files
...
const MISSING_MODULE_PROBE_PATH: &str = "lib/Dancer2/DiagnosticsProbe.pm";
const MIXED_MODULE_PROBE_PATH: &str = "lib/Dancer2/DiagnosticsMixedProbe.pm";
const MISSING_MODULE_PROBE_SOURCE: &str = r#"package Dancer2::DiagnosticsProbe;
use strict;
use warnings;
use Dancer2;
use Definitely::Missing::ForDancer2Receipt;

sub diagnostic_probe { return 1 }

1;
"#;
const MIXED_MODULE_PROBE_SOURCE: &str = r#"package Dancer2::DiagnosticsMixedProbe;
use strict;
use warnings;
use Dancer2::Core::App;
use Dancer2::Core::DSL;
use Definitely::Missing::MixedDancer2Receipt;

sub mixed_diagnostic_probe {
  return (Dancer2::Core::App->new(name => 'receipt', runner => undef), Dancer2::Core::DSL->new(app => undef));
}
...
fn diagnostic_probes() -> Vec<DiagnosticProbe> {
    vec![
        DiagnosticProbe {
            name: "workspace_present_dancer2_imports",
        ...
        DiagnosticProbe {
            name: "dynamic_dsl_export_boundary",
            category: "dynamic_boundary_conservative",
            file: "lib/Dancer2.pm",
            absent_pl701_modules: &[],
            required_codes: &[],
            required_message_substrings: &[],
            forbidden_message_substrings: &["${caller}::${name}"],
        },
        DiagnosticProbe {
            name: "dynamic_plugin_export_boundary",
            category: "dynamic_boundary_conservative",
            file: "lib/Dancer2/Plugin.pm",
            absent_pl701_modules: &[],
            required_codes: &[],
            required_message_substrings: &[],
            forbidden_message_substrings: &["${plugin_class}::${kw}", "${caller}::register"],
        },
        DiagnosticProbe {
            ...
            required_message_substrings: &["Definitely::Missing::ForDancer2Receipt"],
            forbidden_message_substrings: &[],
        },
        DiagnosticProbe {
            ...
            absent_pl701_modules: &["Dancer2::Core::App", "Dancer2::Core::DSL"],
            required_codes: &["PL701"],
            required_message_substrings: &["Definitely::Missing::MixedDancer2Receipt"],
            forbidden_message_substrings: &[],
        },
    ]
}
...
#[test]
fn scenario_40_dancer2_diagnostics_quality_receipt() {
    run_ux_scenario(
        "dancer2_diagnostics_quality",
        SCENARIO_FILE,
        "scenario_40_dancer2_diagnostics_quality_receipt",
        UxCiTier::Pr,
        ...
        |recorder| {
            ...
            let receipt = serde_json::json!({
                ...
                "project": "dancer2",
                "surface": "diagnostics",
                "claim_boundary": "second-project diagnostics quality receipt only; no provider behavior changed or promoted",
            ...
            eprintln!(
                "dancer2_diagnostics_quality_receipt={}",
            ...
            recorder.check(
                "workspace-present Dancer2 imports did not emit PL701",
                false_positive_pl701_total == 0,
            )?;
            recorder.check(
                "dynamic Dancer2 typeglob exports stayed conservative",
