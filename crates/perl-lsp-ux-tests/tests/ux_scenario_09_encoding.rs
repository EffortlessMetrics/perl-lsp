...
fn scenario_09_utf8_bom_file_does_not_crash() {
    ...
    harness
        ...
        .expect("didOpen should succeed with UTF-8 BOM");

    let hover = harness.hover("bom.pl", 2, 3);
    assert!(
        hover.is_ok(),
        "hover crashed on UTF-8 BOM file — UX regression: {:?}",
        hover
    );
...
fn scenario_09_unicode_in_strings_does_not_crash() {
    ...
    harness
        ...
        .expect("didOpen should succeed with use utf8");

    let hover = harness.hover("utf8_decl.pl", 4, 3);
    assert!(
        hover.is_ok(),
        "hover crashed on use utf8 file — UX regression: {:?}",
        hover
    );
...
fn scenario_09_high_codepoint_comments_do_not_crash() {
    ...
    assert!(
        hover.is_ok(),
        "hover crashed on smart-quote comment — UX regression: {:?}",
        hover
    );
...
fn scenario_09_latin1_extended_chars_in_comment() {
    ...
    assert!(
        hover.is_ok(),
        "hover crashed on Latin-1 comment — UX regression: {:?}",
        hover
    );
