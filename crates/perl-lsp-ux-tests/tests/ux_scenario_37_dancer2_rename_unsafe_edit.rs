//! Scenario 37 - Dancer2 rename unsafe-edit receipt.
//!
//! This receipt exercises `textDocument/rename` over a second committed
//! real-workspace skeleton. It records rename safety boundaries without
//! broadening live rename behavior.
//!
//! Receipt signals:
...
fn rename_probes() -> Vec<RenameProbe> {
    vec![
        RenameProbe {
            name: "exact_local_lexical_route_code",
            category: "exact_local_lexical",
            file: APP_FILE,
            zero_based_line: 25,
            needle: "$code",
            cursor_offset: 2,
            new_name: "renamed_route_code",
            expectation: RenameExpectation::ExactLocalEdits,
        },
        RenameProbe {
            name: "generated_accessor_routes",
        ...
        RenameProbe {
            name: "dynamic_typeglob_plugin_keywords",
            category: "dynamic_typeglob_boundary",
            file: PLUGIN_FILE,
            zero_based_line: 15,
            needle: "plugin_keywords",
            cursor_offset: 1,
            new_name: "plugin_words",
            expectation: RenameExpectation::NoUnsafeEdits,
        },
    ]
}
...
fn freshness_report(harness: &UxHarness, files: &[FixtureFile]) -> Result<FreshnessReport> {
    let original = fixture_content(files, APP_FILE)?;
    let updated = original.replace("my $code    = $args{code};", "my $fresh_code = $args{code};");
    if updated == original {
        return Err(anyhow!("freshness fixture must rename route code to fresh_code"));
    }

    harness.change_file_full(APP_FILE, &updated)?;
    std::thread::sleep(Duration::from_millis(300));

    let position = resolve_position_in_source(&updated, APP_FILE, 25, "$fresh_code", 2)?;
    let response = rename_request(harness, APP_FILE, &position, "renamed_fresh_code")?;
    let edits = collect_rename_edits(files, &response, Some(APP_FILE), Some(&updated))?;
    let state = response_state(&response, edits.len());
    let stale_source_edit_count = edits.iter().filter(|edit| edit.source_text == "$code").count();
    let fresh_source_edit_count =
        edits.iter().filter(|edit| edit.source_text == "$fresh_code").count();
    let (_, _, touched_source_texts, new_texts) = summarize_touched_values(&edits);

    Ok(FreshnessReport {
        file: APP_FILE,
        old_name: "$code",
        fresh_name: "$fresh_code",
        requested_name: "$renamed_fresh_code",
        response_state: state,
        edit_count: edits.len(),
...
#[test]
fn scenario_37_dancer2_rename_unsafe_edit_receipt() {
    run_ux_scenario(
        "dancer2_rename_unsafe_edit",
        SCENARIO_FILE,
        "scenario_37_dancer2_rename_unsafe_edit_receipt",
        UxCiTier::Pr,
        ...
        |recorder| {
            ...
            let exact_local_report = reports
                .iter()
                .find(|report| report.name == "exact_local_lexical_route_code")
                .context("missing exact local rename report")?;
            let generated_report = reports
                .iter()
                .find(|report| report.name == "generated_accessor_routes")
                .context("missing generated accessor report")?;
            let dynamic_report = reports
                .iter()
                .find(|report| report.name == "dynamic_typeglob_plugin_keywords")
            ...
            let receipt = json!({
                ...
                "project": "dancer2",
                "surface": "rename",
                "claim_boundary": "second real-workspace rename unsafe-edit receipt only; no live rename behavior promoted",
            ...
            eprintln!(
                "dancer2_rename_unsafe_edit_receipt={}",
            ...
            recorder.check(
                ...
                exact_local_report.response_state == RenameResponseState::Edits
                    ...
                    && exact_local_report.touched_relative_paths == vec![APP_FILE.to_string()]
                    && exact_local_report.touched_source_texts == vec!["$code".to_string()]
                    && exact_local_report.new_texts == vec!["$renamed_route_code".to_string()]
                    && exact_local_lines == BTreeSet::from([25, 26, 29]),
            )?;
            recorder.check(
