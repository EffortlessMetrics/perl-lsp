...
//! Dancer2 skeleton workspace. It records second-project live token quality
...
const FRESHNESS_FILE: &str = "lib/Dancer2/Core/DSL.pm";
...
fn semantic_token_probes() -> Vec<SemanticTokenProbe> {
    vec![
        SemanticTokenProbe {
            name: "dancer2_export_tokens",
            category: "source_backed_package_sub_and_vars",
            file: "lib/Dancer2.pm",
            expected_tokens: &[
                ExpectedToken { text: "package", allowed_types: &["keyword"] },
                ExpectedToken { text: "Dancer2", allowed_types: &["namespace", "class"] },
                ExpectedToken { text: "sub", allowed_types: &["keyword"] },
                ExpectedToken { text: "import", allowed_types: &["function", "method"] },
                ExpectedToken { text: "$runner", allowed_types: &["variable", "parameter"] },
            ],
            dynamic_boundary_texts: &["\"${caller}::${name}\""],
        },
        SemanticTokenProbe {
            name: "app_tokens",
            category: "source_backed_app_symbols",
            file: "lib/Dancer2/Core/App.pm",
            expected_tokens: &[
                ExpectedToken {
                    text: "Dancer2::Core::App",
                    allowed_types: &["namespace", "class"],
                },
                ExpectedToken { text: "add_route", allowed_types: &["function", "method"] },
                ExpectedToken { text: "dispatch", allowed_types: &["function", "method"] },
        ...
        SemanticTokenProbe {
            name: "dsl_tokens",
            category: "source_backed_dsl_symbols",
            file: "lib/Dancer2/Core/DSL.pm",
            expected_tokens: &[
                ExpectedToken {
                    text: "Dancer2::Core::DSL",
                    allowed_types: &["namespace", "class"],
                },
                ExpectedToken { text: "get", allowed_types: &["function", "method"] },
                ExpectedToken { text: "_compile", allowed_types: &["function", "method"] },
                ExpectedToken { text: "$pattern", allowed_types: &["variable", "parameter"] },
            ],
            dynamic_boundary_texts: &[],
        },
        SemanticTokenProbe {
            name: "plugin_typeglob_tokens",
            category: "typeglob_generated_boundary",
            file: "lib/Dancer2/Plugin.pm",
            expected_tokens: &[
                ExpectedToken { text: "Dancer2::Plugin", allowed_types: &["namespace", "class"] },
                ExpectedToken { text: "import", allowed_types: &["function", "method"] },
                ExpectedToken {
                    text: "Dancer2::Plugin::Base",
                    allowed_types: &["namespace", "class"],
                },
                ExpectedToken { text: "$caller", allowed_types: &["variable", "parameter"] },
            ],
            dynamic_boundary_texts: &[
                "\"${caller}::ISA\"",
...
fn freshness_report(harness: &UxHarness, files: &[FixtureFile]) -> Result<FreshnessReport> {
    ...
    let updated = original.replace("sub redirect", "sub redirect_to");
    if updated == original {
        return Err(anyhow!("freshness fixture must rename redirect to redirect_to"));
    }
    ...
    let before_hits = token_text_hits(&before_tokens, &["redirect"]);
    let after_hits = token_text_hits(&after_tokens, &["redirect", "redirect_to"]);
    let stale_token_absent = !after_tokens.iter().any(|token| token.text == "redirect");
    let fresh_token_present = after_tokens.iter().any(|token| token.text == "redirect_to");
...
#[test]
fn scenario_38_dancer2_semantic_tokens_quality_receipt() {
    run_ux_scenario(
        "dancer2_semantic_tokens_quality",
        SCENARIO_FILE,
        "scenario_38_dancer2_semantic_tokens_quality_receipt",
        UxCiTier::Pr,
        ...
        |recorder| {
            ...
            let receipt = json!({
                ...
                "claim_boundary": "second-project real-workspace semantic-token quality receipt only; no compiler-backed semantic-token live cutover",
            ...
            eprintln!(
                "dancer2_semantic_tokens_quality_receipt={}",
            ...
            recorder.check(
                ...
                categories
                    == BTreeSet::from([
                        "source_backed_app_symbols",
                        "source_backed_dsl_symbols",
