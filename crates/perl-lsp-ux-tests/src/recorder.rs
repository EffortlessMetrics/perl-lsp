...
pub fn write_receipt_to_dir(
    ...
    let stem = receipt.scenario_file.trim_end_matches(".rs").replace(['/', '\\'], "_");
    let sha_short =
        receipt.run_identity.sha.as_deref().and_then(|s| s.get(..8)).unwrap_or("unknown");

    let mut filename =
        format!("{}-{}-{}-{}", receipt.workflow_id, stem, receipt.test_name, sha_short);
    ...
    match result {
        ...
        Ok(Err(ref err)) => {
            let receipt = if let Some(skip) = err.downcast_ref::<UxScenarioSkip>() {
                recorder.finish_skipped(skip)
            } else {
                let class = classify_error(err);
                recorder.finish_fail(class)
            };
            write_wrapper_receipt(&recorder, &receipt, receipt_dir);
        }
...
mod tests {
    ...
    #[test]
    fn run_ux_scenario_err_writes_fail_receipt_to_disk() -> Result<(), Box<dyn std::error::Error>> {
        let tmp = tempfile::tempdir()?;
        let receipt_dir = tmp.path().join("receipts");
