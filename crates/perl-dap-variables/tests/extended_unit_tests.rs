...
fn perl_value_scalar_with_special_chars() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(val, PerlValue::Scalar(ref s) if s == special_str
        && s.contains('\n')
        && s.contains('\t')
        && s.contains('"')));
    Ok(())
...
fn perl_value_integer_negative() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(val.type_name(), "SCALAR");
    assert!(matches!(val, PerlValue::Integer(-999)));
    Ok(())
}
...
fn perl_value_integer_max() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(val, PerlValue::Integer(i64::MAX)));
    Ok(())
}
...
fn perl_value_integer_min() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(val, PerlValue::Integer(i64::MIN)));
    Ok(())
}
...
fn perl_value_hash_empty_key() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(val.child_count(), Some(1));
    assert!(matches!(val, PerlValue::Hash(ref pairs) if pairs[0].0.is_empty()));
    Ok(())
}
...
fn perl_value_object_with_empty_hash() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        val,
        PerlValue::Object { ref class, ref value }
            if class == "Empty::Class"
                && matches!(**value, PerlValue::Hash(ref h) if h.is_empty())
    ));
    Ok(())
}
...
fn perl_value_code_with_name() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(val, PerlValue::Code { ref name } if name.as_deref() == Some("my_function")));
    Ok(())
}
...
fn perl_value_code_qualified_name() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!val.is_expandable());
    assert!(
        matches!(val, PerlValue::Code { ref name } if name.as_deref() == Some("Package::function"))
    );
    Ok(())
}
...
fn perl_value_glob_qualified() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!val.is_expandable());
    assert!(matches!(val, PerlValue::Glob(ref name) if name == "Package::varname"));
    Ok(())
}
...
fn perl_value_regex_complex() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!val.is_expandable());
    assert!(
        matches!(val, PerlValue::Regex(ref pattern) if pattern.contains('[') && pattern.contains('}'))
    );
    Ok(())
}
...
fn perl_value_error_simple() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(val, PerlValue::Error(ref msg) if msg == "Variable not found"));
    Ok(())
}
...
fn perl_value_error_with_context() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!val.is_expandable());
    assert!(matches!(val, PerlValue::Error(ref msg) if msg.contains("line 42")));
    Ok(())
}
