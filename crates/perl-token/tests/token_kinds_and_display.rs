...
fn every_variant_has_exactly_one_category() {
    for kind in all_kinds() {
        let categories = [
            TokenKind::is_keyword(kind),
            TokenKind::is_operator(kind),
            TokenKind::is_delimiter(kind),
            TokenKind::is_literal(kind),
            TokenKind::is_identifier_or_sigil(kind),
            TokenKind::is_special(kind),
        ];
...
fn total_variant_count() {
    ...
    assert_eq!(TokenKind::all().len(), 132, "expected 132 total TokenKind variants");
}
...
#[test]
fn field_is_keyword() {
    assert!(TokenKind::is_keyword(TokenKind::Field));
    assert!(!TokenKind::is_operator(TokenKind::Field));
    assert!(!TokenKind::is_delimiter(TokenKind::Field));
    assert!(!TokenKind::is_literal(TokenKind::Field));
    assert!(!TokenKind::is_identifier_or_sigil(TokenKind::Field));
    assert!(!TokenKind::is_special(TokenKind::Field));
}

#[test]
fn goto_is_keyword() {
    assert!(TokenKind::is_keyword(TokenKind::Goto));
}

#[test]
fn defer_is_keyword() {
    assert!(TokenKind::is_keyword(TokenKind::Defer));
}

#[test]
fn eof_is_special_not_keyword() {
    assert!(TokenKind::is_special(TokenKind::Eof));
    assert!(!TokenKind::is_keyword(TokenKind::Eof));
}

#[test]
fn unknown_is_special_not_literal() {
    assert!(TokenKind::is_special(TokenKind::Unknown));
    assert!(!TokenKind::is_literal(TokenKind::Unknown));
}

#[test]
fn identifier_is_not_keyword() {
    assert!(TokenKind::is_identifier_or_sigil(TokenKind::Identifier));
    assert!(!TokenKind::is_keyword(TokenKind::Identifier));
}

#[test]
fn semicolon_is_delimiter_not_operator() {
    assert!(TokenKind::is_delimiter(TokenKind::Semicolon));
    assert!(!TokenKind::is_operator(TokenKind::Semicolon));
}

#[test]
fn comma_is_delimiter_not_operator() {
    assert!(TokenKind::is_delimiter(TokenKind::Comma));
    assert!(!TokenKind::is_operator(TokenKind::Comma));
}

#[test]
fn data_marker_is_literal() {
    assert!(TokenKind::is_literal(TokenKind::DataMarker));
}

#[test]
fn heredoc_depth_limit_is_literal() {
    assert!(TokenKind::is_literal(TokenKind::HeredocDepthLimit));
}

#[test]
fn unknown_rest_is_literal() {
    assert!(TokenKind::is_literal(TokenKind::UnknownRest));
}
...
fn sigils_classified_correctly() {
    ...
    for sigil in &sigils {
        assert!(
            TokenKind::is_identifier_or_sigil(*sigil),
            "{sigil:?} should be identifier_or_sigil"
        );
        assert!(!TokenKind::is_operator(*sigil), "{sigil:?} should not be classified as operator");
    }
}
...
fn declaration_keywords() {
    ...
    for kind in &decl {
        assert!(TokenKind::is_keyword(*kind), "{kind:?} should be a keyword");
    }
}
...
fn comparison_operators() {
    ...
    for kind in &comps {
        assert!(TokenKind::is_operator(*kind), "{kind:?} should be an operator");
    }
}
