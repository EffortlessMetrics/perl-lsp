...
fn line_starts_cache_rope_crlf_roundtrip() {
    ...
    // Direct over-column regression: character=999 on "hello" (line 0) must
    // clamp to byte 5 (end of content), not walk into \r\n at bytes 5-6.
    // This is the specific bug that was fixed — it would return byte 7 before the fix.
    let off = cache.position_to_offset_rope(&rope, 0, 999);
    assert_eq!(off, 5, "over-column on CRLF line must clamp to end of content, not inside \\r\\n");

    // Over-column on "world\r\n" (line 1): content ends at byte 12, \r\n at 12-13.
    // Before fix: would return 14 (past \r\n). After fix: returns 12.
    let off2 = cache.position_to_offset_rope(&rope, 1, 999);
    assert_eq!(off2, 12, "over-column on second CRLF line must clamp to byte 12");
}
...
#[test]
fn line_starts_cache_rope_crlf_only_line() {
    // A line whose entire content is just \r\n (no text characters).
    // Over-large column must clamp to byte 0 (start of line), not walk into \r\n.
    let text = "\r\nab";
    let rope = ropey::Rope::from_str(text);
    let cache = LineStartsCache::new_rope(&rope);

    // Line 0 is empty (no content before \r\n). Any column must clamp to byte 0.
    let off = cache.position_to_offset_rope(&rope, 0, 0);
    assert_eq!(off, 0, "col 0 on empty CRLF line should be byte 0");
    let off_over = cache.position_to_offset_rope(&rope, 0, 100);
    assert_eq!(off_over, 0, "over-column on empty CRLF line must not walk into \\r\\n");

    // Line 1 starts at byte 2 and has "ab".
    let off_a = cache.position_to_offset_rope(&rope, 1, 0);
    assert_eq!(off_a, 2);
    let off_b = cache.position_to_offset_rope(&rope, 1, 1);
    assert_eq!(off_b, 3);
}

#[test]
fn line_starts_cache_rope_crlf_unicode_before_terminator() {
    // Unicode char (3-byte UTF-8, 1 UTF-16 unit) followed by CRLF.
    // Over-large column must stop at the byte after the Unicode char, not enter \r\n.
    // 'あ' = U+3042, UTF-8: 0xE3 0x81 0x82 (3 bytes), len_utf16 = 1
    let text = "あ\r\nz";
    let rope = ropey::Rope::from_str(text);
    let cache = LineStartsCache::new_rope(&rope);

    // Line 0: "あ" — content ends at byte 3. \r is at byte 3, \n at byte 4.
    let off_exact = cache.position_to_offset_rope(&rope, 0, 1);
    assert_eq!(off_exact, 3, "col 1 after あ should land at byte 3 (end of content)");

    let off_over = cache.position_to_offset_rope(&rope, 0, 999);
    assert_eq!(
        off_over, 3,
        "over-column after Unicode+CRLF must clamp to byte 3, not enter \\r\\n"
    );

    // Supplementary plane char (4-byte UTF-8, 2 UTF-16 units) before CRLF.
    // '𠀋' = U+2000B, UTF-8: 4 bytes, len_utf16 = 2
    let text2 = "𠀋\r\nz";
    let rope2 = ropey::Rope::from_str(text2);
    let cache2 = LineStartsCache::new_rope(&rope2);

    // col=2 is exact end of content (after the surrogate pair)
    let off2_exact = cache2.position_to_offset_rope(&rope2, 0, 2);
    assert_eq!(off2_exact, 4, "col 2 after surrogate pair should land at byte 4");

    // col=3 is one past the UTF-16 content — must clamp to byte 4, not enter \r
    let off2_over = cache2.position_to_offset_rope(&rope2, 0, 3);
    assert_eq!(off2_over, 4, "over-column after surrogate pair+CRLF must clamp to byte 4");
}
