...
fn test_initialize_includes_warn_filter() -> TestResult {
    ...
    if perl_dap::feature_catalog::has_feature("dap.exceptions.warn") {
        assert!(
            has_warn,
            "warn filter should be advertised when feature is enabled"
        );
        ...
        assert!(
            !has_warn,
            "warn filter should not be advertised when feature is disabled"
        );
    }
...
fn test_warn_filter_in_feature_catalog() {
    ...
    assert!(
        has_feature,
        "dap.exceptions.warn should be a registered feature"
    );
}
