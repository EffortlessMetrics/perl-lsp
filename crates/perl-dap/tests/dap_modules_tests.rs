...
fn test_modules_with_default_arguments() -> TestResult {
    ...
    assert!(
        modules.is_empty(),
        "Expected empty modules without active session"
    );
...
fn test_modules_response_structure() -> TestResult {
    ...
    assert!(
        ...
        "Response must have 'modules' field"
    );
    assert!(
        body.get("modules").and_then(|v| v.as_array()).is_some(),
        "modules must be an array"
    );
    assert!(
        body.get("totalModules").is_some(),
        "Response must have 'totalModules' field"
    );
    assert!(
