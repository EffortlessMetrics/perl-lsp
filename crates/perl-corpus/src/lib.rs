...
static META_RE: once_cell::sync::Lazy<Regex> =
    once_cell::sync::Lazy::new(|| Regex::new(r"(?m)^#\s*@(?P<k>id|tags|perl|flags):\s*(?P<v>.*)$").unwrap());
...
pub fn parse_file(path: &Path) -> Result<Vec<Section>> {
    ...
    for w in offs.windows(2) {
        ...
        let flags = meta.get("flags").map(|s| {
            s.replace(',', " ")
             .split_whitespace()
             .map(|t| t.to_string())
             .collect::<Vec<_>>()
