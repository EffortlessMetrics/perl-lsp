...
fn is_known_function(name: &str) -> bool {
    ...
    // Check PHF map for built-ins first (O(1) lookup)
    if builtin_signatures_phf::is_builtin(name) {
        return true;
    }

    // Fallback for keywords and operators not in the built-in signatures map
    match name {
        // Quote-like operators
        "q" | "qq" | "qr" | "qw" | "qx" | "tr" | "y" |
        // Keywords
        "package" | "sub" | "import" | "no" |
        // Control flow (some might be missing from builtins)
        "break" | "continue" |
        // Switch (given/when)
        "given" | "when" | "default" => true,
        _ => false,
    }
}
