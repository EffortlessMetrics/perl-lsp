...
fn server_info_name_is_perl_lsp() {
    ...
    if let Some(d) = data {
        ...
        assert_eq!(
            name,
            Some("perl-lsp"),
            "server_info.name should be 'perl-lsp'"
        );
    }
}
...
fn server_info_version_is_semver_shaped() {
    ...
    if let Some(d) = &err.data {
        ...
        if let Some(v) = version {
            ...
            assert!(
                parts.len() >= 2,
                "version '{v}' should have at least major.minor components"
            );
            for part in &parts {
                assert!(
...
fn text_document_sync_is_full() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        change,
        Some(1),
        "change should be TextDocumentSyncKind::FULL (1)"
    );
    Ok(())
}
...
fn text_document_sync_has_open_close_true() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let open_close = v
        .pointer("/textDocumentSync/openClose")
...
fn completion_trigger_characters_include_sigils() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ...
        "should trigger on scalar sigil $"
    );
    assert!(
        trigger_strs.contains(&"@"),
        "should trigger on array sigil @"
    );
    assert!(
        trigger_strs.contains(&"%"),
        "should trigger on hash sigil %"
    );
    // LSP spec requires single-char trigger characters; ">" replaces the non-conforming "->"
    assert!(
        trigger_strs.contains(&">"),
        "should trigger on > for -> (LSP spec: single char)"
    );
    // ":" triggers :: package member completion
    assert!(
        trigger_strs.contains(&":"),
        "should trigger on : for :: package member completion"
    );
...
fn completion_has_resolve_provider_true() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        resolve,
        Some(true),
        "completionProvider.resolveProvider should be true"
    );
    Ok(())
}
...
fn signature_help_retrigger_characters() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ...
        "@ (array variable) retrigger"
    );
    assert!(retrigger_strs.contains(&"%"), "% (hash variable) retrigger");
    assert!(
        retrigger_strs.contains(&"{"),
        "brace (hash/block subscript) retrigger"
    );
    assert!(
        retrigger_strs.contains(&"["),
        "bracket (array subscript) retrigger"
    );
    // Closing brackets intentionally excluded — they signal end of context, not a new parameter
    assert!(
        !retrigger_strs.contains(&"}"),
        "closing brace should not retrigger"
    );
    assert!(
        !retrigger_strs.contains(&"]"),
        "closing bracket should not retrigger"
    );
    Ok(())
}
...
fn rename_provider_has_prepare_provider() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        prepare,
        Some(true),
        "renameProvider.prepareProvider should be true"
    );
    Ok(())
}
...
fn inlay_hint_has_resolve_provider() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let resolve = v
        .pointer("/inlayHintProvider/resolveProvider")
        .and_then(|v| v.as_bool());
    assert_eq!(
        resolve,
        Some(true),
        "inlayHintProvider.resolveProvider should be true"
    );
    Ok(())
}
...
fn pull_diagnostics_inter_file_dependencies_false() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let inter_file = v
        .pointer("/diagnosticProvider/interFileDependencies")
        .and_then(|v| v.as_bool());
    assert_eq!(
        inter_file,
        Some(false),
...
fn pull_diagnostics_identifier_is_perl_lsp() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let identifier = v
        .pointer("/diagnosticProvider/identifier")
        .and_then(|v| v.as_str());
    assert_eq!(
        identifier,
        Some("perl-lsp"),
        "diagnosticProvider.identifier should be 'perl-lsp'"
    );
    Ok(())
}
...
fn semantic_tokens_has_full_and_range_support() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let range = v
        .pointer("/semanticTokensProvider/range")
        .and_then(|v| v.as_bool());
    assert_eq!(
        range,
        Some(true),
        "semanticTokensProvider should support range"
    );
    Ok(())
}
...
fn semantic_tokens_legend_has_token_types() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        type_strs.contains(&"function"),
        "should include 'function' token type"
    );
    assert!(
        type_strs.contains(&"variable"),
        "should include 'variable' token type"
    );
    assert!(
        type_strs.contains(&"keyword"),
        "should include 'keyword' token type"
    );
    assert!(
        type_strs.contains(&"string"),
        "should include 'string' token type"
    );
    assert!(
        type_strs.contains(&"number"),
        "should include 'number' token type"
    );
    assert!(
        type_strs.contains(&"regexp"),
        "should include 'regexp' token type"
    );
    assert!(
        type_strs.contains(&"comment"),
        "should include 'comment' token type"
    );
    assert!(
        type_strs.contains(&"namespace"),
        "should include 'namespace' token type"
    );
...
fn semantic_tokens_legend_has_token_modifiers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        modifiers.len(),
        13,
        "expected 13 token modifiers; got {:?}",
        modifiers
    );
    let mod_strs: Vec<&str> = modifiers.iter().filter_map(|t| t.as_str()).collect();
    assert!(
        mod_strs.contains(&"declaration"),
        "should include 'declaration' modifier"
    );
    assert!(
        mod_strs.contains(&"definition"),
        "should include 'definition' modifier"
    );
    assert!(
        mod_strs.contains(&"readonly"),
        "should include 'readonly' modifier"
    );
    assert!(
        mod_strs.contains(&"deprecated"),
        "should include 'deprecated' modifier"
    );
    ...
    assert!(
        mod_strs.contains(&"scalarVariable"),
        "should include 'scalarVariable' modifier"
    );
    assert!(
        mod_strs.contains(&"arrayVariable"),
        "should include 'arrayVariable' modifier"
    );
    assert!(
        mod_strs.contains(&"hashVariable"),
        "should include 'hashVariable' modifier"
    );
    ...
    assert_eq!(
        scalar_idx, 10,
        "scalarVariable must be at index 10 (bitmask 1024)"
    );
    Ok(())
}
...
fn code_action_always_includes_quickfix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        kind_strs.contains(&"quickfix"),
        "code action kinds should include 'quickfix'"
    );
    Ok(())
}
...
fn code_action_has_resolve_provider() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        resolve,
        Some(true),
        "codeActionProvider.resolveProvider should be true"
    );
    Ok(())
}
...
fn code_action_source_organize_imports_gated_by_flag() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let flags_without = BuildFlags {
        code_actions: true,
        source_organize_imports: false,
        ..Default::default()
    };
...
fn on_type_formatting_more_triggers_include_semicolon() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        more_strs.contains(&";"),
        "moreTriggerCharacter should include ';'"
    );
    Ok(())
}
...
fn code_lens_has_resolve_provider() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let resolve = v
        .pointer("/codeLensProvider/resolveProvider")
        .and_then(|v| v.as_bool());
    assert_eq!(
        resolve,
        Some(true),
        "codeLensProvider.resolveProvider should be true"
    );
    Ok(())
}
...
fn document_link_has_resolve_provider() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let resolve = v
        .pointer("/documentLinkProvider/resolveProvider")
        .and_then(|v| v.as_bool());
    assert_eq!(
        resolve,
        Some(true),
        "documentLinkProvider.resolveProvider should be true"
    );
    Ok(())
}
...
fn notebook_sync_targets_jupyter_perl_cells() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        !selectors.is_empty(),
        "notebookSelector should not be empty"
    );
    ...
    if let Some(cells) = cells {
        let has_perl = cells
            .iter()
            .any(|c| c.get("language").and_then(|l| l.as_str()) == Some("perl"));
...
fn notebook_sync_save_is_true() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let v = serde_json::to_value(&caps)?;
    let save = v
        .pointer("/notebookDocumentSync/save")
        .and_then(|v| v.as_bool());
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let provider = v
        ...
        .ok_or("missing workspaceSymbolProvider")?;
...
fn inline_completion_advertised_in_experimental_object() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        inline.is_some(),
        "experimental.inlineCompletionProvider should be present"
    );
    Ok(())
}
...
fn inline_completion_disabled_has_no_experimental_key() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        inline.is_none(),
        "should not have inlineCompletionProvider when flag is off"
    );
    Ok(())
}
...
fn capabilities_json_production_serializes_to_valid_object() {
    ...
    assert!(
        v.is_object(),
        "capabilities_json should return a JSON object"
    );
}
...
fn capabilities_json_production_has_expected_top_level_keys() {
    ...
    if let Some(o) = obj {
        assert!(
            o.contains_key("textDocumentSync"),
            "missing textDocumentSync"
        );
        assert!(o.contains_key("hoverProvider"), "missing hoverProvider");
        assert!(
            o.contains_key("completionProvider"),
            "missing completionProvider"
        );
        assert!(
            o.contains_key("definitionProvider"),
            "missing definitionProvider"
        );
        assert!(
            o.contains_key("referencesProvider"),
            "missing referencesProvider"
        );
        assert!(
            o.contains_key("documentSymbolProvider"),
...
fn capabilities_json_default_is_minimal() {
    ...
    if let Some(o) = obj {
        ...
        assert!(
            !o.contains_key("hoverProvider"),
            "default should not have hoverProvider"
        );
        assert!(
            !o.contains_key("completionProvider"),
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        v, reparsed,
        "roundtrip serialization should preserve structure"
    );
    Ok(())
}
...
fn ga_lock_profile_excludes_inline_values() {
    let caps = capabilities_for(BuildFlags::ga_lock());
    assert!(
        caps.inline_value_provider.is_none(),
        "ga-lock should exclude inline values"
    );
}
...
fn all_profile_enables_all_providers() {
    ...
    assert!(
        ...
        "all: document_link_provider"
    );
    assert!(
        caps.selection_range_provider.is_some(),
        "all: selection_range_provider"
    );
    assert!(
    ...
    assert!(
        ...
        "all: linked_editing_range_provider"
    );
    assert!(
        caps.inline_value_provider.is_some(),
        "all: inline_value_provider"
    );
    ...
    assert!(
        ...
        "all: call_hierarchy_provider"
    );
    assert!(
        caps.folding_range_provider.is_some(),
        "all: folding_range_provider"
    );
    assert!(
        caps.document_formatting_provider.is_some(),
        "all: document_formatting_provider"
    );
    assert!(
        caps.document_range_formatting_provider.is_some(),
    ...
    assert!(
        caps.notebook_document_sync.is_some(),
        "all: notebook_document_sync"
    );
}
...
fn default_profile_enables_only_text_sync() {
    ...
    assert!(
        ...
        "default must have textDocumentSync"
    );
    // Everything else off
...
fn supported_commands_have_expected_set() {
    ...
    for exp in &expected {
        assert!(
            cmds.contains(&exp.to_string()),
            "supported commands should include '{exp}'"
        );
    }
    assert_eq!(
        cmds.len(),
        expected.len(),
        "supported commands count should match expected"
    );
}
...
fn enabling_hover_only_produces_hover_and_sync() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        present.contains(&&"textDocumentSync".to_string()),
        "should have textDocumentSync"
    );
    assert!(
        present.contains(&&"hoverProvider".to_string()),
        "should have hoverProvider"
    );
