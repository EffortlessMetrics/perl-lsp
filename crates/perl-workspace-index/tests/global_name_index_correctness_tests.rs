...
"#;
    ...
    assert!(!results.is_empty(), "search_symbols should find 'Calc::Arithmetic::add'");
...
"#;
    ...
    assert!(!by_qualified.is_empty(), "search_symbols('File::Find::find') should find the sub");
...
fn test_search_symbols_returns_vec_workspace_symbol() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!results.is_empty(), "search_symbols should return non-empty Vec for existing symbol");
...
fn test_search_symbols_signature_unchanged() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Note: empty query actually returns all symbols (empty string is substring of everything)
