...
pub use perl_workspace_index_lifecycle::{
    ...
    IndexPhaseTransition, IndexResourceLimits, IndexState, IndexStateKind, IndexStateTransition,
    ResourceKind,
};
...
impl IndexCoordinator {
    ...
    pub fn notify_change(&self, _uri: &str) {
        let pending = self.metrics.increment_pending();

        // Check for parse storm
        if self.metrics.exceeds_parse_storm_threshold(pending) {
    ...
    pub fn notify_parse_complete(&self, _uri: &str) {
        ...
        let pending = self.metrics.decrement_pending();
        if pending == 0 {
