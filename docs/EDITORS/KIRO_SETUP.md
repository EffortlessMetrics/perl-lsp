...
- **Kiro IDE** — use the VS Code-compatible `EffortlessMetrics.perl-lsp-rs`
  extension from OpenVSX.
- **Kiro CLI** — use Kiro CLI's workspace-scoped custom LSP configuration to
...
`perllsp` manually only for offline environments, pinned internal deployments,
or when `perl-lsp.autoDownload` is disabled.
...
- If your organization uses a custom extension registry, confirm it mirrors
  `EffortlessMetrics.perl-lsp-rs`.
- If extension download is blocked by firewall or proxy policy, allow OpenVSX
  extension metadata and VSIX download hosts or install from an approved
  internal registry.
