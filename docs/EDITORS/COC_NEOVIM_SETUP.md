...
Download the archive for your platform from [GitHub Releases](https://github.com/EffortlessMetrics/perl-lsp/releases),
extract it, and put the `perllsp` binary on your `PATH`.
...
an extension that provides Perl support or configure a language server in
...
coc.nvim uses Vim/Neovim filetypes. The server starts only when the active
buffer has `filetype=perl`.
...
for coc.nvim-specific behavior only.
...
Keep first-run config small, then add UI settings after the server attaches:
