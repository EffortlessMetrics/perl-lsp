...
If your project only needs built-in include paths, you can omit
`include_paths`. The defaults are `lib`, `.`, and `local/lib/perl5`.
...
Confirm the `perl_lsp` MCP server is listed and that Perl LSP bridge tools are
available.
...
- Confirm the project has a root marker such as `.perl-lsp.toml`,
  `Makefile.PL`, `Build.PL`, `cpanfile`, `dist.ini`, or `.git`.
...
once the first request finishes. If the project is large, tune
