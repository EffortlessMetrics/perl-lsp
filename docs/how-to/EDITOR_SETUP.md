...
| Kilo Code | install the Open VSX `EffortlessMetrics.perl-lsp-rs` extension | [docs/EDITORS/VS_CODE_SETUP.md](../EDITORS/VS_CODE_SETUP.md) |
...
Kilo Code is VS Code-compatible, so the same `perl-lsp-rs` extension works.
Install `EffortlessMetrics.perl-lsp-rs` from Open VSX, then use the same
workspace settings shown in the VS Code section.
