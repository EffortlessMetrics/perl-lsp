...
| Trae (ByteDance) | use VS Code-compatible extension flow or a generic `perllsp --stdio` client | [docs/EDITORS/TRAE_SETUP.md](../EDITORS/TRAE_SETUP.md) |
...
Trae uses a VS Code-compatible settings model. Use the same server command:
`perllsp --stdio`. If marketplace access is available, install the
`EffortlessMetrics.perl-lsp-rs` extension; otherwise configure a generic Perl
language-server client. See [docs/EDITORS/TRAE_SETUP.md](../EDITORS/TRAE_SETUP.md).
