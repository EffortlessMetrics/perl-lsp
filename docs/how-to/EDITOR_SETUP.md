...
| Codex app | use `perllsp --check` / `--check-project` through Codex terminal or project actions; Codex app is not a documented generic LSP client | [docs/EDITORS/CODEX_APP_SETUP.md](../EDITORS/CODEX_APP_SETUP.md) |
| Codex IDE Extension | use Codex inside a VS Code-compatible editor alongside the `EffortlessMetrics.perl-lsp-rs` extension for live Perl LSP features | [docs/EDITORS/VS_CODE_SETUP.md](../EDITORS/VS_CODE_SETUP.md) |
