...
| Trae | install the VS Code-compatible `EffortlessMetrics.perl-lsp-rs` extension; use `perl-lsp.serverPath` only for manual or offline `perllsp` deployments | [docs/EDITORS/TRAE_SETUP.md](../EDITORS/TRAE_SETUP.md) |
...
If the official extension is unavailable, install a generic LSP client extension
first, then configure that extension to launch `perllsp --stdio`.
