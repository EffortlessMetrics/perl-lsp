...
| R-1   | [#8565](https://github.com/EffortlessMetrics/perl-lsp/issues/8565) | `policy/rust-1.95-rustc-floor`                  | `policy(rust): tighten workspace rustc lint floor`               | Promote `unexpected_cfgs` to `warn`, add `unused_must_use = "deny"` (verify it's not already), survey other 1.95 stabilizations worth denying |
