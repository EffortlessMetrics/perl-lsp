...
| `{{test_passed}}` | `.tests.passed` | 828 |
| `{{test_failed}}` | `.tests.failed` | 3 |
| `{{test_ignored}}` | `.tests.ignored` | 818 |
| `{{pass_rate_active}}` | `.tests.pass_rate_active` | 99.6 |
| `{{missing_docs}}` | `.docs.missing_docs` | 484 |
...
✅ **Production Ready**:
- {{pass_rate_active}}% test pass rate across all components
- {{test_passed}} passing, {{test_failed}} failing, {{test_ignored}} ignored
- Total test suite: {{test_total}} tests ({{test_active}} active + {{test_ignored}} quarantined)
- {{missing_docs}} missing documentation warnings tracked for systematic resolution
```
