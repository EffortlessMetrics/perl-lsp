...
1. **Gold corpus directory collision (MEDIUM)** — scouts #4065 and #4066 each proposed a different directory, both wrong. Correct directory: `crates/perl-lsp/tests/fixtures/gold/`. Mitigation: the synthesis comment is the canonical spec; builders read it before writing.
...
2. **Cross-crate neutrality.** `test_corpus/` is project-wide and already consumed by multiple crates. `crates/perl-lsp/tests/fixtures/` would lock the gold fixtures to the `perl-lsp` crate — forcing #4067 (module resolution) and #4065 (diagnostics), neither of which live in `perl-lsp`, to reach across crate boundaries.
...
**Third reality check (umbrella plan-reviewer synthesis, [#4062](https://github.com/EffortlessMetrics/perl-lsp/issues/4062)):** The umbrella synthesis confirms the 7×3 shape, anchors the fixture directory at `crates/perl-lsp/tests/fixtures/gold/` (same location as the B + C gold corpus, enabling fixture sharing), and adds Risk 5 to the risk register: "verify that `with_file('lib/MyModule.pm', content)` creates the file at the correct relative path within the temp workspace root. If the harness flattens paths, `use lib 'lib'` resolution will fail silently." That is the kind of risk a scout could not flag — it depends on an implementation detail of `UxHarness` in `crates/perl-lsp-ux-tests/src/lib.rs:143` that becomes relevant only when you think through how the fixtures will be loaded.
...
- [#4065](https://github.com/EffortlessMetrics/perl-lsp/issues/4065) — Diagnostics scorecard. **Wave 2, gated on gold-corpus format.** Plan-reviewer produced the canonical `.gold.json` schema spec; directory is `crates/perl-lsp/tests/fixtures/gold/`.
