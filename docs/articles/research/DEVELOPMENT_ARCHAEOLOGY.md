...
- **Agent Surface**: 54 agent definitions in `.claude/agents/`, with older lineage directories retained alongside them
...
1. **54 agent definitions plus lineage directories**: `.claude/agents4/` preserves the canonical Q3 swarm pack, `agents5-6` capture the transition, and `.claude/agents/` is effectively `agents7`
