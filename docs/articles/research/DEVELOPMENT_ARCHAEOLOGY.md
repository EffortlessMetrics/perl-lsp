...
- **Agent Surface**: 52 agent definitions plus 2 roster JSON files in `.claude/agents/`, with older lineage directories retained alongside them
...
1. **52 agent definitions plus 2 roster JSON files and lineage directories**: `.claude/agents4/` preserves the canonical Q3 swarm pack, `agents5-6` capture the transition, and `.claude/agents/` is effectively `agents7`
