...
| Apply-edit metadata | `workspace.applyEdit`, `workspace.workspaceEdit.metadataSupport`, `workspace/applyEdit` | Server-originated refactoring apply-edit requests may include deterministic `ApplyWorkspaceEditParams.metadata.label`, `description`, and `isRefactoring` only when both capabilities are present; ordinary `WorkspaceEdit` responses stay metadata-free. | `lsp_318_negative_claims`, `features.toml`, `check-lsp-318-claims` |
