...
- ✅ `perl.workspaceTrustReport` - Return a read-only workspace trust report from current server state, including workspace roots, module-resolution configuration, advisory setup hints, subprocess/perldoc/DAP probe boundaries, index status, support tiers, provider-decision trace keys, and the report claim boundary. It does not resolve Perl, run perldoc, inspect DAP sessions, scan files, probe Perl, refresh parser receipts, or promote provider support tiers.
