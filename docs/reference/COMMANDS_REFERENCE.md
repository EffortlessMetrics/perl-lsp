...
- ✅ `perl.workspaceTrustReport` - Return a read-only workspace trust report from current server state, including workspace roots, module-resolution configuration, index status, support tiers, provider-decision trace keys, and the report claim boundary. It does not scan files, probe Perl, refresh parser receipts, or promote provider support tiers.
