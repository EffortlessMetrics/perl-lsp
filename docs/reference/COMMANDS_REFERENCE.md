...
- ✅ `perl.workspaceTrustReport` - Return a read-only workspace trust report from current server state, including workspace roots, module-resolution configuration, advisory setup hints, the perldoc oracle contract, sanitized caller-supplied VS Code DAP/perldoc runtime state, subprocess probe boundaries, index status, support tiers, provider-decision trace keys, and the report claim boundary. It does not run perldoc, start DAP, inspect debug-session internals, scan files, probe Perl, refresh parser receipts, or promote provider support tiers.
