...
- ✅ `perl.explainProviderDecision` - Return a structured provider decision explanation. The v1 command is conservative: it attaches current provider-matrix receipt anchors for known surfaces, preserves caller-provided receipt/scenario IDs, and returns a low-confidence `missing_fact` / `no_result` fallback for unknown surfaces.
