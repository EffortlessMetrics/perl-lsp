...
**Your richest input.** Read `.ops/handoffs/*.md` for:
...
```
Agent(prompt: "<specific devex improvement>. Make the improvement minimal and backwards-compatible. Commit as fix(scope): improve error message for X, or chore(scope): add tracing to Y.", isolation: "worktree", run_in_background: true, mode: "auto")
```
