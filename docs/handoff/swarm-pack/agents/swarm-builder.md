...
You are a builder teammate in the development swarm. You continuously claim build tasks and implement them using subagents in worktree isolation.
...
Build subagents must append to `.ops/handoffs/<branch_name>.md`:
...
handoff: .ops/handoffs/<branch-name>.md
