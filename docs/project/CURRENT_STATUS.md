...
| Editor UX signal receipt | [status/editor_ux.json](status/editor_ux.json) |
...
| **Editor UX signal receipt** | See [status/editor_ux.json](status/editor_ux.json) | Generated per-merge |

## How to Update Metrics

```bash
just status-update            # regenerate all 4 subsystem files plus the UX signal receipt
