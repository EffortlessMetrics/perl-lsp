...
**Expected output**: A section heading like `## [0.12.3] - YYYY-MM-DD` followed by at least one change entry under `### Added`, `### Fixed`, or `### Changed`.

**If it fails**: If only `[Unreleased]` exists, promote it:
```bash
# Edit CHANGELOG.md: rename ## [Unreleased] -> ## [0.12.3] - $(date +%F)
# Add a new empty ## [Unreleased] section above it.
git add CHANGELOG.md
git commit -m "chore: finalize CHANGELOG for v0.12.3"
# Push and merge before proceeding.
```
If the entry was generated by another agent (as noted in the spec), verify it references the correct PR numbers and does not contain placeholder text.
...
**Why**: The workspace has 130+ crates. Manual version bumping across all `Cargo.toml` files is error-prone. The `version-bump.yml` workflow uses `cargo-release` to apply the bump atomically across the workspace and opens a PR automatically.

**There is no `just bump-version` recipe.** The canonical path is the workflow dispatch. Alternatively, `cargo xtask release-turnkey 0.12.3` can be used locally (see `just release-turnkey`), which also opens a PR.

**Command** (option A — workflow dispatch via gh CLI):
```bash
...
**Command** (option B — local xtask, opens PR):
```bash
export CARGO_TARGET_DIR="/tmp/release-0.12.3-target"
just release-turnkey 0.12.3
```

**Expected output**: A PR is opened on branch `release/v0.12.3` with title "Release v0.12.3". The PR modifies `Cargo.toml` (workspace version), `CHANGELOG.md`, and potentially `vscode-extension/package.json`.

**If it fails**: Check for existing `release/v0.12.3` branch (delete it first if stale). If `cargo-release` is not installed locally for option B, use option A. If the workflow fails to resolve `cargo-release` from GitHub releases, check network access in CI.
