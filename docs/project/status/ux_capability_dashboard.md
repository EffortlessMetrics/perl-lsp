...
| Method completion | `semantic-shadow` | [semantic_capability_dashboard.md](semantic_capability_dashboard.md) `method_completion_shadow_or_cutover_status` (guarded cutover; 0 regressions, 0 unavailable receipts); [#7901](https://github.com/EffortlessMetrics/perl-lsp/pull/7901) literal `bless` receiver inference; [#7917](https://github.com/EffortlessMetrics/perl-lsp/pull/7917) typed receiver-evidence provenance; [#7920](https://github.com/EffortlessMetrics/perl-lsp/pull/7920) receiver-evidence detail text | Semantic method candidates surface own / inherited / generated context when they fully cover the legacy method set. Receiver evidence now includes static package calls (`Foo->method`), `$self->` / `$this->`, `Package->new` constructor assignments, type-engine inferred receivers, and literal `bless ..., "Package"` forms. Method-completion `detail` text now explains the receiver-evidence source — e.g. `receiver: static package`, `receiver: self/this`, `receiver: constructor assignment`, `receiver: literal bless`, `receiver: type engine`. | No explicit confidence labels are shown yet. Unknown receivers still do not get broad fallback completions. Dynamic / non-literal receiver evidence (`bless {}, $class`, expression-tail forms, nested calls, non-builtin `bless`-prefixed identifiers) remains fail-closed. #7920 did not change candidate inclusion or ranking. | Evaluate explicit confidence labels before reopening unknown-receiver fallback policy |
...
Evaluate whether explicit confidence labels improve method-completion detail text:

- `$self->` / `$this->` — current package evidence, high confidence
- `Foo->new` assignment — constructor / type-engine evidence, high confidence or engine-derived confidence
...
- dynamic receiver — low confidence; never creates exact receiver evidence

Unknown-receiver fallback remains a separate policy issue because it changes candidate inclusion.
