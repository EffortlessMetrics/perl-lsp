...
| Method completion | `semantic-shadow` | [semantic_capability_dashboard.md](semantic_capability_dashboard.md) `method_completion_shadow_or_cutover_status` (guarded cutover; 0 regressions, 0 unavailable receipts); [#7901](https://github.com/EffortlessMetrics/perl-lsp/pull/7901) literal `bless` receiver inference; [#7917](https://github.com/EffortlessMetrics/perl-lsp/pull/7917) typed receiver-evidence provenance; [#7920](https://github.com/EffortlessMetrics/perl-lsp/pull/7920) receiver-evidence detail text; [#7926](https://github.com/EffortlessMetrics/perl-lsp/pull/7926) medium-confidence receiver detail labels | Semantic method candidates surface own / inherited / generated context when they fully cover the legacy method set. Receiver evidence now includes static package calls (`Foo->method`), `$self->` / `$this->`, `Package->new` constructor assignments, type-engine inferred receivers, and literal `bless ..., "Package"` forms. Method-completion `detail` text now explains the receiver-evidence source — e.g. `receiver: static package`, `receiver: self/this`, `receiver: constructor assignment`. Medium-confidence evidence (literal `bless`, type-engine inference) carries an explicit `medium confidence` label — e.g. `receiver: literal bless, medium confidence`, `receiver: type engine, medium confidence`. | High-confidence evidence remains unlabelled by design (the common case stays clean). Unknown receivers still do not get broad fallback completions. Dynamic / non-literal receiver evidence (`bless {}, $class`, expression-tail forms, nested calls, non-builtin `bless`-prefixed identifiers) remains fail-closed. #7926 did not change candidate inclusion or ranking. | Design unknown-receiver fallback policy separately |
...
Decide whether to design low-confidence fallback for unknown method receivers.

Unknown-receiver fallback is a separate policy issue because it changes candidate inclusion:

- unknown receiver — currently keeps existing conservative behavior and does not invent broad candidates
- fallback candidates would need explicit low-confidence treatment
- dynamic receiver evidence must never create exact receiver evidence
- fallback must not outrank exact receiver evidence
- candidate inclusion, sort order, and UI wording would all need review together

This is larger than detail text because it changes what the editor offers, not just how it explains an existing offer.
