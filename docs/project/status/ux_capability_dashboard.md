...
Move references through the same narrow navigation gate now used by goto
definition.

The first live reference slice should remain small:

- exact/static references only
- fresh, high-confidence, source-backed occurrences only
- generated, stale, low-confidence, ambiguous, and dynamic-boundary occurrences
  keep legacy fallback or shadow-only receipts
- imported/exported reference occurrences follow only after exact/static
  reference quality is proven
