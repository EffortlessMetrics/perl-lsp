...
| Rename | `semantic-shadow` | [semantic_scorecard.md](semantic_scorecard.md) `rename_plan = 100% pass`, `rename_unsafe_edit_count = 0`; [semantic_shadow_compare.md](semantic_shadow_compare.md) schema-fixture `RenamePlan` receipts trace exact static edits, dynamic-boundary blockers, stale compiler facts, and low-confidence ambiguity; runtime blocker UX tests compare live rename with compiler plans without changing live behavior; [Mojolicious rename unsafe-edit receipt](../../../crates/perl-lsp-ux-tests/tests/ux_scenario_35_mojolicious_rename_unsafe_edit.rs) records project-shaped rename safety | Rename planning is fixture-backed, currently produces no unsafe edits, records fact-source traces proving unsafe compiler facts block rather than authorize edits, and now attaches blocker reason / UX notes for dynamic, generated, stale-fact, and low-confidence blockers. Runtime receipts keep exact static, dynamic-boundary, generated-member, stale-fact, low-confidence, and Mojolicious exact-local/generated/dynamic/freshness rename probes traceable against the live provider result. | Live rename still uses the existing provider path. One Mojolicious unsafe-edit receipt is not promoted into broad live compiler-backed rename behavior. | Prove a narrow live rename cutover only after additional real-workspace unsafe-edit receipts and rollback behavior are documented |
