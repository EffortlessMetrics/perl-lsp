...
| **CPAN top 1000** | 85.4% clean (`3717/4355`) | Ecosystem breadth baseline; `4` unreadable, `634` with errors, cached downloads in `target/cpan-corpus/.cpanm`, baseline `2026-03-20` | `.ci/cpan-corpus-baseline.json` |
| **Project corpus** | 100.0% clean (`91/91`) | Deterministic regression baseline; `69` `test_corpus/` + `22` `perl-corpus` files, `0` errors, `0` timeouts, `0` panics, `64/68` NodeKinds, `12/12` GA features | `test_corpus/` + `crates/perl-corpus/src/gen` |
