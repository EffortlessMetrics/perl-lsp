...
| Safe delete | Exact static source-backed subroutine | `source-backed pilot + current-source reference guard` | Additional cross-project referenced-source blocker receipts | Compiler plan is allowed, target is fresh high-confidence source-backed subroutine, exact references are zero in compiler facts and current source, and rollback proof restores original text | `perl.previewSafeDelete` returns no-edit explanation when live delete proof is incomplete | Imported/exported symbol, generated member, non-subroutine target, no-source target, current-source reference, typeglob alias, AUTOLOAD, symbolic ref, dynamic require, stale fact, low confidence, fallback fact, or rollback failure |
