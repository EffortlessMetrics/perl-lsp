...
| Safe delete | Exact static source-backed subroutine | `source-backed pilot + current-source and identity guards` | Cross-project referenced-source blocker receipts plus non-subroutine, package-wide, generated/no-source, and dynamic blocker receipts | Compiler plan is allowed, target is fresh high-confidence source-backed subroutine, exact references are zero in compiler facts and current source, workspace identity guard accepts the request, and rollback proof restores original text | `perl.previewSafeDelete` returns no-edit explanation when live delete proof is incomplete | Imported/exported symbol, generated member, non-subroutine target, no-source target, current-source reference, cross-project reference, ambiguous workspace identity, typeglob alias, AUTOLOAD, symbolic ref, dynamic require, stale fact, low confidence, fallback fact, or rollback failure |
