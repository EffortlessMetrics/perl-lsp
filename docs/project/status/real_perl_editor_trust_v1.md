...
1. `test(workspace-symbols): add generated-symbol expansion rank/noise receipt`
2. `docs(status): review package-local rename support before broader promotion`
3. `test(semantic-tokens): prove method-declaration compiler-token live-output parity before cutover`
