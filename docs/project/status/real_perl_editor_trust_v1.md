...
The `$c->stash` method-call and `field $name` declaration receipts prove
source-backed compiler spans can align with existing live `method` or
`variable` tokens, but method-call and field-declaration remain receipt-only or
shadow-only until class-specific proof covers false-exact boundaries,
generated/no-source candidates, dynamic and stale facts, edit freshness, and the
intended user-visible output behavior.

With the method-declaration and package-declaration scoped proofs recorded, the
next semantic-token step is another scoped class cutover with
no-token-output-change, false-exact, fallback, and edit-freshness evidence. This
review does not promote method-call or field-declaration compiler facts to live
token output.
