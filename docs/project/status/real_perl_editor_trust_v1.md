...
real-workspace edit-freshness and rollback receipts
before any promotion. Safe delete is now `partial-live-with-fallback` only for
...
1. `docs(status): review semantic-token method-declaration support before broader cutover`
2. `test(workspace-symbols): add another project-shaped generated-symbol rank/noise receipt before broader expansion`
3. `test(rename): add package-local edit-freshness / rollback real-workspace receipts before broader promotion`
