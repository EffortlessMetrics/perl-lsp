...
| Safe delete | `partial live source-backed pilot / boundary-shadowed broader facts` | Mojolicious scenario 36 records file-delete warning UX for a dependent module delete; Dancer2 runtime receipts record symbol-level `_compile`, `routes`, and `plugin_keywords` request shapes where stale, generated, dynamic-boundary, and low-confidence fixtures block deletion with zero live edits; CPAN-style RealBaseline runtime receipts record `RealBaseline::Util::helper` blocked by fresh compiler facts because it is imported by another file and `RealBaseline::Base::reset` allowed by fresh high-confidence semantic facts, with explicit no-live-edit rollback state for both paths; requested RealBaseline `reset` edit rollback proof records a source-backed delete WorkspaceEdit plus inverse rollback edit that restores the original text without applying live edits; covered safe-delete receipt paths persist trace-only provider decisions that `perl.explainProviderDecision` can replay; `perl.previewSafeDelete` exposes blocked/allowed scoped no-edit UX for those paths; `perl.safeDelete` now returns a rollback-backed WorkspaceEdit only for the narrow source-backed RealBaseline `reset` pilot and preserves no-edit blocker behavior for imported symbols | Stale, low-confidence, generated, imported/exported, and dynamic facts cannot authorize symbol deletion; the live pilot is limited to fresh high-confidence unreferenced source-backed symbols with exact rollback proof and does not broaden safe-delete authorization | Additional real-workspace safe-delete live-pilot receipts across generated, dynamic, stale, low-confidence, and second-project source-backed blocker shapes before broader promotion |
...
guard. Safe delete now has a narrow source-backed live pilot, but broad
symbol-level deletion remains gated. The recent rollback, live-blocker, and
fallback/noise receipts sharpen known limitations and next proof, but they do
not authorize broad package/compiler-backed rename or broad live symbol-level
safe-delete cutover.
...
1. `test(safe-delete): verify narrow source-backed symbol-delete pilot across blockers`
...
`perl.safeDelete` now exposes a narrow source-backed symbol-delete live pilot:
it returns a rollback-backed WorkspaceEdit only for fresh high-confidence
unreferenced source-backed symbols and keeps imported, generated, dynamic,
stale, and low-confidence cases no-edit.
...
Safe-delete support tiers have been reviewed after the scoped preview and edit
rollback proof: the row remains `shadowed`, and the next proof is a narrow
source-backed symbol-delete live pilot with rollback/fallback guard before
promotion.
