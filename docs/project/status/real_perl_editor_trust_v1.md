...
1. `test(workspace-symbols): add another scoped generated-symbol class proof before broader generated-symbol expansion`
2. `test(rename): add additional project-shaped package/compiler-backed fallback/edit-freshness receipt before broader rename promotion`
3. `feat(semantic-tokens): expose another reviewed scoped token class through live provider-decision traces`
