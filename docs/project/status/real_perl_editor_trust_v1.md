...
1. `fix(rename): close real-workspace package-pilot empty-plan gap before cutover`
...
now proves the eligible no-edit UX shape for source-backed definition/reference
plans, but real-workspace package-pilot requests still expose an empty compiler
plan boundary that must close before live package-local rename can be reviewed.
