...
real-workspace false-allow, fallback, edit-freshness, and rollback receipts
...
1. `test(rename): add package-local false-allow/fallback real-workspace receipt before broader promotion`
2. `docs(status): review semantic-token method-declaration support before broader cutover`
3. `test(workspace-symbols): add another project-shaped generated-symbol rank/noise receipt before broader expansion`
