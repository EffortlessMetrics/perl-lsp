...
| Safe delete | `boundary-shadowed` | Mojolicious scenario 36 records file-delete warning UX for a dependent module delete; Dancer2 runtime receipts record symbol-level request shapes where stale compiler facts, generated members, a dynamic-boundary fixture on a typeglob request, and low-confidence compiler plans block deletion without live behavior changes; a CPAN-style RealBaseline runtime receipt records `RealBaseline::Util::helper` blocked by fresh compiler facts because it is imported by another file, with zero live edits | Stale, low-confidence, generated, imported/exported, and dynamic facts cannot authorize symbol deletion | Explicit symbol-level blocker UX and exact-static live proof before cutover |
...
1. `feat(safe-delete): expose compiler blocker reasons through provider explanations`
2. `feat(safe-delete): enable exact static safe-delete`
