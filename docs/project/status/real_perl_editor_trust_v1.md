...
1. `test(workspace-symbols): add generated/dynamic/noise expansion receipt`
2. `test(rename): add package rename rollback and fallback/noise preview receipts`
3. `feat(semantic-tokens): pilot one compiler-backed token class after generated/dynamic/stale blockers`
