...
1. `feat(rename): enable narrow package-local rename live pilot`
...
Package/compiler-backed rename support has now been reviewed after the
package-local live-pilot receipt. Same-file lexical rename remains the only live
rename claim today, and package/compiler-backed rename remains
preview/receipt-only until a separate implementation PR actually enables the
narrow live class. The compiler-allowed preview receipt proves the eligible
no-edit UX shape for source-backed definition/reference plans, real-workspace
package-pilot requests close the empty compiler plan boundary with a
source-backed definition edit, and the package-local live-pilot receipt proves
source-backed definition/reference planned edits plus rollback/fallback
guardrails while generated, dynamic, stale, and low-confidence blockers still
return no edits. That is enough to queue a separate narrow package-local live
pilot for source-backed definition/reference plans only; it is not a support-tier
promotion or broad compiler-backed rename authorization.
