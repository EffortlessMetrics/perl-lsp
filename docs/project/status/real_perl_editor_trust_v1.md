...
2. `test(rename): add another project-shaped package/compiler-backed receipt before broader promotion`
3. `test(semantic-tokens): add additional class-specific compiler-token receipt before broader promotion`
...
Semantic-token support has been reviewed after the method-declaration parity
receipt. The row remains `partial-live-with-fallback`: the only compiler-backed
live slice is the source-backed subroutine-declaration token class that matches
existing parser/HIR `function` output and emits no new tokens. Method-call and
method-declaration compiler spans remain receipt-only or shadow-only until
additional class-specific receipts prove a safe promotion path.
