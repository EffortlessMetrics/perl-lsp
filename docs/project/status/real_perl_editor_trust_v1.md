...
safe-delete blocker routing items without broadening deletion.

1. `feat(semantic-tokens): expose another scoped field/method-call trace class`
2. `test(workspace-symbols): add additional generated/no-source project variant only if broader generated-symbol expansion is being considered`
3. `feat(diagnostics): add explain diagnostic payload`
