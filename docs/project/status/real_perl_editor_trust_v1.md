...
the scoped `my`/`state` case, and package-local live rename is limited to fresh
source-backed definition/reference plans that pass semantic proof plus the
workspace source/ambiguity guard. Safe delete remains `shadowed` for
symbol-level deletion. The recent rollback, live-blocker, and fallback/noise
receipts sharpen known limitations and next proof, but they do not authorize
broad package/compiler-backed rename or live symbol-level safe-delete cutover.
...
1. `test(safe-delete): prove symbol-delete edit rollback`
...
edit, and the package-local live-pilot receipts prove source-backed
definition/reference edits plus fallback/no-edit guardrails. The live path also
requires the workspace source/ambiguity guard before returning edits: qualified
source-backed references may be edited, ambiguous cross-package references
return no edits, and generated, dynamic, stale, low-confidence, package-wide,
or missing-proof blockers still return no edits. This is a narrow
