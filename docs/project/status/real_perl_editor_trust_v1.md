...
still needs a scoped support review and cutover proof. This review does not
promote workspace symbols beyond the existing source-backed ready-index slice
plus generated-label pilot.

## Semantic Token Support Review

The package-declaration, method-call, method-declaration, and field-declaration
parity receipts do not justify a broader compiler-backed semantic-token cutover.
...
or `variable` tokens, but they remain receipt-only or shadow-only until
class-specific proof covers
...
workspace-symbol receipt closes the immediate generated-symbol rank/noise
routing item, and the package-declaration receipt closes the immediate
semantic-token class-specific receipt request; route the next provider slice
through support review before another rename receipt.

1. `docs(status): review workspace-symbol generated expansion before broader generated-symbol cutover`
2. `docs(status): review semantic-token class receipts before broader compiler-token promotion`
