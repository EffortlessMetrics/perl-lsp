...
1. `test(workspace-symbols): add project-shaped generated/no-source proof before broader generated-symbol expansion`
2. `feat(semantic-tokens): expose another scoped field/method-call trace class`
3. `test(safe-delete): add cross-project referenced-source blocker receipts before broader symbol-delete promotion`
...
allow proof, source guard, current-source reference guard, and rollback proof
all pass.
...
source-backed receipt, and Dancer2 referenced-source false-allow blockers. The
row remains `partial-live-with-fallback` only for the exact unreferenced
source-backed symbol-delete pilot. RealBaseline `reset` and Dancer2 `to_psgi`
prove that the live path can return client-applied delete WorkspaceEdits with
rollback proof for two project shapes; Dancer2 `header` and current-source
`to_psgi` references prove that project-shaped references block with zero
returned edits. These receipts do not justify broader symbol deletion,
generated/dynamic deletion, non-subroutine deletion, package-wide deletion,
