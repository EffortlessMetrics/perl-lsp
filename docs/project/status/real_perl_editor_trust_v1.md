...
| Document symbols | `partial live source-backed` | Runtime quality receipts record source-backed parser-syntax symbol counts and fact traces; dedicated real-workspace document-symbol quality receipt pending | Astless, stale, dynamic, virtual generated/no-source, low-confidence, and ambiguous candidates keep fallback/gated behavior | Real-workspace document-symbol quality receipt covering generated labels, missing symbols, and dynamic/stale blockers before broader symbol cutover |
...
1. `test(document-symbols): add real-workspace symbol quality receipt`
