...
and gated false-exact/dynamic/stale boundary. Any broader generated-symbol
expansion still needs another scoped class proof. This review does not promote workspace
symbols beyond the existing source-backed ready-index slice plus generated-label
pilot.

## Semantic Token Support Review

The package-declaration, method-call, and field-declaration parity receipts do
not justify a broader compiler-backed semantic-token cutover. The
method-declaration proof now authorizes only the scoped
...
The `package MyApp::Controller::Root` declaration, `$c->stash` method-call,
`method greet` declaration, and `field $name` declaration receipts prove
source-backed compiler spans can align with existing live `namespace`, `method`,
or `variable` tokens, but package-declaration, method-call, and
field-declaration remain receipt-only or shadow-only until class-specific proof
covers false-exact boundaries, generated/no-source candidates, dynamic and stale
facts, edit freshness, and the intended user-visible output behavior.

With the method-declaration scoped proof recorded, the next semantic-token step
is another scoped class cutover with no-token-output-change, false-exact,
fallback, and edit-freshness evidence. This review does not promote
package-declaration, method-call, or field-declaration compiler facts to live
token output.
...
1. `test(rename): add project-shaped package/compiler-backed false-allow and fallback receipt`
2. `test(semantic-tokens): prove another scoped compiler-token class before broader compiler-token promotion`
3. `test(workspace-symbols): add another scoped generated-symbol class proof before broader generated-symbol expansion`
