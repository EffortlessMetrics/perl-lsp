...
1. `test(safe-delete): add third-project or client-applied live-pilot receipt`
...
Dancer2 `to_psgi` receipt: the row remains `partial-live-with-fallback` only
for the exact source-backed symbol-delete pilot. That cutover still does not
justify broader symbol deletion. The next proof is a third-project or
client-applied live-pilot receipt before any non-subroutine, package-wide,
generated/dynamic, or broader safe-delete promotion.
