...
1. `test(workspace-symbols): add additional generated/dynamic false-exact and edit-freshness receipts`
2. `test(rename): add package-local false-allow/fallback real-workspace receipt before broader promotion`
3. `docs(status): review semantic-token method-declaration support before broader compiler-token promotion`
