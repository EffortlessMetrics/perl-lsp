...
guard. Safe delete is now `partial-live-with-fallback` for the narrow
`perl.safeDeleteSymbol` source-backed subroutine pilot only. The recent rollback,
live-blocker, and fallback/noise receipts sharpen known limitations, but they do
not authorize broad package/compiler-backed rename or broader symbol-level
safe-delete promotion.
...
1. `test(workspace-symbols): add narrow generated-symbol pilot proof`
2. `test(semantic-tokens): add project-shaped compiler-backed token-class receipts`
3. `docs(status): review package-local rename live pilot before broader promotion`
...
pilot. Broader symbol deletion remains blocked pending a post-cutover proof
review.
