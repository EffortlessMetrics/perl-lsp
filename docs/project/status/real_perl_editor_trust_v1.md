...
| Diagnostics | `partial live` | Mojolicious baseline explicitly defers broad diagnostic correctness; scenario 31 now covers workspace-present imports, a mixed present/missing import boundary, dynamic route-method conservatism, and true missing-module PL701 while scope diagnostics label low-confidence, ambiguous, and dynamic-boundary-shaped visible-symbol evidence when conservative PL109 diagnostics remain | Conservative diagnostics remain when semantic evidence is absent, ambiguous, stale, or dynamic; weak evidence is labeled instead of silently suppressing true unknowns | Additional second-project false-positive / false-negative receipts before wider diagnostic correctness claims |
...
| Safe delete | `boundary-shadowed` | Mojolicious scenario 36 records file-delete warning UX for a dependent module delete; a Dancer2 runtime receipt records a symbol-level `_compile` request where a stale compiler-fact fixture blocks deletion without live behavior changes; a CPAN-style RealBaseline runtime receipt records `RealBaseline::Util::helper` blocked by fresh compiler facts because it is imported by another file, with zero live edits | Stale, low-confidence, generated, imported/exported, and dynamic facts cannot authorize symbol deletion | Additional real-workspace generated/dynamic/low-confidence blockers plus explicit symbol-level blocker UX receipts |
...
1. `test(workspace-symbols): add second-project noise receipt`
2. `test(diagnostics): add second-project false-positive / false-negative receipt`
