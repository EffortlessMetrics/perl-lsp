...
1. `test(rename): add package rename rollback and fallback/noise preview receipts`
2. `feat(semantic-tokens): pilot one compiler-backed token class after generated/dynamic/stale blockers`
3. `test(workspace-symbols): add third-project generated/dynamic/noise receipt before broader expansion`
