...
The package-declaration, field-declaration, and method-call parity receipts do
...
The `$c->stash` method-call receipt proves a source-backed compiler span can
align with an existing live `method` token, but method-call remains receipt-only
or shadow-only until class-specific proof covers false-exact boundaries,
generated/no-source candidates, dynamic and stale facts, edit freshness, and the
intended user-visible output behavior.

With the method-declaration, package-declaration, and field-declaration scoped
proofs recorded, the next semantic-token step is another scoped class cutover with
no-token-output-change, false-exact, fallback, and edit-freshness evidence. This
review does not promote method-call compiler facts to live token output or
broader field-declaration facts beyond the scoped output-neutral proof.
