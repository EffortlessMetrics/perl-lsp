...
1. `test(safe-delete): add scoped allowed-symbol cutover proof`
2. `feat(provider): persist provider-local decision traces`
3. `test(rename): add scoped package/compiler-backed pilot proof`
