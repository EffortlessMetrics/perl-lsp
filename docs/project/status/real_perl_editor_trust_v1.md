...
| Semantic tokens | `partial live source-backed token class` | Mojolicious scenario 34 records live token counts, LSP 5-tuple/span validity, source-backed token hits, dynamic-boundary string non-promotion, and edit freshness; Dancer2 scenario 38 adds second-project package, DSL, app, typeglob-boundary, and edit-freshness token proof; runtime quality receipts now record both synthetic source-backed and RealBaseline project-shaped source-backed compiler-fact subroutine-declaration classes whose spans match the existing live parser/HIR `function` token output, and the live-output parity receipt proves synthetic, Catalyst-shaped, and RealBaseline receipts capture the exact live handler output without adding tokens; ordinary semantic-token requests now persist a provider-local `partial_live_source_backed` trace for that exact subroutine-declaration token class when the source-backed span already matches live output; semantic-shadow receipts cover generated/no-source and fallback token boundaries | Existing parser/token provider remains the broad live output; generated/no-source, stale, dynamic, fallback, and unmatched compiler classifications stay blocked, fallback-only, or shadowed; the source-backed token-class slice emits no new token output and does not authorize broader compiler-backed token classes | Verify the first source-backed token slice on real-workspace receipts before broader semantic-token classes |
...
1. `test(workspace-symbols): add third-project generated/dynamic/noise receipt before broader expansion`
2. `test(rename): add package/compiler-backed live pilot receipt before cutover`
3. `test(semantic-tokens): verify first source-backed token live slice before broader classes`
