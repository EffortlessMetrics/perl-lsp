...
The method-call and method-declaration parity receipts do not justify a broader
...
The `$c->stash` method-call and `method greet` declaration receipts prove
source-backed compiler spans can align with existing live `method` tokens, but
they remain receipt-only or shadow-only until class-specific proof covers
