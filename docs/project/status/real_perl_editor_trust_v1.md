...
With method-declaration, package-declaration, field-declaration, and method-call
scoped proofs recorded, the next semantic-token step is support review before
any broader compiler-token promotion.
...
1. `docs(status): review semantic-token scoped token classes before broader compiler-token promotion`
