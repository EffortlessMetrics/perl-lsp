...
| --- | --- | --- | --- |
| Diagnostics | `partial live` | Existing semantic queries suppress selected dynamic false positives; fallback diagnostics remain available | Fact-source tracing plus false-positive / false-negative fixture receipts |
| Completion | `partial live / shadowed` | Existing completion paths and semantic-shadow fixtures can use visible symbols and generated members | Provider-impact rows for compiler facts and ranking stability |
| Hover | `shadowed` | Hover shadow code can query visible symbols and provenance | Promote only after provenance labels and fallback behavior are fixture-backed |
