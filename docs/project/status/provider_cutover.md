...
- Rename and safe-delete now have boundary-shadowed proof for exact static
  ...
  rename remain blocked or fallback/shadow data.
...
| Rename | `partial live lexical / boundary-shadowed compiler facts` | Same-file sigiled lexical rename can use current-document scoped AST proof only when exactly one `my` or `state` declaration edit is proven; rename plan receipts still trace exact static edits, dynamic-boundary blockers, stale compiler facts, low-confidence ambiguity, runtime blocker UX notes, live-vs-compiler exact-static receipt data, Mojolicious scenario 35 and Dancer2 scenario 37 real-workspace unsafe-edit proof, the scoped lexical cutover in [#8915](https://github.com/EffortlessMetrics/perl-lsp/pull/8915), and `lsp_rename_tests::test_workspace_rename_workspace_edit_rolls_back_cleanly` rollback proof before any broad compiler-backed refactor behavior | Additional package/compiler-backed rename receipts plus fallback/noise proof before broader refactor migration |
| Safe delete | `boundary-shadowed` | Safe-delete receipts trace exact static allow decisions, dynamic-boundary blockers, framework-generated blockers, stale compiler facts, runtime blocker UX notes, live-vs-compiler exact-static receipt data, Mojolicious scenario 36 file-delete warning UX, and a Dancer2 symbol-level stale-fact blocker request before any live compiler-backed symbol refactor behavior | Actual real-workspace semantic unsafe-delete blockers and explicit live blocker UX before any broader refactor migration |
...
| Semantic tokens | `shadowed` | Existing parser/token provider remains the live source; semantic-shadow fixtures trace parser/HIR, compiler-backed, dynamic-boundary, and stale fact candidates; runtime quality receipts capture live token count and shadow state without changing live behavior; Mojolicious scenario 34 and Dancer2 scenario 38 record project-shaped token/span validity and edit freshness | Narrow compiler-backed token-class proof before any live cutover |
