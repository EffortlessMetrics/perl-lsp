...
- Rename note, 2026-05-18: `refactor_runtime_blocker_ux_package_local_live_pilot_dancer2_fallback_refreshes_after_edit`
  records a Dancer2 `header` package/compiler-backed fallback and
  edit-freshness boundary. It proves compiler preview can see only a
  source-backed definition edit while live rename still falls back to broader
  current-source workspace-index edits, and the fallback edit set refreshes
  after `didChange` before broader compiler-backed rename promotion.
