...
- The semantic-token support review keeps both method-call and
  method-declaration compiler-token receipts receipt-only or shadow-only. They
  prove span alignment with existing live parser/HIR `method` tokens, not a
  broader compiler-backed semantic-token cutover.
...
- Semantic-token note, 2026-05-17: method-call and method-declaration parity
  receipts do not move method-class compiler facts into live token output.
  Broader compiler-backed semantic tokens still require class-specific cutover
  proof.
