...
| Semantic tokens | `partial live source-backed token class` | Existing parser/token provider remains the broad live source; semantic-shadow fixtures trace parser/HIR, compiler-backed, generated/no-source, dynamic-boundary, stale, and fallback candidates; runtime quality receipts capture live token count, shadow state, no-token-output-change proof, both synthetic source-backed and RealBaseline project-shaped compiler-fact subroutine-declaration classes matched to existing live `function` token output, and live-output parity across synthetic, Catalyst-shaped, and RealBaseline receipts; ordinary semantic-token requests persist a provider-local `partial_live_source_backed` trace only for that exact source-backed compiler-token class after live-output parity is proven; Mojolicious scenario 34 and Dancer2 scenario 38 record project-shaped token/span validity and edit freshness | Real-workspace verification for the first source-backed token slice before broader classes |
