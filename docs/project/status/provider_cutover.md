...
- The method-declaration scoped cutover proof proves a source-backed
  ...
  classes.
- The semantic-token support review keeps package-declaration, method-call, and
  field-declaration compiler-token receipts receipt-only or shadow-only. They
  prove span alignment with existing live parser/HIR `namespace`, `method`, and
  `variable` tokens, not a broader compiler-backed semantic-token cutover. The
  method-declaration scoped proof closes the first class cutover step while
  keeping broader compiler-token promotion gated by no-token-output-change,
  false-exact, fallback, and edit-freshness coverage.
...
- Rename note, 2026-05-17: `refactor_runtime_blocker_ux_package_local_live_pilot_catalyst_false_allow_falls_back_or_blocks`
...
- Semantic-token note, 2026-05-17: method-declaration parity has moved to a
  scoped output-neutral `token:method_declaration:` proof only. Package-
  declaration, method-call, and field-declaration parity receipts do not move
  class-specific compiler facts into live token output. Broader compiler-backed
  semantic tokens still require class-specific cutover proof.
