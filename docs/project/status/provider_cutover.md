...
| Rename | `boundary-shadowed` | Rename plan receipts trace exact static edits, dynamic-boundary blockers, stale compiler facts, low-confidence ambiguity, runtime blocker UX notes, live-vs-compiler exact-static receipt data, and Mojolicious scenario 35 real-workspace unsafe-edit proof for exact local lexical edits plus generated/dynamic no-edit boundaries before any broad compiler-backed refactor behavior | Additional real-workspace unsafe-edit receipts and a narrow lexical/package rename live-cutover proof before any broader refactor migration |
