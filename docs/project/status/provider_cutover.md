...
| Safe delete | `partial live source-backed pilot / boundary-shadowed` | Safe-delete receipts trace exact static allow decisions, dynamic-boundary blockers, framework-generated blockers, stale compiler facts, runtime blocker UX notes, live-vs-compiler exact-static receipt data, Mojolicious scenario 36 file-delete warning UX, Dancer2 and RealBaseline symbol-level blocker/allowed request shapes, requested RealBaseline `reset` delete edit plus inverse rollback proof, `perl.previewSafeDelete` scoped no-edit UX, and `perl.safeDelete` narrow source-backed live-pilot output for the rollback-proven RealBaseline `reset` path while imported-symbol blockers still return no edits | Additional real-workspace safe-delete live-pilot receipts across generated, dynamic, stale, low-confidence, and second-project source-backed blocker shapes before any broader refactor migration |
