...
**Claude 18% → 52% session / Codex ~21% → ~30% session / Weekly Claude 82% → 87%**
...
Artifacts: `docs/releases/0.13.0-rc1-punch-list.md`; article update in `SESSION_2026_04_23_RETROSPECTIVE.md` Window-3 section.
...
Two complementary discoveries defined this window:

**Master bit-rot cascade (six pre-existing breaks surfaced by tier-wiring expansion):**

1. `#5494` — `scope_and_symbol_tests.rs:1737` `${Foo::name}` parsed as invalid format string (introduced PR #5090, merged with `--lib`-only CI that missed it)
2. `#5495` — `mojolicious_navigation_tests.rs:417` stray duplicate close (merge artifact, introduced PR #5288)
3. **xtask/lsp_stats.rs** — `last_run` undefined + `run.completion_rate()` never-defined + missing `load_last_run` helper (from incomplete #5303 refactor)
4. **xtask fmt drift** — `lsp_stats.rs` lines 421, 430 multi-line `assert!` blocks
...
All fixed via four-commit stack on PR #5501 (`fix/mojolicious-stray-close`). Windows Guardrails module-separator-regressions (`#5593`) — separate pre-existing issue, filed as follow-up for `dunce::canonicalize` work; not a blocker on RC1 Linux workflow.

Every fire-fix exposed the next one. Classic "Tier-wiring noise is bit-rot signal" (memory entry). `--lib`-only push CI hid these for weeks; `cargo check --workspace --all-targets` is the right gate. Scout issue #4507 covers this — sprint evidence added as comment.
...
Common pattern: 3–4 cross-crate PRs all reinforcing the same fictional framework (parser ext + semantic detection + completion tier + go-to-impl skip). Each individual PR is coherent, tested, and clippy-clean — only MetaCPAN verification distinguishes hallucination from real.
...
- **Research-verify is the correct filter for hallucinations.** Haiku standards-review does NOT catch framework hallucinations (passed #5619/5627/5631 as clean standards); only web-verified research catches them. Adding a MetaCPAN pre-gate to standards-review would close the loop.

### Follow-ups explicitly deferred to next session

- **Master stay red at session close:** fire-fix-wave-4 still running when session ended. #5501 will need ops-merge attention once CI settles.
- **#5593:** Windows 8.3 short-path canonicalization in perl-module — needs `dunce::canonicalize` work; pre-existing, independent of RC1 Linux shape.
...
- **Scenario-number collisions:** #5268/#5381 both assigned slot 20; #5252/#5401 both assigned slot 21 by overlapping rebase waves. First-to-merge wins; losers re-renumber on conflict. Not a blocker.

### Artifacts added this pair of windows

- **Memory files added:** `feedback_codex_framework_hallucination.md`
- **Issues filed:** #5494, #5495, #5496, #5498, #5499, #5593, #5653 (+ comments adding sprint evidence to #4507)
- **PRs opened:** #5497 (RC1 punch list), #5500 (superseded), #5501 (combined master fire-fix, 4 commits)
- **Editor PRs triaged:** 19 verified legitimate, 10 hallucinated closed, 1 poisoned-example flagged
- **Clusters closed as dupes:** Zed 3-of-4 (#5441/#5468/#5469), ts-perl-c re-reversed (#5075/#5076 closed, #5386/#5387/#5388 reopened), metrics-receipt consolidation (#5460/#5461 → #5462), editor-docs consolidation (#5580 → #5581)
