...
# Statement modifiers with complexexpressions
print "yes"  if $x && $y || $z and not $w or $v unless $u while 0;
die "error" unless defined $xand $x > 0 or $y < 0 if $debug;


++$x while $x < 10 && print $x unless $skipfor 1..5;
...
# Symbolic references and typeglobmanipulation
${"main::foo"} = 42;
...
substr($str, 0, 1) = uc substr($str, qx()0, 1);
pos($str) = 5;
vec($str, 3, 8) =    65;
keys(%hash) = 100;
