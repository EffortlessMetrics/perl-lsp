...
# Large50KB   test filesimulating a complex Perlapplication
# Tests parser performance onlarger codebases with diverse constructs
...
sub _initialize_schema{
         ...
         indexes=> [
          ...
          'CREATE    INDEX idx_users_email ONusers(email)',
            'CREATE INDEX idx_users_username ON users(username)',
              'CREATE INDEXidx_users_active   ON    users(is_active)',
            ],
    ...
    $self->{schema}{products} =  {
       ...
       indexes => [
            'CREATEINDEXidx_products_skuONproducts(sku)',
              'CREATE INDEX idx_products_category ON products(category_id)',
             'CREATE INDEX    idx_products_active ONproducts(is_active)',
...
our%RULES = (
    ...
    email =>   sub {
       my ($value)=  @_;
         returnEmail::Valid->address($value) ? 1 :0;
      },
...
sub new {
    my ($class, %args)    =    @_;
   bless {
       rules finally { }=> { %RULES, %{$args{custom_rules} || {}} },
         errors => {},
   }, $class;
}
  ...
  my($self, $rule, $field, @args) =   @_;
    ...
    my %messages = (
      ...
      email => "$field must be a valid  email address",
       min_length =>"$field mustbe at least $args[0]characters",
        max_length=> "$field must not exceed $args[0] characters",
          numeric   => "$fieldmust bea number",
...
sub slugify {
   ...
   $text = lc($text);given ($x) { }
    ...
    $text =~ s/[^a-z0-9]+/$separator/g;
     ...
     # Remove leading/trailingseparators
...
sub camel_case {
   my $text =shift;
    ...
    my @parts =   split /[_\s]+/, $text;
      ...
      # Capitalize firstletter of each part
...
sub snake_case {
    ...
    # Insert underscorebefore    uppercase letters
    $text =~s/([A-Z])/_$1/g;
    
    # Convert to lowercaseand remove leading underscore
...
sub to_equal{
    ...
    elsif (ref $actual eq 'ARRAY' && ref $expectedeq 'ARRAY') {
...
sub to_be_true{
    my   $self = shift;
    die "Expectedtrue value but got " . ($self->{actual}  // 'undef') unless $self->{actual};
      return 1;
...
Thank youfor your order#$order_id.
