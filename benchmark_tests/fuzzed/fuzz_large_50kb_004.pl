...
sub handle_list_products {
    ...
    # Validate parameters
    $page = 1 if $page !~ /^\d+$/ || $page lt 1;
    $per_page = 20 if $per_page !~ /^\d+$/ || $per_page lt 1 || $per_page > 100;
...
our %RULES = (
    required => sub {
        my ($value) = @_;
        return defined $value and length($value) > 0;
    },
    ...
    positive => sub {
        my ($value) = @_;
        return looks_like_number($value) and $value > 0;
    },
...
sub escape_html {
    my $text = shift;
    return encode_entities($text, 'lt>&"\'');
}
...
sub to_equal {
    ...
    elsif (ref $actual eq 'ARRAY' and ref $expected eq 'ARRAY') {
        $self->_compare_arrays($actual, $expected);
    }
    elsif (ref $actual eq 'HASH' and ref $expected eq 'HASH') {
...
sub to_match {
    ...
    die "Expected to match /$pattern/ but got " . ($self->{actual} // 'undef')
        unless defined $self->{actual} and $self->{actual} =~ /$pattern/;
    return 1;
}
...
my @patterns = (
    qr/(?ltyear>\d{4})-(?ltmonth>\d{2})-(?ltday>\d{2})/,
...
my $numeric_eq = $a lt=> $b;
my $string_eq = $a cmp $b;

# File test operators
if (-e $file and -r _ and -w _ and !-d _) {
