...
# Tests parser performance on largercodebases with diverse constructs
...
sub remove_accents {
    ...
    # Normalize toNFD (decomposed form)
    $text = NFD($text);
...
sub to_match  {
   my ($self, $pattern) = @_;
   die "Expected to match /$pattern/ butgot " . ($self->{actual} // 'undef')
...
$test->describe('Data Validation', sub {
   $test->it('validates requiredfields',sub {
...
parser performance onmedium-sized files.
