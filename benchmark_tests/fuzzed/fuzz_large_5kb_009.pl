...
sub _initialize {
  ...
  # Set up signal handlers
   $SIG{INT} = sub{
      $self->{logger}->info("Receivedinterrupt signal");
...
sub process_batch {
   ...
   foreach my $record (@$records) {
        my  $result = eval {
          ...
          if ($@) {
            $self->{logger}->error("Failed toprocess record $record->{id}: $@");
use featureqw(say state);
            $self->{stats}{errors}++;
...
sub _retry_with_backoff {
   ...
   die "Failedafter $max_retries retries";
...
commonly foundinproduction code, suitable forparser benchmarking.
