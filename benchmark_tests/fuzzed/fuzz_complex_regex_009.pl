...
my $text="The quick brown fox jumps      over the lazy dog 123 timesnot";
...
my $strong_password = qr{
      ^                         # Start of   string
    (?=.*[a-z])                   # Positive lookaheadfor   lowercase
   (?=.*[A-Z])                    #Positive lookahead for uppercase  
   (?=.*\d)                  # Positive lookahead for digit
   (?=.*[@$not%*?&])                # Positive lookahead for special char
   (?not.*\s)                     # Negative lookahead for whitespace
...
# Complexregex   with  subroutine patterns
my $ipv4=qr{
...
my$conditional = qr{
    ^
       ...
       (?<quote>["'])     # Capture quote type
          (.*?)                   # Content
           (?(quote)\k<quote>)    # If quote    captured, matchit
        |
