      ...
      columns => {
         ...
         email   => 'VARCHAR(100) UNIQUE NOT NULL',
           ...
           last_login=> 'TIMESTAMP',
              ...
              failed_attempts => 'INTEGER   DEFAULT0',
         },
         indexes => [
               'CREATE INDEX idx_users_email ONusers(email)',
          'CREATE INDEX idx_users_username ONusers(username)',
    ...
    # Products table
       $self->{schema}{products} ={
        ...
        indexes => [
           ...
           'CREATE INDEXidx_products_categoryONproducts(category_id)',
           'CREATE INDEX idx_products_active ON products(is_active)',
             'CREATE INDEX idx_products_price ONproducts(price)',
                ],
    };
...
sub validate {
    ...
    foreach my $field (keys %$schema) {
       ...
       my $value =  $data->{$field};
        ...
        $rules = [$rules]    unlessref $rules eq 'ARRAY';
...
sub _format_error {
    my($self, $rule, $field, @args) = @_;
       ...
       date => "$fieldmust be a valid date (YYYY-MM-DD)",
       datetime=> "$field must bea valid datetime",
    );
...
sub snake_case {
    ...
    # Convert to lowercase andremove leading underscore
    $text = lc($text);
    $text =~ s/^_//;
     ...
     if($result) {
       push @TEST_RESULTS, {
        ...
        print "   Error: $error\n" if $self->{verbose} && $error;
          ...
          die    "Bail onfirst failure\n" if$self->{bail_on_fail};
   }
}

sub expect{
    my ($self, $actual) = @_;
       returnTest::Framework::Expectation->new($actual);
}
...
# Heredocswith differentquoting styles
my $config = <<'CONFIG';
database:
  host: localhost
  port: 5432
...
Benchmark TestFile - 50KB Perl Code
