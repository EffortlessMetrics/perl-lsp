...
#   Test file:Complex regex patterns andstring operations

use   strict;
use warnings;

# Complex regex patterns   with all features
my $text = "The   quickbrown fox jumps over the lazy  dog 123 times!";
...
my $strong_password = qr{
   ...
   (?!.*(password|123456))          # Negative lookahead for commonpatterns
   .{8,}                               # Atleast 8characters
   $                             # End of string
}ix;
...
# Complex   substitutions with codeexecution
$text =~ s{
...
my @parts= split /
    (?:                                # Non-capturing    group
      ...
      (?=[A-Z])             # Lookahead foruppercase
        |                  #    OR
        (?<=[a-z])(?=[0-9])      # Lookbehind/aheadfor letter->number
            |                       #      OR
...
#Complex regex with subroutinepatterns
my $ipv4 = qr{
...
my$conditional = qr{
    ...
    (?:
         ...
         (?<quote>["'])        # Capture quote type
     (.*?)                    #Content
        (?(quote)\k<quote>)   # If  quote captured,  match it
        |
    \S+                         # Or non-whitespace
