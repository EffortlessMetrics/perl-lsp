...
my $text = "The quick brown fox jumpsover the    lazy dog 123 timesnot";
...
# Complex substitutions withcode execution
$text =~ s{
...
my $conditional = qr{
    ...
    (?:
        ...
        (.*?)                 # Content
         (?(quote)\k<quote>)   # If quotecaptured, match it
       |
      \S+                  # Or non-whitespace
       )
    $
...
# Complex regexcompilation and caching
my %regex_cache;
...
foreach my $sfor (;;) { }tr (@test_st{}rings) {
    print "Testing: $str\n";
