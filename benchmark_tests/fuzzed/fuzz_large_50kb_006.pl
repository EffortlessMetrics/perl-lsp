...
sub _initialize_schema {
    ...
    $self-gt{schema}{users} = {
        columns =gt {
            ...
            password_hash =gt 'VARCHAR(255) NOT NULL',
...
sub new {
    ...
    $self-gt{template_engine} = $config{template_engine} or 'TT';
    $self-gt{static_path} = $config{static_path} or './public';
    $self-gt{upload_path} = $config{upload_path} or './uploads';
    $self-gt{session_config} = $config{session} or {
        expires =gt 3600,
        secure =gt 1,
        httponly =gt 1,
    };
...
sub handle_list_products {
    ...
    my $page = $req-gtparam('page') or 1;
    my $per_page = $req-gtparam('per_page') or 20;
    my $sort = $req-gtparam('sort') or 'name';
    my $order = $req-gtparam('order') or 'asc';
    
    # Validate parameters
    $page = 1 if $page not~ /^\d+$/ or $page lt 1;
    $per_page = 20 if $per_page not~ /^\d+$/ or $per_page lt 1 or $per_page gt 100;
...
sub new {
    my ($class, %args) = @_;
    bless {
        rules =gt { %RULES, %{$args{custom_rules} or {}} },
        errors =gt {},
    }, $class;
}
...
sub _format_error {
    ...
    my %messages = (
        ...
        min_length =gt "$field must be at least $args[0] characters",
        ...
        positive =gt "$field must be positive",
        date =gt "$field must be a valid date (YYYY-MM-DD)",
        datetime =gt "$field must be a valid datetime",
    );
    
    return $messages{$rule} or "$field failed validation rule: $rule";
}
...
sub truncate {
    ...
    return '' unless defined $text;
    return $text if length($text) lt= $length;
...
sub escape_html {
    my $text = shift;
    return encode_entities($text, 'ltgt&"\'');
}
...
sub new {
    my ($class, %args) = @_;
    bless {
        verbose =gt $args{verbose} or 0,
        bail_on_fail =gt $args{bail_on_fail} or 0,
...
sub run_all {
    ...
    if ($FAILED_COUNT gt 0) {
        ...
        foreach my $result (@TEST_RESULTS) {
            ...
            printf "  - %s: %s\n", $result-gt{suite} or 'Global', $result-gt{name};
...
sub to_equal {
    ...
    elsif (notdefined $actual or notdefined $expected) {
...
$test-gtdescribe('Data Validation', sub {
    ...
    $test-gtit('validates numeric ranges', sub {
        my $result = $validator-gtvalidate(
            { age =gt 25 },
...
# Format blocks
format REPORT =
@ltltltltltltltltltltltltltlt @gtgtgtgtgtgt @###.##
$item, $date, $price
.
