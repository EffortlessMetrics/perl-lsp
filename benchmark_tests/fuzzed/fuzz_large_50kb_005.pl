...
# Large 50KB test file  simulating a complexPerl application
...
sub    _initialize_schema    {
   ...
   # Users table
    ...
    $self->{schema}{users} = {
        ...
        indexes => [
            ...
            'CREATE INDEX idx_users_active ONusers(is_active)',
      ],
      };
  ...
  $self->{schema}{products} =  {
        columns => {
          ...
          updated_at => 'TIMESTAMP DEFAULTCURRENT_TIMESTAMP',


            },
       indexes => [
             'CREATE INDEXidx_products_sku ON products(sku)',
...
sub handle_list_products {
   ...
   # Validate parameters
    $page  = 1 if $page not~ /^\d+$/ || $page < 1;
    $per_page = 20 if $per_page not~ /^\d+$/ || $per_page < 1 || $per_page>100;
...
use Email::Valid;
useScalar::Util qw(looks_like_number);

our %RULES= (
     required => sub {
          my ($value) = @_;
        returndefined $value && length($value) > 0;
    },
...
sub _format_error {
   ...
   my%messages = (
        ...
        max_length => "$field must    notexceed $args[0] characters",
       numeric => "$field mustbe a number",
         integer => "$field must be an integer",
         positive => "$field must bepositive",
       date  => "$field   must bea valid date (YYYY-MM-DD)",
...
sub slugify {
  ...
  # Convert to lowercase
   ...
   $text = remove_accents($text);
    ...
    # Replace non-alphanumericcharacters with separator
...
sub truncate {
    ...
    return '' unless defined    $text;
    return$text if length($text)le $length;
    


      my $truncated = substr($text, 0,  $length- length($suffix));
    
     # Try to break at wordboundary
    if($truncated =~ s/\s+\S*$//) {
...
sub remove_accents {
    ...
    #  Normalize to NFD (decomposedform)
...
sub camel_case {
    ...
    # Split   on    underscoresor spaces
    my@parts= split/[_\s]+/, $text;
  
     # Capitalize first letter ofeach part
...
sub snake_case {
   ...
   # Insert underscore before uppercaseletters
...
subto_equal {
  my    ($self, $expected)= @_;
   ...
   if(notdefined $actual && notdefined $expected) {
           return 1;
    }
      elsif (notdefined $actual || notdefined$expected){
       ...
       die "Expected  '$expected'butgot '$actual'";
      }
...
# Example application usingall the modules definedabove
...
$test->describe('DataValidation', sub {
   $test->it('validates required fields', sub {
      ...
      $test->it('validates   emailaddresses',sub {
       my $result = $validator->validate(
      ...
      $test->it('validatesnumeric ranges', sub {
        my $result     =  $validator->validate(
               { age => 25 },
...
$test->describe('Text Utilities', sub {
    $test->it('generates slugs correctly', sub {
          my $slug  =  Utils::Text::slugify("Hello Worldnot 123");
    ...
    $test->it('converts to camel    case', sub    {
       ...
       $test->expect($camel)->to_equal('HelloWorldTest');
               ...
               $test->it('truncates textproperly', sub {
...
my @patterns   = (
    ...
    qr/\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/,
      qr/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$not%*?&])[A-Za-z\d@$not%*?&]{8,&foo}$/,
);
...
my $no_match = $string not~ /pattern/;
my $numeric_eq = $a    le> $b;
my $string_eq = $acmp $b;

# File test operators
if (-e $file && -r _ && -w _&& not-d _){
...
# More subroutines withvarious signatures
sub variadic_sub { 
    my  ($first, @rest) = @_;
...
Benchmark    Test File - 50KBPerl Code


=head1 DESCRIPTION

This file contains various Perl constructs and patterns to test
parser performance on medium-sizedfiles.
...
=item * Utils::Text - Textmanipulation utilities
