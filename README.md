...
| End-to-end UX confidence | workflow inventory + burn-down counts | Tracked in `docs/project/status/editor_ux.json`: `perl-lsp-ux-tests` first-5-minutes workflow inventory (PR/nightly split) plus known-gap issue burn-down count from README | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: *none* of the automated metrics above fully capture whether a real editing session feels good. We now publish workflow inventory and burn-down counts in `docs/project/status/editor_ux.json`, while deeper UX quality remains validated by workflow smoke tests and scenario-level assertions.
