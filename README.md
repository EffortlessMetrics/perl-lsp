...
| End-to-end UX confidence | Scenario + burn-down scorecard | Tracked in `docs/project/status/editor_ux.json` via `perl-lsp-ux-tests` scenario inventory and known UX-gap issue burn-down count (from this README's "Known gaps toward solid UX" section), with summary bullets in `docs/project/status/quality.md` | Anything about parser breadth, protocol catalog size, or capability count |
