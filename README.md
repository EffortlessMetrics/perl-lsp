...
| End-to-end UX confidence | Published in `docs/project/status/editor_ux.json` when receipts are present | Tracked by the `perl-lsp-ux-tests` first-5-minutes harness, `cargo xtask metrics lsp-stats --json`, and status regeneration via `cargo xtask update-status --only quality` | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: parser and capability totals still don't guarantee a good editing experience. UX confidence is now tracked via the workflow harness receipts (`.ci/metrics/editor_ux.json`) and reflected in `docs/project/status/editor_ux.json`, with manual smoke tests and known-gap burn-down still used for signal coverage that isn't instrumented yet.
