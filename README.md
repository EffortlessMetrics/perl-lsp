...
| End-to-end UX confidence | tracked signals (`editor_ux.json`) | Published in `docs/project/status/editor_ux.json`: first-5-minutes scenario inventory, known-gap issue refs, and top-line metric tracking state; still supplemented by manual smoke workflows and open-issue burn-down | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: *none* of the automated metrics above fully measure whether a real editing session feels good. We now publish tracked UX signals in `docs/project/status/editor_ux.json`, but still rely on workflow smoke tests and the known-gap issue list below for final confidence.
