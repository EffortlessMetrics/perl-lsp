...
| End-to-end UX confidence | `docs/project/status/editor_ux.json` signal receipt | Tracked via executable manual-smoke workflow inventory parity, first-5-minutes harness scenario coverage (PR/nightly lanes), and known-gap issue-link burn-down count | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: parser/capability coverage is still not a proxy for felt editing quality. We now publish a signal receipt for workflow inventory coverage, CI lane coverage, and known-gap issue burn-down in `docs/project/status/editor_ux.json`, while still relying on scenario-level smoke tests for correctness.
