...
| End-to-end UX confidence | tracked signal set | Covered by manual editor smoke workflows, the `perl-lsp-ux-tests` first-5-minutes harness, and published tracked signals in `docs/project/status/editor_ux.json` (workflow inventory coverage, PR-lane coverage, top-line instrumentation rate) | Anything about parser breadth, protocol catalog size, or capability count |
