...
| End-to-end UX confidence | Tracked composite | Published in `docs/project/status/editor_ux.json`: harness workflow inventory coverage (`workflow_inventory_coverage_rate`) plus explicit tracked-signal inventory (manual smoke workflows, first-5-minutes harness, known-gap issue backlog) | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: *none* of the automated parser/capability metrics above directly measure whether a real editing session feels good. We now publish a UX confidence tracking receipt (`docs/project/status/editor_ux.json`) that records harness coverage and tracked UX signals, while keeping the known-gap list below as the product-facing backlog.
