...
| End-to-end UX confidence | Tracked workflow lane coverage: **94.1% PR** (16/17) + **100% full lane** (17/17) | Quantified from the `perl-lsp-ux-tests` first-5-minutes fixture matrix (`ci_tier` split) and published in `docs/project/status/editor_ux.json` via `cargo xtask update-status --only quality` | Anything about parser breadth, protocol catalog size, or capability count |

The last row is the important one: *none* of the automated metrics above fully capture whether a real editing session feels good. We now publish fixture-backed workflow lane coverage for first-5-minutes UX, but we still pair that with workflow smoke tests and the known-gap list below.
