...
| End-to-end UX confidence | Scorecard-backed (see `docs/project/status/editor_ux.json`) | Tracked by the `perl-lsp-ux-tests` first-5-minutes harness plus top-line scorecard metrics (`workflow_pass_rate`, `workflow_stability_rate`, `p95_time_to_first_useful_result_ms`) published via `cargo xtask update-status --only quality` | Anything about parser breadth, protocol catalog size, or capability count |
