...
impl UxRunRecorder {
    ...
    /// Finalize as skipped with a failure class and return the receipt.
    pub fn finish_skipped(&self, class: UxFailureClass) -> UxScenarioRunReceipt { /* ... */ }
    ...
    F: FnOnce(&mut UxRunRecorder) -> anyhow::Result<()>,
{
    ...
    match result {
        ...
        Ok(Err(ref err)) => {
            let receipt = if let Some(skip) = err.downcast_ref::<UxScenarioSkip>() {
                recorder.finish_skipped(skip.failure_class)
            } else {
                recorder.finish_fail(classify_error(err))
            };
            let _ = recorder.write_receipt(&receipt);
        }
        Err(panic_payload) => {
