...
- In-progress slices: `grep "in-progress" .ops-perl-lsp/completed-slices.md 2>/dev/null`
...
Read .ops-perl-lsp/discovered-issues.md and completed-slices.md for dedup.
...
Check: .ops-perl-lsp/discovered-issues.md (agent-flagged leads).
Launch 5-8 Explore subagents per round. Write handoff files. Use TaskCreate for each slice.
Message builder-1 and builder-2 when tasks are ready.
```
...
Subagent prompt pattern (minimal — 7 lines):
  "Read .ops-perl-lsp/handoffs/<branch>.md for context and test template.
   Read .ops-perl-lsp/known-pitfalls.md for traps.
   Invoke /swarm-protocol and /coding-standards.
...
After each merge, update .ops-perl-lsp/completed-slices.md status to "merged".
...
Append to .ops-perl-lsp/known-pitfalls.md for reusable lessons.
