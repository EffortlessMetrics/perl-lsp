...
Push improvements directly: add edge case tests, fix logic issues, simplify code. Then approve with a summary of what you changed and set the `reviewed-deep` label to signal approval.
...
gh pr edit <number> --add-label "reviewed-deep"
...
- **You are the final quality gate.** On approval, you MUST set the `reviewed-deep` label. Without it, the PR cannot merge.
...
- **Recommend next steps.** Typical recommendations:
  - "Approved with improvements — `reviewed-deep` set, ready for merge"
