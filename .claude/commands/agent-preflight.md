...
5. **No git stash entries** — Git stash is shared across all worktrees. Any entries risk cross-contamination between agents. Exit 5 if failed.
6. **CARGO_TARGET_DIR isolation** — Computes the recommended `CARGO_TARGET_DIR` (a per-branch path under `/tmp/`) and reports it. Prevents shared build artifact collisions between concurrent agents. **Note:** Because the script runs in a subshell, you must set the variable yourself before running cargo commands — see the builder environment setup section.
