...
export function describeDownloadTarget(target: string): string {
    switch (target) {
        case 'x86_64-unknown-linux-gnu':
            return 'Linux x64 / AMD64 using glibc (most distributions)';
        case 'aarch64-unknown-linux-gnu':
            return 'Linux ARM64 using glibc (most distributions)';
        case 'x86_64-unknown-linux-musl':
            return 'Linux x64 / AMD64 using musl (Alpine/musl)';
        case 'aarch64-unknown-linux-musl':
            return 'Linux ARM64 using musl (Alpine/musl)';
        ...
        case 'x86_64-linux-android':
            return 'Android/Termux x64';
        case 'aarch64-linux-android':
            return 'Android/Termux ARM64';
        default:
            return target;
    }
}
...
export function linuxLibcChoiceToTargetSuffix(choice: string | undefined): 'gnu' | 'musl' | null {
    ...
    if (normalized === 'glibc' || normalized === 'gnu') {
        return 'gnu';
    }
    if (normalized === 'musl') {
        return 'musl';
...
export class BinaryDownloader {
    ...
    async ensureBinary(forceDownload = false): Promise<string | null> {
        ...
        try {
            ...
            if (errorMsg.includes('ECONNREFUSED') || errorMsg.includes('ETIMEDOUT') || errorMsg.includes('timeout')) {
                ...
                const platformMatch = /detected target:\s*([^)\s]+)/.exec(errorMsg)
                    ?? /platform:\s*([^\s.]+)/.exec(errorMsg);
    ...
    private async downloadWithProgress(): Promise<string> {
        return vscode.window.withProgress({
            ...
            if (!asset || !assetName) {
                ...
                this.outputChannel.appendLine(`Detected platform: ${describeDownloadTarget(target)}`);
                this.outputChannel.appendLine(`Detected target: ${target}`);
                this.outputChannel.appendLine(`Available assets: ${availableAssets}`);
                throw new Error(`No pre-built binary found for ${describeDownloadTarget(target)} (detected target: ${target}). Available assets: ${availableAssets}`);
            }
    ...
    private detectMusl(): boolean {
        ...
        try {
            const result = child_process.execFileSync('ldd', ['--version'], {
                encoding: 'utf8',
